...
exports.createAgentGameRuntime = createAgentGameRuntime;
const node_crypto_1 = require("node:crypto");
const types_1 = require("./types");
const groupChat_1 = require("./groupChat");
const gamePackage_1 = require("./gamePackage");
...
function asAppSessionError(error, fallback) {
    if (isRecord(error) && typeof error.code === 'string' && typeof error.message === 'string') {
        return error;
    }
    return (0, types_1.createAppSessionError)(fallback, error instanceof Error ? error.message : String(error));
}
function throwAppSessionError(error, fallback) {
    throw asAppSessionError(error, fallback);
}
function emptyBudget() {
    return { llmCalls: 0, llmCallsUsed: 0, writes: 0, writesUsed: 0 };
}
function leaseKey(groupId, seat) {
    return `${groupId}|${seat}`;
}
...
function protocolHash(hex) {
    return `sha256:${hex}`;
}
function backoffMs(baseMs, maxMs, attempt, factor = 2) {
...
function sanitizeStartParams(params) {
...
function createAgentGameRuntime(input) {
    ...
    const adapterSandboxFactory = input.createAdapterSandbox ?? ((sandboxInput) => {
        ...
        const { createAdapterSandbox } = require('./adapterSandbox');
        return createAdapterSandbox(sandboxInput);
    });
    let state = { version: 1, sessions: [], grants: [], leases: [] };
    const adapters = new Map();
    const busy = new Set();
    const dirty = new Set();
    const ownerId = `oac-daemon-${process.pid}-${(0, node_crypto_1.randomUUID)()}`;
    let heartbeat = null;
    let restored = false;
    let disposed = false;
    function persist() {
    ...
    function actorMayAccess(session, actor) {
        return session.agentId === actor.actorGlobalMetaId || session.actorId === actor.actorId;
    }
    function requireAccess(session, actor) {
        if (!actorMayAccess(session, actor)) {
            throwAppSessionError((0, types_1.createAppSessionError)('session_not_found', `session ${session.sessionId} not found`), 'session_not_found');
        }
    }
    function acquireLease(key, sessionId) {
        const now = nowMs();
        ...
        state.leases.push({
            key,
            sessionId,
            leaseId: `lease-${(0, node_crypto_1.randomUUID)()}`,
            ownerId,
    ...
    // ---- Adapter helpers ----
    async function loadSessionAdapter(session) {
        ...
        if (pkg.adapterHash !== session.adapterHash) {
            throwAppSessionError((0, types_1.createAppSessionError)('adapter_invalid', 'adapterHash changed after session start; package is frozen.'), 'adapter_invalid');
        }
    ...
    async function smokeLoadAdapter(adapter, gameId) {
        try {
            const missing = [];
            ...
            if (missing.length) {
                throwAppSessionError((0, types_1.createAppSessionError)('adapter_invalid', `Adapter is missing required ABI exports: ${missing.join(', ')}`), 'adapter_invalid');
            }
    ...
    async function adapterCall(adapter, method, args) {
        if (!adapter) {
            throwAppSessionError((0, types_1.createAppSessionError)('adapter_error', 'Adapter is not loaded for this session.'), 'adapter_error');
        }
        try {
            return await adapter.call(method, args);
        }
        catch (error) {
    ...
    // ---- Grant / budget ----
    function assertGrantActive(session) {
        const grant = grantById(session.grantId);
        if (!grant || grant.revoked) {
            throwAppSessionError((0, types_1.createAppSessionError)('consent_denied', 'The task authorization was revoked or no longer exists.'), 'consent_denied');
        }
        if (nowMs() >= grant.expiresAt) {
            throwAppSessionError((0, types_1.createAppSessionError)('authorization_expired', 'The task authorization expired.'), 'authorization_expired');
        }
        return grant;
    }
    function assertGrantBinding(session, grant) {
        ...
        if (!bound) {
            throwAppSessionError((0, types_1.createAppSessionError)('consent_denied', 'The task authorization does not match this session.'), 'consent_denied');
        }
    }
    async function autoPauseIfNeeded(session) {
        const grant = grantById(session.grantId);
        if (!grant || grant.revoked) {
            session.status = 'stopped';
            session.lastError = (0, types_1.createAppSessionError)('consent_denied', 'The task authorization was revoked.');
        ...
        if (nowMs() >= grant.expiresAt) {
            ...
            session.lastError = (0, types_1.createAppSessionError)('authorization_expired', 'The task authorization expired; the session is paused.');
            markDirty(session);
            await persist();
            return true;
        }
        if (session.budget.llmCallsUsed >= session.budget.llmCalls
            ...
            session.status = 'paused';
            session.lastError = (0, types_1.createAppSessionError)('budget_exhausted', 'The session budget is exhausted; the session is paused.');
            markDirty(session);
            await persist();
            return true;
        }
    ...
    // ---- Message intake ----
    async function catchUp(session) {
        ...
        for (;;) {
            ...
            try {
                ...
                if (status === 404 || status === 403) {
                    throwAppSessionError((0, types_1.createAppSessionError)('group_not_found', `Group is not accessible: ${session.groupId}`), 'group_not_found');
                }
                throw error;
            }
            if (!batch.length) {
            ...
            for (const message of batch) {
                ...
                const plaintext = message.encryption === 'aes'
                    ? (0, groupChat_1.decryptGroupContent)(message.content, session.groupId)
                    : message.content;
                const envelope = (0, groupChat_1.parseAgentGameEnvelope)(plaintext);
                ...
                if (envelope.type === 'action' && envelope.prevStateHash) {
                    const currentHash = await adapterCall(adapter, 'serializeState', [session.state]).then((serialized) => (0, gamePackage_1.sha256Hex)(JSON.stringify(serialized)));
    ...
    // ---- Envelope + write ----
    function envelopeFor(session, type, payload, extra = {}) {
        return {
    ...
    async function writeEvent(session, event) {
        ...
        if (!grant.protocolPaths.includes(path)) {
            throwAppSessionError((0, types_1.createAppSessionError)('consent_denied', `Protocol path is not covered by the task authorization: ${path}`), 'consent_denied');
        }
        if (session.budget.writesUsed >= session.budget.writes) {
            throwAppSessionError((0, types_1.createAppSessionError)('rate_limited', 'The session write budget is exhausted.'), 'rate_limited');
        }
        ...
        if (!result.ok) {
            ...
            throwAppSessionError((0, types_1.createAppSessionError)(code, result.message || 'Group chat write failed.'), code);
        }
    ...
    // ---- Action loop ----
    async function callLlmWithRetry(session, observation, schema, context, feedback) {
        if (session.budget.llmCallsUsed >= session.budget.llmCalls) {
            throwAppSessionError((0, types_1.createAppSessionError)('rate_limited', 'The session LLM budget is exhausted.'), 'rate_limited');
        }
        const messages = [
            {
                role: 'system',
        ...
        for (let attempt = 1; attempt <= maxLlmAttempts; attempt += 1) {
            ...
            try {
                ...
                const code = isRecord(error) && typeof error.code === 'string'
                    ? String(error.code)
                    : 'llm_unavailable';
                lastError = (0, types_1.createAppSessionError)(code === 'llm_timeout' ? 'llm_timeout' : 'llm_unavailable', error instanceof Error ? error.message : String(error));
        ...
        session.status = 'paused';
        session.lastError = lastError ?? (0, types_1.createAppSessionError)('llm_unavailable', 'LLM is unavailable.');
        markDirty(session);
        await persist();
        throw lastError ?? (0, types_1.createAppSessionError)('llm_unavailable', 'LLM is unavailable.');
    }
    async function retryPending(session) {
        if (!session.pending) {
            return;
        }
        ...
        if (session.pending.writeCount >= maxWriteAttempts) {
            session.status = 'paused';
            session.lastError = (0, types_1.createAppSessionError)('write_failed', 'The action could not be written after repeated retries.');
            markDirty(session);
            await persist();
            return;
        }
    ...
    async function processTurn(session) {
        ...
        try {
            ...
            const adapter = adapterFor(session);
            const turn = await adapterCall(adapter, 'getTurn', [session.state]);
            ...
            for (let attempt = 1; attempt <= 3; attempt += 1) {
                parsed = await adapterCall(adapter, 'parseAction', [text, { schema, observation }]);
                ...
                if (attempt === 3) {
                    ...
                    session.lastError = (0, types_1.createAppSessionError)('adapter_error', parsed?.error || 'Adapter could not parse the LLM action after retries.');
                    markDirty(session);
                    await persist();
                    return;
                }
            ...
            const validated = await adapterCall(adapter, 'validateAction', [session.state, action, { schema, observation }]);
            if (!validated || validated.ok !== true) {
                session.status = 'error';
                session.lastError = (0, types_1.createAppSessionError)(validated?.code && validated.code !== 'adapter_error' ? validated.code : 'adapter_error', validated?.message || 'Adapter rejected the proposed action.');
                markDirty(session);
                await persist();
                return;
            }
            ...
            if (!Number.isInteger(actionSeq) || actionSeq <= 0) {
                session.status = 'error';
                session.lastError = (0, types_1.createAppSessionError)('adapter_error', 'Adapter getTurn returned an invalid actionSeq.');
                markDirty(session);
                await persist();
                return;
            }
            const prevStateHash = await adapterCall(adapter, 'serializeState', [session.state])
                .then((serialized) => (0, gamePackage_1.sha256Hex)(JSON.stringify(serialized)));
            ...
            const draftEvent = envelopeFor(session, 'action', actionPayload, {
                ...
                stateHash: '',
                eventId: `draft:${(0, node_crypto_1.randomUUID)()}`,
            });
            ...
            const stateHash = await adapterCall(adapter, 'serializeState', [draft])
                .then((serialized) => (0, gamePackage_1.sha256Hex)(JSON.stringify(serialized)));
            const event = envelopeFor(session, 'action', actionPayload, {
                actionSeq,
                prevStateHash: protocolHash(prevStateHash),
                stateHash: protocolHash(stateHash),
                eventId: `${session.agentId}:${(0, node_crypto_1.randomUUID)()}`,
            });
            session.pending = {
                event,
                actionSeq,
    ...
    function scheduleProcess(session) {
    ...
    // ---- Session control ----
    async function validateStart(startInput) {
        try {
    ...
    async function validateStartOrThrow(startInput) {
        await ensureLoaded();
        ...
        if (!params.appId || !params.groupId || !params.gameId || !params.manifestUri
            ...
            throwAppSessionError((0, types_1.createAppSessionError)('invalid_params', 'appId, sessionType, groupId, gameId, manifestUri, rulesHash, seat and agentId are required.'), 'invalid_params');
        }
        if (params.sessionType !== types_1.APP_SESSION_TYPE) {
            throwAppSessionError((0, types_1.createAppSessionError)('invalid_params', `sessionType must be "${types_1.APP_SESSION_TYPE}".`), 'invalid_params');
        }
        if (params.agentId !== startInput.actorGlobalMetaId) {
            throwAppSessionError((0, types_1.createAppSessionError)('invalid_params', 'agentId must match the current actor globalMetaId.'), 'invalid_params');
        }
        const pkg = await input.loadGamePackage({ manifestUri: params.manifestUri });
        if (pkg.manifest.gameId !== params.gameId) {
            throwAppSessionError((0, types_1.createAppSessionError)('adapter_invalid', `gameId does not match the game manifest: ${pkg.manifest.gameId}`), 'adapter_invalid');
        }
        const maxPlayers = pkg.manifest.maxPlayers;
        if (Number.isInteger(maxPlayers) && Number(maxPlayers) > 0 && Number(maxPlayers) < 2) {
            throwAppSessionError((0, types_1.createAppSessionError)('adapter_invalid', 'game manifest maxPlayers must be at least 2.'), 'adapter_invalid');
        }
        const adapter = adapterSandboxFactory({
            adapterCode: pkg.adapterCode,
            adapterHash: pkg.adapterHash,
        ...
        if (!matchCreated) {
            throwAppSessionError((0, types_1.createAppSessionError)('group_not_found', `Group has no agent-game match.created event: ${params.groupId}`), 'group_not_found');
        }
        if (matchCreated.rulesHash && matchCreated.rulesHash !== params.rulesHash) {
            throwAppSessionError((0, types_1.createAppSessionError)('rules_hash_mismatch', 'rulesHash does not match the match.created event.'), 'rules_hash_mismatch');
        }
        if (matchCreated.gameId && matchCreated.gameId !== params.gameId) {
            throwAppSessionError((0, types_1.createAppSessionError)('adapter_invalid', 'gameId does not match the match.created event.'), 'adapter_invalid');
        }
        ...
        if (existingLease && existingLease.expiresAt > nowMs()) {
            throwAppSessionError((0, types_1.createAppSessionError)('session_conflict', `Seat ${params.seat} of group ${params.groupId} is already leased.`), 'session_conflict');
        }
        return pkg;
    }
    async function readMatchCreated(groupId) {
        let startIndex = 0;
        for (;;) {
            let batch;
            try {
                ...
                const status = error.status;
                if (status === 404 || status === 403) {
                    throwAppSessionError((0, types_1.createAppSessionError)('group_not_found', `Group is not accessible: ${groupId}`), 'group_not_found');
                }
                throw error;
            }
            if (!batch.length) {
            ...
            for (const message of batch) {
                const plaintext = message.encryption === 'aes'
                    ? (0, groupChat_1.decryptGroupContent)(message.content, groupId)
                    : message.content;
                const envelope = (0, groupChat_1.parseAgentGameEnvelope)(plaintext);
    ...
    async function checkSeatClaimable(params, pkg) {
        const adapter = adapterSandboxFactory({
            adapterCode: pkg.adapterCode,
            adapterHash: pkg.adapterHash,
        });
        try {
            ...
            try {
                ...
                return { ok: false, error: (0, types_1.createAppSessionError)('adapter_invalid', `Adapter initialState failed: ${error instanceof Error ? error.message : String(error)}`) };
            }
            let startIndex = 0;
            for (;;) {
                ...
                for (const message of batch) {
                    const plaintext = message.encryption === 'aes'
                        ? (0, groupChat_1.decryptGroupContent)(message.content, params.groupId)
                        : message.content;
                    const envelope = (0, groupChat_1.parseAgentGameEnvelope)(plaintext);
            ...
            try {
                const turn = await adapter.call('getTurn', [state]);
                if (turn && turn.phase === 'finished') {
                    return { ok: false, error: (0, types_1.createAppSessionError)('seat_unavailable', 'The match is already finished; seats are not claimable.') };
                }
            }
            catch {
            ...
            if (existingSeat && normalizeText(existingSeat) && normalizeText(existingSeat) !== params.seat) {
                return { ok: false, error: (0, types_1.createAppSessionError)('seat_unavailable', `Agent is already seated as ${normalizeText(existingSeat)}.`) };
            }
            return { ok: true };
        }
        catch (error) {
            return {
                ok: false,
                error: (0, types_1.createAppSessionError)('adapter_error', `Seat check failed: ${error instanceof Error ? error.message : String(error)}`),
            };
        }
        finally {
            adapter.dispose();
    ...
    async function claimSeat(session, adapter) {
        let existingSeat = null;
        try {
        ...
        if (existingSeat && normalizeText(existingSeat) && normalizeText(existingSeat) !== session.seat) {
            throwAppSessionError((0, types_1.createAppSessionError)('seat_unavailable', `Agent is already seated as ${normalizeText(existingSeat)}.`), 'seat_unavailable');
        }
        if (!existingSeat) {
            ...
            for (let attempt = 1; attempt <= maxWriteAttempts; attempt += 1) {
                try {
                    ...
                    if (errorCode === 'authorization_expired' || errorCode === 'consent_denied' || errorCode === 'rate_limited') {
                        throwAppSessionError(error, errorCode);
                    }
            ...
            if (!claimed) {
                throwAppSessionError((0, types_1.createAppSessionError)('write_failed', 'Seat claim could not be written.'), 'write_failed');
            }
            try {
                let nowSeat = null;
                let hasGetSeat = true;
                ...
                if (hasGetSeat && !nowSeat) {
                    throwAppSessionError((0, types_1.createAppSessionError)('seat_unavailable', `Seat ${session.seat} of ${session.groupId} is not claimable.`), 'seat_unavailable');
                }
            }
            catch (error) {
    ...
    async function start(startInput) {
        if (disposed) {
            throwAppSessionError((0, types_1.createAppSessionError)('internal_error', 'Runtime is disposed.'), 'internal_error');
        }
        await ensureLoaded();
        const params = sanitizeStartParams(startInput);
        const existing = state.sessions.find((session) => session.groupId === params.groupId
        ...
        const key = leaseKey(params.groupId, params.seat);
        const sessionId = `sess-${(0, node_crypto_1.randomUUID)()}`;
        ...
        try {
            ...
            const now = nowMs();
            const grant = {
                grantId: `grant-${(0, node_crypto_1.randomUUID)()}`,
            ...
            session = {
                sessionId,
                appId: params.appId,
                sessionType: types_1.APP_SESSION_TYPE,
                resourceUri: startInput.resourceUri,
                groupId: params.groupId,
                gameId: params.gameId,
    ...
    function releaseLeaseByKey(key, sessionId) {
    ...
    async function status(sessionId, actor) {
        if (!restored) {
            throwAppSessionError((0, types_1.createAppSessionError)('session_not_found', `session ${sessionId} not found`), 'session_not_found');
        }
    ...
    async function pause(sessionId, actor) {
        await ensureLoaded();
        const session = sessionById(sessionId);
        if (!session) {
            throwAppSessionError((0, types_1.createAppSessionError)('session_not_found', `session ${sessionId} not found`), 'session_not_found');
    ...
    async function resume(sessionId, actor) {
        await ensureLoaded();
        const session = sessionById(sessionId);
        if (!session) {
            throwAppSessionError((0, types_1.createAppSessionError)('session_not_found', `session ${sessionId} not found`), 'session_not_found');
        ...
        if (!acquireLease(key, session.sessionId)) {
            session.lastError = (0, types_1.createAppSessionError)('session_conflict', 'Seat is leased by another runner.');
            markDirty(session);
            await persist();
            throwAppSessionError((0, types_1.createAppSessionError)('session_conflict', 'Seat is leased by another runner.'), 'session_conflict');
        }
        try {
    ...
    async function stop(sessionId, actor, options = {}) {
        await ensureLoaded();
        const session = sessionById(sessionId);
        if (!session) {
            throwAppSessionError((0, types_1.createAppSessionError)('session_not_found', `session ${sessionId} not found`), 'session_not_found');
        ...
        if (session.status !== 'stopped') {
            if (options.releaseSeat === true) {
                ...
                if (adapter) {
                    try {
                        const releaseEvent = await adapterCall(adapter, 'releaseSeat', [session.state, { seat: session.seat, groupId: session.groupId }]);
    ...
    function notifyGroupActivity(groupId) {
    ...
    async function startRuntime() {
        ...
        restored = true;
        state = (await input.store.load()) ?? { version: 1, sessions: [], grants: [], leases: [] };
        const report = { restored: 0, running: 0, paused: 0, stopped: 0, conflicts: 0 };
        for (const session of state.sessions) {
            ...
            if (nowMs() >= grant.expiresAt) {
                session.status = 'paused';
                session.lastError = (0, types_1.createAppSessionError)('authorization_expired', 'The task authorization expired.');
                report.paused += 1;
                continue;
            }
            if (session.budget.llmCallsUsed >= session.budget.llmCalls
                || session.budget.writesUsed >= session.budget.writes) {
                session.status = 'paused';
                session.lastError = (0, types_1.createAppSessionError)('budget_exhausted', 'The session budget is exhausted.');
                report.paused += 1;
                continue;
            }
            try {
