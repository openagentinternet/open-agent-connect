...
exports.CHAIN_HISTORY_SUMMARY_MAX_CHARS = exports.CHAIN_HISTORY_SUMMARY_LLM_TIMEOUT_MS = exports.CHAIN_HISTORY_SUMMARY_DEFAULT_DAILY_CAP = exports.CHAIN_HISTORY_SUMMARY_DEFAULT_PER_TICK = void 0;
exports.isDshHostExecutorConnected = isDshHostExecutorConnected;
exports.runDreamAutomationTick = runDreamAutomationTick;
exports.buildChainHistorySummaryPrompt = buildChainHistorySummaryPrompt;
exports.runChainHistorySummaryTick = runChainHistorySummaryTick;
exports.reportDreamTickOutcomes = reportDreamTickOutcomes;
exports.reportChainHistorySummaryTickOutcomes = reportChainHistorySummaryTickOutcomes;
exports.startAutomationTickLoop = startAutomationTickLoop;
...
function isDshHostExecutorConnected() {
    return ((0, hostLlmExecutorBridge_1.getActiveHostLlmExecutorBridge)()?.connectedExecutors() ?? 0) > 0;
}
function errorMessage(error) {
    return error instanceof Error ? error.message : String(error);
}
function readDateList(value) {
 ...
 */
async function runDreamAutomationTick(deps) {
    const outcomes = [];
    ...
    for (const outcome of outcomes) {
        if (handsOffSlugs.has(outcome.slug))
            continue;
        try {
...
// ---------------------------------------------------------------------------
exports.CHAIN_HISTORY_SUMMARY_DEFAULT_PER_TICK = 10;
exports.CHAIN_HISTORY_SUMMARY_DEFAULT_DAILY_CAP = 40;
/** Summaries are short; a 60s budget is generous even for a slow runtime. */
exports.CHAIN_HISTORY_SUMMARY_LLM_TIMEOUT_MS = 60_000;
/** Stored summaries are capped so a chatty model cannot bloat the ledger. */
exports.CHAIN_HISTORY_SUMMARY_MAX_CHARS = 500;
/** Port of the plugin prompt builder (`dsh-plugin/src/chain-history-summary.ts`). */
function buildChainHistorySummaryPrompt(input) {
    const system = [
 ...
 */
async function runChainHistorySummaryTick(deps) {
    const outcomes = [];
    if (deps.isHostExecutorConnected()) {
        // Stand-down: DSH owns the summary drain while its host executor is connected.
        return outcomes;
    }
    const perTick = Math.max(1, Math.floor(deps.perTick ?? exports.CHAIN_HISTORY_SUMMARY_DEFAULT_PER_TICK));
    const dailyCap = Math.max(1, Math.floor(deps.dailyCap ?? exports.CHAIN_HISTORY_SUMMARY_DEFAULT_DAILY_CAP));
    ...
    for (const profile of profiles) {
        ...
        if (!slug)
            continue;
        const outcome = { slug, done: 0, failed: 0 };
        outcomes.push(outcome);
        try {
            ...
            for (const item of items) {
                if (tickRemaining <= 0 || dailyRemaining <= 0)
                    break;
                ...
                try {
                    ...
                    const trimmed = summary.trim().slice(0, exports.CHAIN_HISTORY_SUMMARY_MAX_CHARS);
                    if (!trimmed)
...
function reportDreamTickOutcomes(outcomes, log) {
    for (const outcome of outcomes) {
        if (outcome.error) {
...
function reportChainHistorySummaryTickOutcomes(outcomes, log) {
    for (const outcome of outcomes) {
        if (outcome.error) {
 ...
 */
function startAutomationTickLoop(pass, options) {
