...
contract _opcat_labs_scrypt_ts_opcat_4_1_0__rs__Genesis {
  constructor() {
  }
  public function checkDeploy(_opcat_labs_scrypt_ts_opcat_4_1_0__rs__TxOut[6] outputs, int outputCount, _opcat_labs_scrypt_ts_opcat_4_1_0__rs__SHPreimage __scrypt_ts_shPreimage, Sig __scrypt_ts_preimageSig, bytes __scrypt_ts_spentAmounts, bytes __scrypt_ts_spentScriptHashes) {
    ...
    bytes outputBytes = b'';
    loop (6) : index {
        _opcat_labs_scrypt_ts_opcat_4_1_0__rs__TxOut _output = outputs[index];
        if(index < outputCount) {
          require(len(_output.scriptHash) > 0);
          outputBytes += _opcat_labs_scrypt_ts_opcat_4_1_0__rs__TxUtils.buildDataOutput(_output.scriptHash, _output.satoshis, _output.dataHash);
          if(index > 0) {
    ...
    loop (6) : index {
        if(index < __scrypt_ts_inputCount) {
          bytes inputScriptHash = _opcat_labs_scrypt_ts_opcat_4_1_0__rs__ContextUtils.getSpentScriptHash(__scrypt_ts_spentScriptHashes, (index));
