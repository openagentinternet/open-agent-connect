...
exports.checkDataSigImpl = void 0;
 ...
 */
function checkDataSigImpl(
// eslint-disable-next-line @typescript-eslint/no-unused-vars
self, signature, message, publickey) {
    const bufSig = Buffer.from((0, index_js_1.toByteString)(signature), 'hex');
    const bufPubkey = Buffer.from((0, index_js_1.toByteString)(publickey), 'hex');
    // Use Interpreter instance for encoding validation (reuse opcat library methods)
    const interpreter = new opcat_1.Interpreter();
    interpreter.flags = opcat_1.Interpreter.DEFAULT_FLAGS;
    ...
    try {
        ...
        const sig = opcat_1.crypto.Signature.fromDER(bufSig);
        ...
        const hashbuf = Buffer.from((0, index_js_1.sha256)(message), 'hex').reverse();
...
exports.checkDataSigImpl = checkDataSigImpl;
