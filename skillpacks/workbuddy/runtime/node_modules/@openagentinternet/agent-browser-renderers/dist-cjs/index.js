...
exports.DEFAULT_PROTOCOL_RENDERERS = exports.renderPinInspectorHtml = void 0;
exports.renderSimpleBuzzDetail = renderSimpleBuzzDetail;
exports.renderSkillServiceDetail = renderSkillServiceDetail;
exports.renderGenericProtocolPin = renderGenericProtocolPin;
exports.renderProtocolPinHtml = renderProtocolPinHtml;
const pinInspector_js_1 = require("./pinInspector.js");
var pinInspector_js_2 = require("./pinInspector.js");
Object.defineProperty(exports, "renderPinInspectorHtml", { enumerable: true, get: function () { return pinInspector_js_2.renderPinInspectorHtml; } });
exports.DEFAULT_PROTOCOL_RENDERERS = [
...
function versionBlock(resource) {
    return `<dl class="browser-protocol-proof">${Object.entries(record(data(resource).version)).map(([key, value]) => (`<dt>${escapeHtml(key)}</dt><dd>${escapeHtml(value)}</dd>`)).join('')}</dl>`;
}
function renderSimpleBuzzDetail(resource) {
...
function renderSkillServiceDetail(resource) {
    const body = record(payload(resource));
    ...
    <dl class="browser-protocol-fields">
      ${['price', 'pricing', 'serviceType', 'provider', 'endpoint', 'inputSchema', 'outputSchema'].map((key) => (Object.prototype.hasOwnProperty.call(body, key)
...
function renderGenericProtocolPin(resource) {
    return (0, pinInspector_js_1.renderPinInspectorHtml)(resource, 'Generic protocol pin');
}
function renderProtocolPinHtml(resource) {
    const rendererId = text(data(resource).rendererId);
    if (rendererId === 'simplebuzz.detail')
        return renderSimpleBuzzDetail(resource);
    if (rendererId === 'skill-service.detail')
        return renderSkillServiceDetail(resource);
