...
const botHomepageTemplates_js_1 = require("./botHomepageTemplates.js");
...
function validateHttpUrlList(value) {
...
function readRequiredObject(value, message) {
    if (value && typeof value === 'object' && !Array.isArray(value)) {
        return value;
    }
    throw new Error(message);
}
function readNameResolutionConfig(value, defaults) {
    return {
...
function readOptionalBoolean(value, fallback, message) {
    if (value === undefined)
        return fallback;
...
function mergeNameResolutionSettings(current, input, defaults) {
...
function createBrowserSettingsSnapshot(input) {
    const defaults = (0, config_js_1.createDefaultBrowserConfig)();
    return {
        browser: { ...(input.config.browser ?? {}) },
        effectiveBrowser: { ...(0, config_js_1.resolveBrowserConfig)(input.config, input.env ?? {}) },
        defaults: { ...defaults },
...
function applyBrowserSettingsUpdate(current, rawBrowserInput) {
    ...
    if (Object.prototype.hasOwnProperty.call(browserInput, 'botHomepageTemplateId')) {
        ...
        if (!(0, botHomepageTemplates_js_1.isBotHomepageTemplateId)(templateId)) {
            throw new Error(`browser.botHomepageTemplateId must be one of ${botHomepageTemplates_js_1.BOT_HOMEPAGE_TEMPLATES.map((template) => template.id).join(', ')}.`);
        }
    ...
    if (Object.prototype.hasOwnProperty.call(browserInput, 'nameResolution')) {
        nextBrowser.nameResolution = mergeNameResolutionSettings(nextBrowser.nameResolution, browserInput.nameResolution, defaults.nameResolution);
    }
