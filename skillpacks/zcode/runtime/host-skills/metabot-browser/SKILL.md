...
This ZCode setup has no documented in-app browser or preview surface. Present every `localUiUrl` returned by the MetaBot CLI as a clickable markdown link for the human to open. If the running session does provide a web preview surface, prefer opening the `localUiUrl` there instead of handing it to the external browser.
