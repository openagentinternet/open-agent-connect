...
Open pages exactly as this skill instructs — through the MetaBot CLI open commands (`browser tab open` first, `browser open` when no Browser page is running); the daemon pushes the page into every open Browser page regardless of host. Always also present every `localUiUrl` returned by the MetaBot CLI as a clickable absolute-URL markdown link for the human. If this ZCode session provides a web preview surface, prefer opening the `localUiUrl` there instead of handing it to the external browser; never use the external system browser or external browser automation.
