...
function injectOacBrowserLocalization(script) {
    ...
    const withLocalizedText = script.replace(browserTextPattern, `${OAC_BROWSER_LOCALIZATION_SCRIPT}\nfunction browserText(key, fallback) {\n  return oacBrowserText(key, fallback);\n}`);
    return withLocalizedText.replace(runtimeLabelValuePattern, `function runtimeLabelValue(key, fallback) {\n  var raw = textValue(runtimeLabels()[key]) || fallback;\n  return browserText('runtime.' + key, raw);\n}`);
}
