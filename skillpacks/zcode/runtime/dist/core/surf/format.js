...
exports.formatSurfRunList = formatSurfRunList;
exports.formatSurfStatusText = formatSurfStatusText;
exports.formatSurfBatchPins = formatSurfBatchPins;
exports.formatSurfPinVersions = formatSurfPinVersions;
exports.formatSurfProtocolRegistry = formatSurfProtocolRegistry;
exports.formatSurfSocialPosts = formatSurfSocialPosts;
exports.formatSurfSocialPostDetail = formatSurfSocialPostDetail;
exports.formatSurfSocialComments = formatSurfSocialComments;
const surfReads_js_1 = require("./surfReads.js");
/** Human-readable surf run list (max 10 rows, newest first). */
function formatSurfRunList(runs) {
    if (runs.length === 0) {
...
function formatSurfStatusText(input) {
...
function formatSurfBatchPins(entries) {
    const blocks = [];
    for (const [requestedId, entry] of Object.entries(entries)) {
        if ((0, surfReads_js_1.isBatchErrorEntry)(entry)) {
        ...
        if (entry.meta.title)
            lines.push(`- title: ${clip(entry.meta.title, 200)}`);
        if (entry.text != null) {
        ...
        if (entry.version.count != null)
            lines.push(`- versions: ${entry.version.count}`);
        blocks.push(lines.join('\n'));
    }
    return blocks.join('\n\n') || '(no pins returned)';
}
/** R4 pin versions sheet. */
function formatSurfPinVersions(versions) {
    const lines = [
...
function formatSurfProtocolRegistry(page) {
    if (page.items.length === 0)
        return 'No registered protocols.';
    ...
    if (page.hasMore && page.nextCursor)
        lines.push(`(more: pass cursor="${page.nextCursor}")`);
    if (page.rejected.length)
        lines.push(`(${page.rejected.length} declaration(s) rejected by validation)`);
    return lines.join('\n');
}
/** Social feed bullets (search_social_posts). */
function formatSurfSocialPosts(posts, moreCursor) {
    if (posts.length === 0)
        return 'No social posts matched.';
    ...
    if (moreCursor)
        lines.push(`(more: pass cursor="${moreCursor}")`);
    return lines.join('\n');
}
/** Social post detail sheet. */
function formatSurfSocialPostDetail(post) {
    const lines = [
        `## ${post.pinId}`,
...
function formatSurfSocialComments(page) {
    if (page.items.length === 0)
