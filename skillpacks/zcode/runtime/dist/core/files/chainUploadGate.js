...
exports.wrapUploadWithGate = wrapUploadWithGate;
...
function resolveReal(value) {
    try {
        ...
        const parent = resolveReal(node_path_1.default.dirname(value));
        return node_path_1.default.join(parent, node_path_1.default.basename(value));
    }
}
/**
...
function isPathInsideDir(filePath, dir) {
    if (!filePath || !dir)
        return false;
    const rel = node_path_1.default.relative(resolveReal(node_path_1.default.resolve(dir)), resolveReal(node_path_1.default.resolve(filePath)));
    return rel === '' || (!rel.startsWith(`..${node_path_1.default.sep}`) && rel !== '..' && !node_path_1.default.isAbsolute(rel));
}
/**
 ...
 */
async function checkUploadAllowed(filePath, deps) {
    if (!deps.getWorkspaceDir || !deps.confirmExternalUpload)
        return null;
 ...
 */
function wrapUploadWithGate(upload, deps) {
    return (async (params) => {
