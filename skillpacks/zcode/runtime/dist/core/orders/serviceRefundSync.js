...
exports.applyServiceRefundRequestsToState = applyServiceRefundRequestsToState;
exports.applyServiceRefundFinalizationsToState = applyServiceRefundFinalizationsToState;
exports.mergeServiceRefundSyncState = mergeServiceRefundSyncState;
...
function inferPaymentChain(chain, asset) {
...
function toRequestContext(request) {
...
function toFinalizeContext(finalize) {
    return {
...
function uniqueByPriority(entries, priorities) {
...
function sellerOrderPins(order) {
    return [
...
function isBuyerOrderTrace(trace) {
    return Boolean(trace.order) && normalizeLower(trace.order?.role) === 'buyer';
}
function findBuyerTraceForRequest(traces, request) {
    return uniqueByPriority(traces.filter(isBuyerOrderTrace), [
...
function findSellerOrderForRequest(orders, request) {
    return uniqueByPriority(orders, [
...
function findBuyerTraceForFinalize(traces, finalize) {
    return uniqueByPriority(traces.filter(isBuyerOrderTrace), [
...
function findSellerOrderForFinalize(orders, finalize) {
    return uniqueByPriority(orders, [
...
function patchBuyerRequestTrace(trace, request, nowMs) {
    if (!trace.order) {
...
function patchSellerRequestOrder(order, request, nowMs) {
    ...
    const changesPatchFields = (normalizeText(order.refundRequestPinId) !== normalizeText(patch.refundRequestPinId)
    ...
    const next = (0, sellerOrderState_1.transitionSellerOrderRecord)(order, {
        ...patch,
...
function synthesizeSellerOrder(request, identity, nowMs) {
...
function canSynthesizeSellerOrder(state, request, identity) {
...
function applyServiceRefundRequestsToState(input) {
    ...
    for (const parsedRequest of input.requests) {
        ...
        if (buyerMatch.status === 'found' && buyerMatch.item) {
            ...
            if (patched.changed) {
                nextState = {
                    ...nextState,
                    traces: nextState.traces.map((trace) => (trace.traceId === buyerMatch.item?.traceId ? patched.trace : trace)),
                };
        ...
        if (sellerMatch.status === 'found' && sellerMatch.item) {
            ...
            if (patched.changed) {
                nextState = {
                    ...nextState,
                    sellerOrders: nextState.sellerOrders.map((order) => (order.id === sellerMatch.item?.id ? patched.order : order)),
                };
...
function sellerOrderAlreadyRefundedWithFinalizeProof(order, finalize) {
...
function finalizeAlreadyAppliedWithPaidProof(finalize, trace, order) {
    const matchedRows = [
...
function patchBuyerFinalizeTrace(trace, finalize, nowMs) {
    if (!trace.order) {
        return { trace, changed: false };
    }
...
function patchSellerFinalizeOrder(order, finalize, nowMs) {
    ...
    return {
        order: (0, sellerOrderState_1.transitionSellerOrderRecord)(pending, {
            ...patch,
            state: 'refunded',
            updatedAt: nowMs,
        }),
...
function blockBuyerFinalizeTrace(trace, reason, nowMs) {
    if (!trace.order)
        return trace;
    return {
        ...trace,
...
async function verifyPaidFinalize(finalize, input) {
...
async function applyServiceRefundFinalizationsToState(input) {
    const result = emptyCounters(input.state);
    let nextState = input.state;
    for (const parsedFinalize of input.finalizations) {
        ...
        if (matchedTrace) {
            ...
            nextState = {
                ...nextState,
                traces: nextState.traces.map((trace) => (trace.traceId === matchedTrace.traceId ? patched.trace : trace)),
            };
        }
        if (matchedOrder) {
            ...
            nextState = {
                ...nextState,
                sellerOrders: nextState.sellerOrders.map((order) => (order.id === matchedOrder.id ? patched.order : order)),
            };
        }
        if (changed) {
...
function blockFinalizeMatches(input) {
    ...
    if (input.trace && !traceAlreadyRefundedWithFinalizeProof(input.trace, input.finalize)) {
        nextState = {
            ...nextState,
            traces: nextState.traces.map((trace) => (trace.traceId === input.trace?.traceId
    ...
    if (input.order && !sellerOrderAlreadyRefundedWithFinalizeProof(input.order, input.finalize)) {
        nextState = {
            ...nextState,
            sellerOrders: nextState.sellerOrders.map((order) => (order.id === input.order?.id
...
function buyerTraceRefundMatchValues(trace) {
    return [
        trace.traceId,
        trace.order?.id,
...
function sellerOrderRefundMatchValues(order) {
    return [
        order.id,
...
function mergeTerminalRefundedTraces(currentTraces, syncedTraces) {
    const terminalCurrent = currentTraces.filter(buyerTraceHasTerminalRefundProof);
    const usedCurrentTraceIds = new Set();
    const merged = syncedTraces.map((syncedTrace) => {
        const matched = terminalCurrent.find((currentTrace) => (sharedNormalizedValue(buyerTraceRefundMatchValues(currentTrace), buyerTraceRefundMatchValues(syncedTrace))));
        if (!matched)
            return syncedTrace;
        usedCurrentTraceIds.add(matched.traceId);
        return matched;
    });
    const missingCurrent = terminalCurrent.filter((currentTrace) => (!usedCurrentTraceIds.has(currentTrace.traceId)
        && !syncedTraces.some((syncedTrace) => (sharedNormalizedValue(buyerTraceRefundMatchValues(currentTrace), buyerTraceRefundMatchValues(syncedTrace))))));
    return [...merged, ...missingCurrent];
}
function mergeTerminalRefundedSellerOrders(currentOrders, syncedOrders) {
    const terminalCurrent = currentOrders.filter(sellerOrderHasTerminalRefundProof);
    const usedCurrentOrderIds = new Set();
    const merged = syncedOrders.map((syncedOrder) => {
        const matched = terminalCurrent.find((currentOrder) => (sharedNormalizedValue(sellerOrderRefundMatchValues(currentOrder), sellerOrderRefundMatchValues(syncedOrder))));
        if (!matched)
            return syncedOrder;
        usedCurrentOrderIds.add(matched.id);
        return matched;
    });
    const missingCurrent = terminalCurrent.filter((currentOrder) => (!usedCurrentOrderIds.has(currentOrder.id)
        && !syncedOrders.some((syncedOrder) => (sharedNormalizedValue(sellerOrderRefundMatchValues(currentOrder), sellerOrderRefundMatchValues(syncedOrder))))));
    return [...merged, ...missingCurrent];
}
function mergeServiceRefundSyncState(input) {
    return {
        ...input.syncedState,
