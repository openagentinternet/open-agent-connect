...
exports.formatQaQuestionBullets = formatQaQuestionBullets;
exports.formatQaAnswerBullets = formatQaAnswerBullets;
exports.formatQaQuestionDetail = formatQaQuestionDetail;
exports.formatAlreadyAnsweredNotice = formatAlreadyAnsweredNotice;
exports.qaQuestionDetailViewLink = qaQuestionDetailViewLink;
const uri_1 = require("../metaweb/uri");
const publish_1 = require("./publish");
function metaIdUri(globalMetaId) {
...
function questionViewLink(question) {
    return (0, uri_1.buildPinBrowserUri)({ pinId: question.pinId, path: publish_1.SIMPLE_QUESTION_PATH });
}
function answerViewLink(answer) {
    return (0, uri_1.buildPinBrowserUri)({ pinId: answer.pinId, path: publish_1.SIMPLE_ANSWER_PATH });
}
/** Ready-to-quote markdown bullets for question items; titles are pin:// links, authors metaid:// links. */
function formatQaQuestionBullets(items) {
...
function formatQaAnswerBullets(items) {
...
function formatQaQuestionDetail(input) {
    ...
    if (question.createdAt)
        lines.push(`- asked at: ${formatTime(question.createdAt)}${question.isMempool ? ' (mempool, unconfirmed)' : ''}`);
    ...
    if (question.summary)
        lines.push(`- question summary: ${truncate(question.summary, 600)}`);
    if (question.pinId)
        lines.push(`- view: ${(0, uri_1.markdownSelfLink)(questionViewLink(question))}`);
 ...
 */
function formatAlreadyAnsweredNotice(questionPinId, answers, source = 'local') {
    const lines = [
    ...
    for (const answer of answers) {
        ...
        lines.push(`  view link: ${(0, uri_1.markdownSelfLink)(`pin://${answer.answerPinId}`)}`);
    }
    ...
    lines.push('The protocol allows multiple answers per bot and nothing here forbids another one — publishing again is your decision. If the new answer substantially improves the old one, call post_simpleanswer again with allow_repeat=true. For small additions, a PayComment on your existing answer usually serves better.');
    return lines.join('\n');
}
/** Convenience wrapper naming the question-page view link for detail results. */
function qaQuestionDetailViewLink(detail) {
    return detail.question.pinId ? (0, uri_1.markdownSelfLink)(questionViewLink(detail.question)) : '';
}
