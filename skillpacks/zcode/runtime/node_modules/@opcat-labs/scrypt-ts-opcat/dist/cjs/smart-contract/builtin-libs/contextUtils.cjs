"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.ContextUtils = void 0;
const decorators_js_1 = require('../decorators.cjs');
const assert_js_1 = require('../fns/assert.cjs');
const byteString_js_1 = require('../fns/byteString.cjs');
const hashes_js_1 = require('../fns/hashes.cjs');
const smartContractLib_js_1 = require('../smartContractLib.cjs');
const primitives_js_1 = require('../types/primitives.cjs');
const stdUtils_js_1 = require('./stdUtils.cjs');
const txUtils_js_1 = require('./txUtils.cjs');
/**
 * Utility class for Bitcoin smart contract context operations.
 * Provides methods for:
 * - ECDSA signature generation and verification
 * - Transaction preimage serialization and validation
 * - Prevouts and spent data verification
 * - Lock time checking
 * - Various cryptographic and serialization utilities
 *
 * Contains precomputed cryptographic constants for optimized signing operations.
 * All methods are static and can be used without class instantiation.
 * @category Library
 * @onchain
 */
class ContextUtils extends smartContractLib_js_1.SmartContractLib {
    /**
     * Normalizes a value to be within the range [0, modulus) by taking modulo and ensuring positivity.
     * @param k - The value to normalize
     * @param modulus - The modulus value
     * @returns The normalized value (k mod modulus) guaranteed to be non-negative
     */
    static normalize(k, modulus) {
        const res = k % modulus;
        // ensure it's positive
        return (res < 0) ? res + modulus : res;
    }
    /**
     * Generates a DER-encoded ECDSA signature from given parameters.
     *
     * @param h - The message hash to sign (bigint)
     * @param privKey - The private key to sign with (PrivKey)
     * @param inverseK - The inverse of the nonce value (bigint)
     * @param r - The r component of the signature (bigint)
     * @param rBigEndian - The r component in big-endian byte format (ByteString)
     * @param sigHashType - The signature hash type (ByteString)
     * @returns A DER-encoded signature (Sig)
     *
     * @remarks
     * - Normalizes the s value to be in the lower range of the curve order
     * - Conforms to strict DER encoding rules
     * - Handles endianness conversion for signature components
     */
    static sign(h, privKey, inverseK, r, rBigEndian, sigHashType) {
        let s = inverseK * (h + r * privKey);
        const N = 0xfffffffffffffffffffffffffffffffebaaedce6af48a03bbfd25e8cd0364141n;
        s = ContextUtils.normalize(s, N);
        // lower S
        if (s > N / 2n) {
            s = N - s;
        }
        // require(s != 0);		// check offchain
        /*
        * DER: h + l + rh + rl + r + sh + sl + s + hashtype
        * note: r & s are at most 33 bytes, thus no need to convert endian of rl & sl
        */
        const rlen = BigInt((0, byteString_js_1.len)(rBigEndian));
        const slen = (0, byteString_js_1.len)((0, byteString_js_1.intToByteString)(s));
        // we convert s to 32 bytes, otherwise reverseByteString(, 32) fails when s is strictly less than 31 bytes (note: 31 bytes works)
        // slice it after reversing to remove extra leading zeros, otherwise strict DER rule fails it due to not minimally encoded
        const sBigEndian = (0, byteString_js_1.slice)((0, byteString_js_1.reverseByteString)((0, byteString_js_1.intToByteString)(s, 32n), BigInt(32)), 32n - slen);
        const l = 4n + rlen + BigInt(slen);
        // rBigEndian must be mininally encoded, to conform to strict DER rule
        const rb = (0, byteString_js_1.toByteString)('30')
            + (0, byteString_js_1.intToByteString)(l)
            + (0, byteString_js_1.toByteString)('02')
            + (0, byteString_js_1.intToByteString)(rlen)
            + rBigEndian
            + (0, byteString_js_1.toByteString)('02')
            + (0, byteString_js_1.intToByteString)(slen)
            + sBigEndian
            + (0, byteString_js_1.toByteString)(sigHashType);
        return (0, primitives_js_1.Sig)(rb);
    }
    /**
     * Converts a big-endian unsigned byte string to a signed integer.
     * @param b The input byte string in big-endian format.
     * @returns The converted Int32 value.
     */
    static fromBEUnsigned(b) {
        // change endian first
        // append positive sign byte. This does not hurt even when sign bit is already positive
        return (0, byteString_js_1.byteStringToInt)((0, byteString_js_1.reverseByteString)(b, 32n) + (0, byteString_js_1.toByteString)('00'));
    }
    /**
     * Checks and signs a SHPreimage by serializing it, hashing, and creating a signature.
     *
     * @param shPreimage - The SHPreimage to be checked and signed.
     * @param sigHashType - The signature hash type to use for signing.
     * @returns The generated signature for the provided SHPreimage.
     */
    static checkSHPreimage(shPreimage, sigHashType) {
        const preimage = ContextUtils.serializeSHPreimage(shPreimage);
        const h = (0, hashes_js_1.hash256)(preimage);
        const sig = ContextUtils.sign(ContextUtils.fromBEUnsigned(h), ContextUtils.privKey, ContextUtils.invK, ContextUtils.r, ContextUtils.rBigEndian, sigHashType);
        return sig;
    }
    /**
     * Serializes a SHPreimage object into a SigHashPreimage.
     * Validates all fields of the input SHPreimage meet required specifications before serialization.
     * @param shPreimage - The SHPreimage object containing transaction preimage data
     * @returns A SigHashPreimage constructed from the validated and concatenated fields
     * @throws If any field validation fails (invalid length, negative value, or unsupported sigHashType)
     */
    static serializeSHPreimage(shPreimage) {
        (0, assert_js_1.assert)((0, byteString_js_1.len)(shPreimage.nVersion) == 4n, 'invalid length of nVersion');
        (0, assert_js_1.assert)((0, byteString_js_1.len)(shPreimage.hashPrevouts) == 32n, 'invalid length of hashPrevouts');
        (0, assert_js_1.assert)(shPreimage.inputIndex >= 0n, 'invalid value of inputIndex');
        (0, assert_js_1.assert)((0, byteString_js_1.len)(shPreimage.outpoint) == 36n, 'invalid length of outpoint');
        (0, assert_js_1.assert)((0, byteString_js_1.len)(shPreimage.spentScriptHash) == 32n, 'invalid length of spentScriptHash');
        (0, assert_js_1.assert)((0, byteString_js_1.len)(shPreimage.spentDataHash) == 32n, 'invalid length of spentDataHash');
        (0, assert_js_1.assert)(shPreimage.value >= 0n, 'invalid value of value');
        (0, assert_js_1.assert)((0, byteString_js_1.len)(shPreimage.nSequence) == 4n, 'invalid length of nSequence');
        (0, assert_js_1.assert)((0, byteString_js_1.len)(shPreimage.hashSpentAmounts) == 32n, 'invalid length of hashSpentAmounts');
        (0, assert_js_1.assert)((0, byteString_js_1.len)(shPreimage.hashSpentScriptHashes) == 32n, 'invalid length of hashSpentScriptHashes');
        (0, assert_js_1.assert)((0, byteString_js_1.len)(shPreimage.hashSpentDataHashes) == 32n, 'invalid length of hashSpentDataHashes');
        (0, assert_js_1.assert)((0, byteString_js_1.len)(shPreimage.hashSequences) == 32n, 'invalid length of hashSequences');
        (0, assert_js_1.assert)((0, byteString_js_1.len)(shPreimage.hashOutputs) == 32n, 'invalid length of hashOutputs');
        (0, assert_js_1.assert)(shPreimage.nLockTime >= 0n, 'invalid value of nLockTime');
        (0, assert_js_1.assert)(shPreimage.sigHashType == 1n
            || shPreimage.sigHashType == 2n
            || shPreimage.sigHashType == 3n
            || shPreimage.sigHashType == 0x81n
            || shPreimage.sigHashType == 0x82n
            || shPreimage.sigHashType == 0x83n, 'invalid value of sigHashType');
        const preimage = shPreimage.nVersion
            + shPreimage.hashPrevouts
            + stdUtils_js_1.StdUtils.toLEUnsigned(shPreimage.inputIndex, 4n)
            + shPreimage.outpoint
            + shPreimage.spentScriptHash
            + shPreimage.spentDataHash
            + txUtils_js_1.TxUtils.satoshisToByteString(shPreimage.value)
            + shPreimage.nSequence
            + shPreimage.hashSpentAmounts
            + shPreimage.hashSpentScriptHashes
            + shPreimage.hashSpentDataHashes
            + shPreimage.hashSequences
            + shPreimage.hashOutputs
            + stdUtils_js_1.StdUtils.toLEUnsigned(shPreimage.nLockTime, 4n)
            + (0, byteString_js_1.intToByteString)(shPreimage.sigHashType, 4n);
        return (0, primitives_js_1.SigHashPreimage)(preimage);
    }
    /**
     * Extract outpoint directly from SHPreimage.
     * Use this for current input's outpoint instead of slicing from prevouts.
     * @param shPreimage - The SHPreimage containing the outpoint field
     * @returns The Outpoint struct with txHash and outputIndex
     */
    static getOutpoint(shPreimage) {
        return {
            txHash: (0, byteString_js_1.slice)(shPreimage.outpoint, 0n, 32n),
            outputIndex: stdUtils_js_1.StdUtils.byteStringToUInt32((0, byteString_js_1.slice)(shPreimage.outpoint, 32n, 36n)),
        };
    }
    /**
     * Verify that the prevouts context passed in by the user is authentic
     * @param prevouts prevouts context passed in by the user that need to be verified
     * @param prevout prevout context passed in by the user that need to be verified
     * @param t_hashPrevouts hashPrevouts in preimage which is trustable
     * @param t_inputIndex the index of the input, which is trustable
     * @returns the number of inputs, which is trustable
     */
    static checkPrevouts(prevouts, t_hashPrevouts, t_inputIndex, t_inputCount) {
        // check prevouts
        (0, assert_js_1.assert)((0, hashes_js_1.hash256)(prevouts) == t_hashPrevouts, 'hashPrevouts mismatch');
        (0, assert_js_1.assert)(t_inputIndex < t_inputCount, 'invalid prevouts');
        (0, assert_js_1.assert)(t_inputCount == stdUtils_js_1.StdUtils.checkLenDivisibleBy(prevouts, 36n), 'invalid prevouts');
        const b = (0, byteString_js_1.slice)(prevouts, t_inputIndex * 36n, (t_inputIndex + 1n) * 36n);
        return {
            txHash: (0, byteString_js_1.slice)(b, 0n, 32n),
            outputIndex: stdUtils_js_1.StdUtils.byteStringToUInt32((0, byteString_js_1.slice)(b, 32n, 36n)),
        };
    }
    /**
     * Check if the spent scripts array passed in matches the shaSpentScripts
     * @param spentScriptHashes array of spent scripts passed in that need to be verified
     * @param t_hashSpentScripts the hash of the merged spent scripts, which comes from preimage and is trustable
     * @param t_inputCount must be trustable, the number of inputs
     */
    static checkSpentScripts(spentScriptHashes, t_hashSpentScripts, t_inputCount) {
        (0, assert_js_1.assert)((0, hashes_js_1.hash256)(spentScriptHashes) == t_hashSpentScripts, 'hashSpentScripts mismatch');
        (0, assert_js_1.assert)(t_inputCount == stdUtils_js_1.StdUtils.checkLenDivisibleBy(spentScriptHashes, 32n), 'invalid spentScriptHashes');
    }
    /**
     * Check if the spent amounts array passed in matches the hashSpentAmounts
     * @param spentAmounts array of spent amounts passed in that need to be verified
     * @param hashSpentAmounts the hash of the merged spent amounts, which comes from preimage and is trustable

     */
    static checkSpentAmounts(spentAmounts, hashSpentAmounts) {
        (0, assert_js_1.assert)((0, hashes_js_1.hash256)(spentAmounts) == hashSpentAmounts, 'hashSpentAmounts mismatch');
        return stdUtils_js_1.StdUtils.checkLenDivisibleBy(spentAmounts, 8n);
    }
    /**
     * Verifies the integrity of spent data hashes against the provided transaction hash and input count.
     *
     * @param spentDataHashes - The hashes of spent data to be verified
     * @param hashSpentDataHashes - The expected hash of all spent data hashes
     * @param inputCount - The expected number of inputs (must match the length of spentDataHashes)
     * @throws Throws an error if hash verification fails or if input count doesn't match
     */
    static checkSpentDataHashes(spentDataHashes, hashSpentDataHashes, inputCount) {
        (0, assert_js_1.assert)((0, hashes_js_1.hash256)(spentDataHashes) == hashSpentDataHashes, 'hashSpentDataHashes mismatch');
        (0, assert_js_1.assert)(inputCount == stdUtils_js_1.StdUtils.checkLenDivisibleBy(spentDataHashes, 32n), 'invalid spentDataHashes');
    }
    /**
     * Retrieves the spent script hash for a specific input index from the given spent scripts buffer.
     * @param spentScriptHashes - Buffer containing all spent script hashes
     * @param inputIndex - Index of the input to retrieve the spent script hash for
     * @returns The spent script hash for the specified input index as a 32-byte string
     */
    static getSpentScriptHash(spentScriptHashes, inputIndex) {
        return (0, byteString_js_1.slice)(spentScriptHashes, inputIndex * 32n, (inputIndex + 1n) * 32n);
    }
    /**
     * Retrieves the spent amount for a specific input index from the given spent amounts buffer.
     * @param spentAmounts - Buffer containing all spent amounts in little-endian format
     * @param inputIndex - Index of the input to retrieve the spent amount for
     * @returns The spent amount for the specified input index as a 32-bit integer
     */
    static getSpentAmount(spentAmounts, inputIndex) {
        return stdUtils_js_1.StdUtils.fromLEUnsigned((0, byteString_js_1.slice)(spentAmounts, inputIndex * 8n, (inputIndex + 1n) * 8n));
    }
    /**
     * Retrieves the data hash for a specific input index from spent data hashes.
     * @param spentDataHashes - The collection of spent data hashes
     * @param inputIndex - The index of the input to retrieve
     * @returns The 32-byte data hash corresponding to the specified input index
     */
    static getSpentDataHash(spentDataHashes, inputIndex) {
        return (0, byteString_js_1.slice)(spentDataHashes, inputIndex * 32n, (inputIndex + 1n) * 32n);
    }
    /**
     * Checks if the lock time conditions are met for a given preimage and lock time.
     * @param shPreimage The preimage containing lock time and sequence values.
     * @param nlockTime The required minimum lock time to validate against.
     * @returns True if the lock time conditions are satisfied, false otherwise.
     */
    static checknLockTime(shPreimage, nlockTime) {
        const nSequence = stdUtils_js_1.StdUtils.fromLEUnsigned(shPreimage.nSequence);
        return (nSequence < 4294967295n && (nlockTime < 500000000n ? shPreimage.nLockTime < 500000000n : true) && shPreimage.nLockTime >= nlockTime);
    }
}
exports.ContextUtils = ContextUtils;
// The following arguments can be generated using sample code at
// https://gist.github.com/scrypt-sv/f6882be580780a88984cee75dd1564c4.js
ContextUtils.privKey = (0, primitives_js_1.PrivKey)(0x26f00fe2340a84335ebdf30f57e9bb58487117b29355718f5e46bf5168d7df97n);
ContextUtils.pubKey = (0, primitives_js_1.PubKey)((0, byteString_js_1.toByteString)('02ba79df5f8ae7604a9830f03c7933028186aede0675a16f025dc4f8be8eec0382'));
// invK is the modular inverse of k, the ephemeral key
ContextUtils.invK = 0xc8ffdbaa05d93aa4ede79ec58f06a72562048b775a3507c2bf44bde4f007c40an;
// r is x coordinate of R, which is kG
ContextUtils.r = 0x1008ce7480da41702918d1ec8e6849ba32b4d65b1e40dc669c31a1e6306b266cn;
// rBigEndian is the signed magnitude representation of r, in big endian
ContextUtils.rBigEndian = (0, byteString_js_1.toByteString)('1008ce7480da41702918d1ec8e6849ba32b4d65b1e40dc669c31a1e6306b266c');
__decorate([
    (0, decorators_js_1.prop)(),
    __metadata("design:type", BigInt)
], ContextUtils, "privKey", void 0);
__decorate([
    (0, decorators_js_1.prop)(),
    __metadata("design:type", String)
], ContextUtils, "pubKey", void 0);
__decorate([
    (0, decorators_js_1.prop)(),
    __metadata("design:type", BigInt)
], ContextUtils, "invK", void 0);
__decorate([
    (0, decorators_js_1.prop)(),
    __metadata("design:type", BigInt)
], ContextUtils, "r", void 0);
__decorate([
    (0, decorators_js_1.prop)(),
    __metadata("design:type", String)
], ContextUtils, "rBigEndian", void 0);
__decorate([
    (0, decorators_js_1.method)(),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [BigInt, BigInt]),
    __metadata("design:returntype", BigInt)
], ContextUtils, "normalize", null);
__decorate([
    (0, decorators_js_1.method)(),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [BigInt, BigInt, BigInt, BigInt, String, String]),
    __metadata("design:returntype", String)
], ContextUtils, "sign", null);
__decorate([
    (0, decorators_js_1.method)(),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String]),
    __metadata("design:returntype", BigInt)
], ContextUtils, "fromBEUnsigned", null);
__decorate([
    (0, decorators_js_1.method)(),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object, String]),
    __metadata("design:returntype", String)
], ContextUtils, "checkSHPreimage", null);
__decorate([
    (0, decorators_js_1.method)(),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object]),
    __metadata("design:returntype", String)
], ContextUtils, "serializeSHPreimage", null);
__decorate([
    (0, decorators_js_1.method)(),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object]),
    __metadata("design:returntype", Object)
], ContextUtils, "getOutpoint", null);
__decorate([
    (0, decorators_js_1.method)(),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String, String, BigInt, BigInt]),
    __metadata("design:returntype", Object)
], ContextUtils, "checkPrevouts", null);
__decorate([
    (0, decorators_js_1.method)(),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String, String, BigInt]),
    __metadata("design:returntype", void 0)
], ContextUtils, "checkSpentScripts", null);
__decorate([
    (0, decorators_js_1.method)(),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String, String]),
    __metadata("design:returntype", BigInt)
], ContextUtils, "checkSpentAmounts", null);
__decorate([
    (0, decorators_js_1.method)(),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String, String, BigInt]),
    __metadata("design:returntype", void 0)
], ContextUtils, "checkSpentDataHashes", null);
__decorate([
    (0, decorators_js_1.method)(),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String, BigInt]),
    __metadata("design:returntype", String)
], ContextUtils, "getSpentScriptHash", null);
__decorate([
    (0, decorators_js_1.method)(),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String, BigInt]),
    __metadata("design:returntype", BigInt)
], ContextUtils, "getSpentAmount", null);
__decorate([
    (0, decorators_js_1.method)(),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String, BigInt]),
    __metadata("design:returntype", String)
], ContextUtils, "getSpentDataHash", null);
__decorate([
    (0, decorators_js_1.method)(),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object, BigInt]),
    __metadata("design:returntype", Boolean)
], ContextUtils, "checknLockTime", null);
//# sourceMappingURL=contextUtils.js.map