...
import { buildMetafileAcceleratedContentUrl, normalizeMetafileContentBaseUrl, } from './metafileContentUrl.js';
...
export async function resolveMetaAppPinToRecord(input) {
    ...
    try {
        ...
        if (!response.ok || !response.json) {
            return browserCommandFailed(response.status === 404 ? 'browser_resource_not_found' : 'browser_resolve_failed', response.status === 404 ? 'MetaApp pin was not found.' : `MetaApp pin lookup failed with HTTP ${response.status}.`);
        }
        ...
        if (protocolPath !== METAAPP_PROTOCOL_PATH) {
            return browserCommandFailed('browser_protocol_mismatch', `Pin path ${protocolPath || '(empty)'} is not a MetaApp protocol pin.`, { expectedPath: METAAPP_PROTOCOL_PATH, actualPath: protocolPath || null });
        }
