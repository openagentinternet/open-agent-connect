...
class TxComposer {
    ...
    static deserialize(composerStr) {
    ...
    appendP2PKHInput(utxo) {
        this.tx.addInput(new mvc.Transaction.Input.PublicKeyHash({
    ...
    appendInput(input) {
        this.tx.addInput(new mvc.Transaction.Input({
            output: new mvc.Transaction.Output({
    ...
    appendP2PKHOutput(output) {
        this.tx.addOutput(new mvc.Transaction.Output({
    ...
    appendChangeOutput(changeAddress, feeb = 0.05, extraSize = 0) {
        ...
        let fee = Math.ceil((this.tx.toBuffer().length + unlockSize + extraSize + mvc.Transaction.CHANGE_OUTPUT_MAX_SIZE) * feeb) + 1;
    ...
    unlockP2PKHInput(privateKey, inputIndex, sigtype = exports.sighashType) {
        ...
        const sig = new mvc.Transaction.Signature({
            ...
            inputIndex,
            signature: mvc.Transaction.Sighash.sign(tx, privateKey, sigtype, inputIndex, tx.inputs[inputIndex].output.script, tx.inputs[inputIndex].output.satoshisBN),
            sigtype,
        });
    ...
    getTxFormatSig(privateKey, inputIndex, sigtype = exports.sighashType) {
        let sig = (0, mvc_scrypt_1.signTx)(this.tx, privateKey, this.getInput(inputIndex).output.script, this.getInput(inputIndex).output.satoshis, inputIndex, sigtype);
        return sig;
    }
    getInputPreimage(inputIndex, sigtype = exports.sighashType) {
        return (0, mvc_scrypt_1.getPreimage)(this.tx, this.getInput(inputIndex).output.script, this.getInput(inputIndex).output.satoshis, inputIndex, sigtype);
    }
    getUnspentValue() {
    ...
    getSigHashLit() {
        this.sigHashList.forEach((v) => {
            v.sighash = (0, mvc_scrypt_1.toHex)(mvc.Transaction.Sighash.sighash(this.tx, v.sighashType, v.inputIndex, this.getInput(v.inputIndex).output.script, this.getInput(v.inputIndex).output.satoshisBN));
        });
