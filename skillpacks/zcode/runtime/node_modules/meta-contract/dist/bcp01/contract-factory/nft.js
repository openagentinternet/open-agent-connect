...
class NftFactory {
    ...
    static calUnlockingScriptSize(bsvInputLen, genesisScript, opreturnData, operation) {
        ...
        let checkScriptTx = new mvc.Transaction(dummy_1.dummyTx.serialize(true));
        checkScriptTx.addOutput(new mvc.Transaction.Output({
            script: unlockCheckContact.lockingScript,
            satoshis: 10000,
        }));
        let sellContract = nftSell_1.NftSellFactory.getDummyInstance();
        let sellTx = new mvc.Transaction(dummy_1.dummyTx.serialize(true));
        sellTx.addOutput(new mvc.Transaction.Output({
