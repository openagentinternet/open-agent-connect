...
function getSensibleID(script0) {
    if (script0.length < GENESIS_TX_ID_OFFSET)
        return { txid: '', index: 0 };
    let script = Buffer.from(script0);
    let sensibleIDBuf = script.slice(script.length - GENESIS_TX_ID_OFFSET, script.length - GENESIS_TX_ID_OFFSET + GENESIS_TX_ID_LEN);
...
function getRabinPubKeyHashArrayHash(script) {
    return script
        .slice(script.length - RABIN_PUBKEY_HASH_ARRAY_HASH_OFFSET, script.length - RABIN_PUBKEY_HASH_ARRAY_HASH_OFFSET + RABIN_PUBKEY_HASH_ARRAY_HASH_LEN)
        .toString('hex');
}
...
function newDataPart({ tokenName, tokenSymbol, decimalNum, genesisHash, address,
// bufferValue = 总为8位0
genesisTxid, protoType, protoVersion, }) {
