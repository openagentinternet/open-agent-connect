...
async function metaletV4Get(path, params) {
...
async function metaletV4Post(path, body) {
...
function buildMetaIdInscriptionScript(data) {
...
function countScriptChunks(script) {
    let count = 0;
    let i = 0;
...
function buildP2SHOutputScript(lockScript) {
...
function buildP2PKHOutputScript(address, _network) {
...
function estimateTxSize(p2pkhInputCount, outputCount, p2shUnlockScriptSize = 0) {
    let size = 10;
...
function selectUtxos(availableUtxos, targetAmount, feeRate, outputCount, p2shUnlockScriptSize = 0) {
    const selectedUtxos = [];
    let totalInput = 0;
...
function signP2PKHInput(tx, inputIndex, keyPair, prevOutputScript) {
...
function signP2SHInput(tx, inputIndex, tempKeyPair, lockScript, inscriptionScript) {
...
function buildDogeInscriptionTxs(metaidData, utxos, walletKeyPair, feeRate, changeAddress, network, revealOutValue, ecpairInstance) {
    ...
    const { selectedUtxos: commitUtxos, fee: commitFee, totalInput: commitTotalInput } = selectUtxos(utxos, INSCRIPTION_OUTPUT_VALUE, feeRate, 2, 0);
    ...
    if (commitChange >= P2SH_DUST_LIMIT) {
        commitTx.addOutput(buildP2PKHOutputScript(changeAddress, network), commitChange);
    }
    ...
    revealTx.addOutput(buildP2PKHOutputScript(metaidData.revealAddr, network), revealOutValue);
    const availableUtxos = utxos.filter((u) => !commitUtxos.some((c) => c.txId === u.txId && c.outputIndex === u.outputIndex));
    if (commitChange >= P2SH_DUST_LIMIT) {
        availableUtxos.push({
            txId: commitTxId,
    ...
    const { selectedUtxos: revealUtxos, fee: revealFee, totalInput: revealTotalInput } = selectUtxos(availableUtxos, revealOutValue - INSCRIPTION_OUTPUT_VALUE, feeRate, 2, estimatedUnlockSize);
    for (const utxo of revealUtxos) {
        revealTx.addInput(Buffer.from(utxo.txId, 'hex').reverse(), utxo.outputIndex);
    }
    const revealChange = INSCRIPTION_OUTPUT_VALUE + revealTotalInput - revealOutValue - revealFee;
    if (revealChange >= P2SH_DUST_LIMIT) {
        revealTx.addOutput(buildP2PKHOutputScript(changeAddress, network), revealChange);
    }
...
exports.dogeChainAdapter = {
    ...
    async fetchUtxos(address) {
        const data = await metaletV4Get('/doge/address/utxo-list', { address });
    ...
    async fetchBalance(address) {
        try {
            const data = await metaletV4Get('/doge/address/balance-info', { address });
    ...
    async fetchFeeRate() {
        try {
            const data = await metaletV4Get('/doge/fee/summary', {});
            const list = data?.list ?? [];
    ...
    async fetchRawTx(txid) {
        const data = await metaletV4Get('/doge/tx/raw', { txId: txid });
        const rawTx = normalizeText(data?.hex ?? data?.rawTx ?? '');
        if (!rawTx)
            throw new Error(`DOGE raw tx response is empty for ${txid}.`);
        return rawTx;
    },
    async broadcastTx(rawTx) {
        const data = await metaletV4Post('/doge/tx/broadcast', { rawTx });
        const txId = normalizeText(data?.TxId ?? data?.txId ?? data);
    ...
    async buildTransfer(input) {
        ...
        if (!rawTx)
            throw new Error('DOGE signTransaction returned no raw transaction.');
    ...
    async buildInscription(input) {
        try {
            ...
            const ecc = await Promise.resolve().then(() => __importStar(require('@bitcoinerlab/secp256k1')));
            bitcoin.initEccLib(ecc.default);
            const ECPairInstance = (0, ecpair_1.default)(ecc.default);
            ...
            const { commitTx, revealTx, commitFee, revealFee } = buildDogeInscriptionTxs(metaidData, utxos, walletKeyPair, feeRate, address, network, INSCRIPTION_OUTPUT_VALUE, ECPairInstance);
            ...
            throw new Error(`DOGE buildInscription failed: ${err instanceof Error ? err.message : String(err)}`);
        }
    },
};
exports.default = exports.dogeChainAdapter;
