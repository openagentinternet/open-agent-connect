...
exports.codexBackendFactory = void 0;
...
function buildCodexArgs(request) {
    return [
        'app-server',
        '--listen',
        'stdio://',
        ...(0, backend_1.filterBlockedArgs)(request.extraArgs, {
...
function getItemId(params) {
    ...
    if (isRecord(item))
        return getString(item.id) ?? getString(params.itemId) ?? 'codex-item';
    return getString(params.itemId) ?? 'codex-item';
}
function getItemKind(params) {
    if (!isRecord(params))
        return '';
    const item = params.item;
    if (isRecord(item))
        return getString(item.type) ?? getString(item.kind) ?? '';
    return getString(params.type) ?? getString(params.kind) ?? '';
}
function getItemPhase(params) {
    if (!isRecord(params))
        return '';
    const item = params.item;
    if (isRecord(item))
        return getString(item.phase) ?? '';
    return getString(params.phase) ?? '';
}
function getCommandInput(params) {
    if (!isRecord(params))
        return {};
    const item = params.item;
    if (isRecord(item)) {
...
function getCompletedOutput(params) {
    if (!isRecord(params))
        return '';
    const item = params.item;
    if (isRecord(item)) {
...
function createCodexBackend(binaryPath, env) {
    return {
        provider: 'codex',
        async execute(request, emitter, signal) {
            ...
            const turnDone = new Promise((resolve) => {
                ...
                const emitSemantic = (event) => {
                    emitter.emit(event);
                    if (event.type === 'text')
                        outputParts.push(event.content);
                    resetSemanticTimer();
                };
                const handleRawNotification = (message) => {
                    ...
                    if (method === 'item/agentMessage/delta') {
                        ...
                        if (delta)
                            emitSemantic({ type: 'text', content: delta });
                        return;
                    }
                ...
                const handleLegacyNotification = (message) => {
                    ...
                    if (type === 'task_started') {
                        ...
                        if (content)
                            emitSemantic({ type: 'text', content });
                    }
                ...
                rl.on('line', (line) => {
                    ...
                    if (message.id !== undefined && (message.result !== undefined || message.error !== undefined)) {
                        ...
                        if (message.error) {
                            pendingRequest.reject(new Error((0, backend_1.stringifyError)(message.error)));
                        }
                        else {
            ...
            const awaitRpc = async (promise, phase) => {
            ...
            try {
                ...
                if (finalStatus !== 'completed')
                    throw new Error(finalError ?? 'codex initialization did not complete');
...
exports.codexBackendFactory = createCodexBackend;
