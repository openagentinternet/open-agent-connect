...
exports.formatMetawebSearchBullets = formatMetawebSearchBullets;
exports.metawebProtocolFollowupHint = metawebProtocolFollowupHint;
exports.formatMetawebPinDetail = formatMetawebPinDetail;
...
function formatMetawebSearchBullets(items) {
    return items.map((item) => {
        const uri = (0, uri_1.buildSearchItemBrowserUri)(item);
...
function metawebProtocolFollowupHint(protocol) {
...
function formatMetawebPinDetail(pin) {
    ...
    const creatorPart = pin.creator.globalMetaId
        ...
        : creatorLabel;
    const viewLink = (0, uri_1.markdownSelfLink)((0, uri_1.buildPinBrowserUri)({
    ...
    if (viewLink)
        lines.push(`- view: ${viewLink}`);
    lines.push(`- author: ${creatorPart}`);
    if (pin.createdAt)
        lines.push(`- created: ${formatTime(pin.createdAt)}`);
    if (pin.operation !== 'create')
        lines.push(`- operation: ${pin.operation}${pin.currentPinId && pin.currentPinId !== pin.pinId ? ` (latest: ${pin.currentPinId})` : ''}`);
    if (pin.meta.tags.length)
        lines.push(`- tags: ${pin.meta.tags.map(flattenInline).filter(Boolean).join(', ')}`);
