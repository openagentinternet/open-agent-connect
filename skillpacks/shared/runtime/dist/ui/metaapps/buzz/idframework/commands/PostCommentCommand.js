/**
 * PostCommentCommand - Create a paycomment pin for a buzz.
 * Comment is text-only in V1.
 */
export default class PostCommentCommand {
  async execute({ payload = {}, stores }) {
    ...
    var pinId = this._normalizePinId(payload.pinId || payload.pinid || payload.id || payload.commentTo || '');
    var content = typeof payload.content === 'string' ? payload.content : '';
    var path = String(payload.path || '').trim() || '/protocols/paycomment';
