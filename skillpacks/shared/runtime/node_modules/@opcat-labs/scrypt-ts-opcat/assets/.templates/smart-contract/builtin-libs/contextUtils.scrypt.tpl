...
library ContextUtils {
  ...
  static function serializeSHPreimage(SHPreimage shPreimage) : SigHashPreimage {
    ...
    require(len(shPreimage.outpoint) == 36);
    ...
    bytes preimage = shPreimage.nVersion + shPreimage.hashPrevouts + StdUtils.toLEUnsigned(shPreimage.inputIndex, 4) + shPreimage.outpoint + shPreimage.spentScriptHash + shPreimage.spentDataHash + TxUtils.satoshisToByteString(shPreimage.value) + shPreimage.nSequence + shPreimage.hashSpentAmounts + shPreimage.hashSpentScriptHashes + shPreimage.hashSpentDataHashes + shPreimage.hashSequences + shPreimage.hashOutputs + StdUtils.toLEUnsigned(shPreimage.nLockTime, 4) + num2bin(shPreimage.sigHashType, 4);
    return SigHashPreimage(preimage);
  }
  static function getOutpoint(SHPreimage shPreimage) : Outpoint {
    return {shPreimage.outpoint[0 : 32], StdUtils.byteStringToUInt32(shPreimage.outpoint[32 : 36])};
  }
