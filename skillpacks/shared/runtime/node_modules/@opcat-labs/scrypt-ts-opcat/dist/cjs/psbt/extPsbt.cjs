...
exports.ExtPsbt = void 0;
const index_js_1 = require('../smart-contract/types/index.cjs');
const consts_js_1 = require('../smart-contract/consts.cjs');
const script_js_1 = require('../smart-contract/types/script.cjs');
const contextProvider_js_1 = require('./contextProvider.cjs');
const common_js_1 = require('../utils/common.cjs');
const tools = __importStar(require("uint8array-tools"));
const byteString_js_1 = require('../smart-contract/fns/byteString.cjs');
const index_js_2 = require('../smart-contract/fns/index.cjs');
const psbt_js_1 = require('./psbt.cjs');
const networks_js_1 = require('../networks.cjs');
const opcat_1 = require("@opcat-labs/opcat");
const proof_js_1 = require('../utils/proof.cjs');
const { BufferWriter } = opcat_1.encoding;
 ...
 */
class ExtPsbt extends psbt_js_1.Psbt {
    constructor(opts = {}, data) {
        if (typeof opts.network === 'string') {
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            opts.network = (0, networks_js_1.fromSupportedNetwork)(opts.network);
        }
        super(opts, data);
        this._sigHashTypes = new Map();
        this._sigRequests = new Map();
        this._finalizers = new Map();
        this._inputContracts = new Map();
        this._B2GUtxos = new Map();
        this._B2GInfos = new Map();
        this._outputContracts = new Map();
        this._inputUnlockScripts = new Map();
        this._changeOutputIndex = null;
        this._finalizing = false;
        this._isSealed = false;
        this._ctxProvider = new contextProvider_js_1.ContextProvider(this);
    }
    /**
    ...
    static fromBuffer(buffer, opts = {}) {
        ...
        const psbt = psbt_js_1.Psbt.fromBuffer(buffer, opts);
        return new ExtPsbt(opts, psbt.data);
    }
    /**
     ...
     */
    setSighashType(inputIndex, sigHashType) {
        if (this._sigHashTypes.has(inputIndex)) {
            ...
            if (this._sigHashTypes.get(inputIndex) !== sigHashType)
                throw new Error('the sigHashType cannot be different with sigHashType at @method(sigHashType)');
        }
        else {
     ...
     */
    getSigHashType(inputIndex) {
        return this._sigHashTypes.get(inputIndex);
    }
    /**
     ...
     */
    getSpentDataHashes() {
        let spentDataHashes = (0, byteString_js_1.toByteString)('');
        for (let i = 0; i < this.data.inputs.length; i++) {
            const opcatUtxo = this.getInputOutput(i);
            spentDataHashes += (0, index_js_2.sha256)(tools.toHex(opcatUtxo.data));
        }
    ...
    get hashSpentDatas() {
        ...
        return (0, index_js_2.hash256)(spentDataHashesJoin);
    }
    /**
    ...
    spendUTXO(utxos) {
        ...
        for (let i = 0; i < utxos.length; i++) {
            ...
            const sigAddress = script_js_1.Script.fromHex(utxo.script).toAddress(this.network);
            ...
            this.addInput({
                ...
                opcatUtxo: {
                    data: (0, common_js_1.hexToUint8Array)(utxo.data),
                    script: (0, common_js_1.hexToUint8Array)(utxo.script),
            ...
            if (utxo.txHashPreimage) {
                ...
                this._B2GUtxos.set(inputIndex, utxo);
            }
        }
        return this;
    }
    ...
    addContractInput(contract, contractCall) {
        ...
        if (!fromUtxo) {
            throw new Error(`The contract input '${contract.constructor.name}' does not bind to any UTXO`);
        }
        this.addInput({
            hash: fromUtxo.txId,
            index: fromUtxo.outputIndex,
            opcatUtxo: {
                script: (0, common_js_1.hexToUint8Array)(fromUtxo.script),
                data: (0, common_js_1.hexToUint8Array)(fromUtxo.data),
        ...
        if ("txHashPreimage" in fromUtxo) {
            this._B2GUtxos.set(inputIndex, contract.utxo);
        }
        if (contractCall) {
     ...
     */
    updateContractInput(inputIndex, contractCall) {
        ...
        const finalizer = (_self, _inputIndex, // Which input is it?
        _input) => {
    ...
    addContractOutput(contract, satoshis) {
        const Contract = Object.getPrototypeOf(contract).constructor;
        let dataHex = '';
        ...
        this.addOutput({
            ...
            data: (0, common_js_1.hexToUint8Array)(dataHex),
        });
     ...
     * @returns The PSBT instance for chaining.
     */
    change(toAddr, feeRate, data) {
        const changeScript = script_js_1.Script.fromAddress(toAddr);
        if (typeof data === 'string') {
            data = (0, common_js_1.hexToUint8Array)(data);
        }
    ...
    finalizeChangeOutput() {
        ...
        if (changeAmount >= consts_js_1.DUST_LIMIT) {
            ...
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            this.__CACHE.__TX.outputs.splice(this._changeOutputIndex, 1);
            this._changeOutputIndex = null;
        }
        return this;
    }
    ...
    get network() {
        ...
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        return this.opts.network || opcat_1.Networks.livenet;
    }
    /**
    ...
    _setInputFinalizer(inputIndex, finalizer) {
    ...
    _bindcontractsUtxo() {
        ...
        this._outputContracts.forEach((contract, outputIndex) => {
            ...
            contract.bindToUtxo({
                txId: signedTx.id,
                outputIndex,
                data: (0, common_js_1.uint8ArrayToHex)(utxo.data),
                satoshis: Number(utxo.value),
                txHashPreimage: (0, common_js_1.uint8ArrayToHex)(signedTx.toTxHashPreimage()),
            });
        });
    }
    /**
    ...
    get isFinalized() {
        ...
        return this.data.inputs.reduce((finalized, input) => {
            return finalized && (0, psbt_js_1.isFinalized)(input);
        }, true);
    }
    _cacheInputUnlockScript(inputIndex, unlockScript) {
        this._inputUnlockScripts.set(inputIndex, unlockScript);
    }
    _hasInputUnlockScript(inputIndex) {
        return this._inputUnlockScripts.has(inputIndex);
    }
    _getInputUnlockScript(inputIndex) {
     ...
     */
    isContractInput(inputIndex) {
        return this._inputContracts.has(inputIndex);
    }
    _unfinalizedCallArgsSize() {
        let size = 0;
        this.data.inputs.forEach((input, _inputIndex) => {
            if (!this.isContractInput(_inputIndex)) {
                ...
                if (!(0, psbt_js_1.isFinalized)(input)) {
                    ...
                    if (input.finalScriptSig) {
                        ...
                        throw new Error('The input should be finalized with either finalScriptSig');
                    }
                }
            }
        });
    ...
    _throwIfNonModifiable(extraMsg) {
     ...
     */
    psbtOptions(autoFinalized = false) {
        const toSignInputs = [];
     ...
     */
    getSig(inputIndex, options) {
        const input = this.data.inputs[inputIndex];
        let signature = Uint8Array.from(new Array(P2PKH_SIG_LEN).fill(0));
        ...
        if (input.partialSig) {
            const pSig = input.partialSig.find((partialSig) => {
                const sigAddr = opcat_1.PublicKey.fromHex(tools.toHex(partialSig.pubkey)).toAddress(this.network).toString();
                const reqAddr = options.address || (options.publicKey ? opcat_1.PublicKey.fromHex(options.publicKey).toAddress(this.network).toString() : undefined);
        ...
        return (0, index_js_1.Sig)(tools.toHex(signature));
    }
    /**
    ...
    getUtxo(outputIndex) {
        ...
        try {
            ...
            address = script_js_1.Script.fromBuffer(Buffer.from(this.txOutputs[outputIndex].script)).toAddress(this.network).toString();
        }
        catch (_error) {
            address = '';
        }
        return {
            ...
            script: (0, common_js_1.uint8ArrayToHex)(this.txOutputs[outputIndex].script),
            data: (0, common_js_1.uint8ArrayToHex)(this.txOutputs[outputIndex].data),
            address: address,
     ...
     */
    async getBacktraceInfo(provider, inputIndex, prevPrevTxFinder) {
        const input = this.txInputs[inputIndex];
        const prevTxId = (0, common_js_1.uint8ArrayToHex)(Uint8Array.prototype.slice.call(input.hash).reverse());
        const prevTxHex = await provider.getRawTransaction(prevTxId);
        const prevTx = opcat_1.Transaction.fromString(prevTxHex);
        const prevPrevTxHex = await prevPrevTxFinder(prevTx, provider, inputIndex);
        const prevPrevTx = opcat_1.Transaction.fromString(prevPrevTxHex);
        const prevPrevTxId = (0, common_js_1.uint8ArrayToHex)(Uint8Array.prototype.slice.call(prevTx.inputs[input.index].prevTxId).reverse());
        return {
            prevTxInput: {
                prevTxHash: prevPrevTxId,
                prevOutputIndex: BigInt(prevTx.inputs[input.index].outputIndex),
                sequence: BigInt(input.sequence),
                scriptHash: (0, index_js_2.sha256)(prevTx.inputs[input.index].script.toHex()),
            },
            prevTxInputIndex: BigInt(inputIndex),
            prevPrevTxPreimage: (0, proof_js_1.toTxHashPreimage)(prevPrevTx.toTxHashPreimage()),
        };
    }
    /**
     ...
     */
    async calculateBacktraceInfo(provider, prevPrevTxFinder) {
        for (let i = 0; i < this.txInputs.length; i++) {
            if (this.isContractInput(i)) {
                const info = await this.getBacktraceInfo(provider, i, prevPrevTxFinder || (async (prevTx, provider, _inputIndex) => {
                    const prevTxId = (0, common_js_1.uint8ArrayToHex)(prevTx.inputs[0].prevTxId);
                    const prevTxHex = await provider.getRawTransaction(prevTxId);
                    return prevTxHex;
                }));
    ...
    txHashPreimage() {
        ...
        return (0, common_js_1.uint8ArrayToHex)(this.extractTransaction().toTxHashPreimage());
    }
    /**
...
exports.ExtPsbt = ExtPsbt;
//# sourceMappingURL=extPsbt.js.map