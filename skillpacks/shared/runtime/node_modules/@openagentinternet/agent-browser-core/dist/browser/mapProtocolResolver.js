...
export async function resolveMapUriToResource(input) {
    ...
    if (!response.ok || !response.json) {
        return browserCommandFailed(response.status === 404 ? 'browser_resource_not_found' : 'browser_resolve_failed', response.status === 404 ? 'Protocol pin was not found.' : `Protocol pin lookup failed with HTTP ${response.status}.`);
    }
    ...
    if (protocolPath !== parsed.protocolPath) {
        return browserCommandFailed('browser_protocol_mismatch', `MAP URI expected ${parsed.protocolPath} but MAN returned ${protocolPath || '<empty>'}.`);
    }
