...
- If no injected context is available, run `metabot network services --online --query "<short task keywords>"` internally to refresh and search the local online service cache before preparing the request.
...
- `metabot services call` is the only command that starts remote delegation.
