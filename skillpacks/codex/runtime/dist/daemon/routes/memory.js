...
exports.handleMemoryRoutes = void 0;
 ...
 *  wrapped payload. */
const RAW_INPUT_VERBS = new Set(['hygieneRun']);
...
function validatePayload(verb, payload) {
...
const handleMemoryRoutes = async (context) => {
    ...
    if (!verb) {
        ...
        if (allowed.length > 0) {
            ...
            context.sendJson(200, (0, commandResult_1.commandFailed)('unknown_command', `Unknown memory route: ${url.pathname}`));
        }
        return true;
    }
    ...
    if (method === 'POST') {
        ...
        if (verb === 'list' || verb === 'knowledgeList') {
            ...
            if (limit === 'invalid') {
                context.sendJson(200, (0, commandResult_1.commandFailed)('invalid_flag', 'limit must be a positive integer.'));
                return true;
            }
            if (limit !== undefined)
                input.limit = limit;
        ...
        if (verb === 'impressionsShow'
            ...
            context.sendJson(200, (0, commandResult_1.commandFailed)('missing_subject', 'subject is required.'));
            return true;
        }
    }
...
exports.handleMemoryRoutes = handleMemoryRoutes;
