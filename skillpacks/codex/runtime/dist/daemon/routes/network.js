...
function parseStrictLimit(value) {
    if (value == null)
        return {};
    const normalized = value.trim();
    if (!normalized)
    ...
    if (!/^\d+$/.test(normalized) || !Number.isInteger(parsed) || parsed < 1 || parsed > 100) {
        return {
            error: (0, commandResult_1.commandFailed)('invalid_flag', `Unsupported --limit value: ${value}. Supported range: 1-100.`),
        };
    }
    return { limit: parsed };
}
const handleNetworkRoutes = async (context) => {
    ...
    if (url.pathname === '/api/network/products') {
        ...
        const result = handlers.network?.listProducts
            ...
            : (0, commandResult_1.commandFailed)('not_implemented', 'Network products handler is not configured.');
        context.sendJson(200, result);
        return true;
    }
