...
exports.lookupPeerChatPublicKey = lookupPeerChatPublicKey;
...
exports.llmDiscoverySweepRunningForHomeDir = llmDiscoverySweepRunningForHomeDir;
...
const serviceRefundProtocol_1 = require("../core/orders/serviceRefundProtocol");
const serviceRefundLifecycle_1 = require("../core/orders/serviceRefundLifecycle");
const serviceRefundChainReader_1 = require("../core/orders/serviceRefundChainReader");
const serviceRefundSync_1 = require("../core/orders/serviceRefundSync");
...
const writeGuard_1 = require("../core/metaapp/writeGuard");
const stageEvents_1 = require("../core/metaapp/stageEvents");
...
function readLargeFileUploadFailureFeeAssist(error) {
    return readLargeFileUploadFailureData(error)?.feeAssist;
}
...
function buildMetabotUpdateInput(input) {
    ...
    if (hasOwnField(input, 'ownerGlobalMetaId')) {
        update.ownerGlobalMetaId = (0, botRole_1.normalizeOptionalGlobalMetaId)(input.ownerGlobalMetaId);
    }
...
function buildMetabotCreateInput(input) {
    ...
    if (hasOwnField(input, 'allowChatSkills')) {
        const allowChatSkills = (0, chatSkillPolicy_1.normalizeAllowChatSkills)(input.allowChatSkills);
    ...
    if (hasOwnField(input, 'ownerGlobalMetaId')) {
        createInput.ownerGlobalMetaId = (0, botRole_1.normalizeOptionalGlobalMetaId)(input.ownerGlobalMetaId);
    }
    return createInput;
}
function calculateMetabotCreateInfoFields(input) {
    ...
    if (input.avatarDataUrl !== undefined)
        fields.push('avatar');
    if (input.primaryProvider !== undefined || input.fallbackProvider !== undefined)
...
function createTimeLlmDiscoveryDisabled(discover) {
    return !discover && process.env[TEST_SKIP_BACKGROUND_LLM_DISCOVERY_ENV] === '1';
}
function mergeMetabotCreateRuntimeCandidates(...groups) {
...
function hasMetabotCreateProviderCandidate(runtimes, provider) {
...
async function resolveDefaultMetabotCreateProviders(input) {
    const targetRuntimeStore = (0, llmRuntimeStore_1.createLlmRuntimeStore)((0, paths_1.resolveMetabotPaths)(input.homeDir));
    const targetRuntimeState = await targetRuntimeStore.read();
    const sourceHomeDir = input.sourceHomeDir ? node_path_1.default.resolve(input.sourceHomeDir) : node_path_1.default.resolve(input.homeDir);
    const sourceRuntimeState = sourceHomeDir === node_path_1.default.resolve(input.homeDir)
        ? targetRuntimeState
        : await (0, llmRuntimeStore_1.createLlmRuntimeStore)((0, paths_1.resolveMetabotPaths)(sourceHomeDir)).read();
    let candidateRuntimes = mergeMetabotCreateRuntimeCandidates(targetRuntimeState.runtimes, sourceRuntimeState.runtimes);
    const discover = input.discover ?? llmRuntimeDiscovery_1.discoverLlmRuntimes;
    ...
    for (const provider of [defaults.primaryProvider, defaults.fallbackProvider]) {
        ...
        const runtime = (0, metabotProfileManager_1.selectBestRuntimeForProvider)(candidateRuntimes, provider);
        if (!runtime)
            continue;
...
async function applyDefaultMetabotCreateProviders(input) {
    ...
    const systemDefaultProviderRoles = [];
...
async function collectCreateLlmBindingOutcome(input) {
    const [bindingState, runtimeState] = await Promise.all([
        (0, llmBindingStore_1.createLlmBindingStore)(input.homeDir).read(),
        (0, llmRuntimeStore_1.createLlmRuntimeStore)((0, paths_1.resolveMetabotPaths)(input.homeDir)).read(),
    ]);
...
function createMemoryRuntimeStore(state) {
    return {
        read: async () => state,
        write: async () => state,
...
function createMemoryBindingStore(state) {
    return {
        read: async () => state,
        write: async () => state,
        ...
        listEnabledByMetaBotSlug: async (slug) => (state.bindings.filter((binding) => binding.metaBotSlug === slug && binding.enabled)),
    };
}
...
function buildProspectivePrimaryBindingState(input) {
    ...
    const existing = selectVisibleRoleBinding(nextBindings.filter((binding) => (binding.metaBotSlug === input.profile.slug && binding.role === 'primary')));
    ...
    const runtime = (0, metabotProfileManager_1.selectRuntimeForProvider)(input.runtimeState.runtimes, input.primaryProvider);
    const binding = (0, llmTypes_1.normalizeLlmBinding)(buildBindingForRuntime({
...
function calculateMetabotChangedFields(current, update) {
    ...
    if (update.allowChatSkills !== undefined
...
function calculateMetabotSubmittedInfoFields(update) {
    const fields = [];
    ...
    if (update.avatarDataUrl !== undefined)
        fields.push('avatar');
    if (update.primaryProvider !== undefined || update.fallbackProvider !== undefined)
        fields.push('primaryProvider');
    if (update.allowChatSkills !== undefined)
        fields.push('allowChatSkills');
    if (update.homepage !== undefined)
...
function isPublicIdentityUpdate(update) {
...
function calculateMetabotUpdateInfoFields(current, update) {
...
async function validateMetabotProviderAvailability(profile, update) {
    ...
    for (const provider of requestedProviders) {
        const available = runtimeState.runtimes.some((runtime) => (runtime.provider === provider && runtime.health === 'healthy'));
        if (!available) {
...
async function ensureEmptyPersonaFiles(paths, metabotName) {
    ...
    for (const file of files) {
        try {
            const current = (await node_fs_1.promises.readFile(file.filePath, 'utf8')).trim();
        ...
        try {
            await node_fs_1.promises.writeFile(file.filePath, '\n', 'utf8');
        }
        catch {
...
async function resolveTraceRedirectTarget(input) {
    ...
    if (local && peer) {
        return (0, conversationUrl_1.buildConversationHref)(local, peer);
    }
    const referenceId = normalizeText(input.sessionId) || normalizeText(input.traceId);
    if (!referenceId) {
        return (0, conversationUrl_1.buildConversationHref)(local, peer);
    }
    const profiles = await (0, identityProfiles_1.listIdentityProfiles)(input.systemHomeDir).catch(() => []);
    for (const profile of profiles) {
        try {
            const session = await (0, traceProjection_1.getUnifiedA2ATraceSessionForProfile)({
                profile,
            ...
            if (session) {
                ...
                if (resolvedLocal && resolvedPeer) {
                    return (0, conversationUrl_1.buildConversationHref)(resolvedLocal, resolvedPeer);
                }
            }
        }
        catch {
 ...
 */
function slugifyMetaAppForkTitle(title) {
    const slug = title
        .toLowerCase()
        .replace(/[^a-z0-9]+/g, '-')
        .replace(/^-+|-+$/g, '')
...
function compareSellerReceivedRefundItems(left, right) {
...
function mergeInitiatedRefundsPayloads(payloads) {
...
function mergeProviderRefundsPayloads(payloads, kind) {
...
async function writeDirectorySeeds(directorySeedsPath, providers) {
    ...
    await node_fs_1.promises.writeFile(directorySeedsPath, `${JSON.stringify({
...
function extractOrderReferenceLineValue(content) {
    return normalizeText(extractOrderLineValue(content, 'order id'))
        || normalizeText(extractOrderLineValue(content, 'order reference'))
        || normalizeText(extractOrderLineValue(content, 'order pin id'));
}
...
async function buildTraceInspectorPayload(input) {
    ...
    return {
        ...
        localUiUrl: (0, conversationUrl_1.buildConversationLocalUiUrl)(input.daemon, localGlobalMetaId, peerGlobalMetaId)
...
function readChatPubKeyFromPayload(payload) {
...
async function lookupPeerChatPublicKey(globalMetaId, options = {}) {
...
function findLatestTerminalPublicStatusSnapshot(input) {
    const sessionIds = new Set(input.sessionState.sessions
        .filter((entry) => entry.traceId === input.traceId)
    ...
    return input.sessionState.publicStatusSnapshots
        .filter((entry) => sessionIds.has(entry.sessionId))
        .filter((entry) => (entry.status !== null && (0, watchEvents_1.isTerminalTraceWatchStatus)(entry.status)))
        .sort((left, right) => normalizeTraceTimestamp(left.resolvedAt) - normalizeTraceTimestamp(right.resolvedAt))
        .at(-1) ?? null;
}
...
const refundMutationLocks = new Map();
async function withRefundMutationLock(homeDir, operation) {
    const key = node_path_1.default.resolve(homeDir);
...
function llmDiscoveryStatusForHomeDir(homeDir) {
    const status = llmDiscoverySweepStatusByHomeDir.get(node_path_1.default.resolve(homeDir));
    if (!status)
        return undefined;
    return {
...
function llmDiscoverySweepRunningForHomeDir(homeDir) {
    return llmDiscoverySweepStatusByHomeDir.get(node_path_1.default.resolve(homeDir))?.running === true;
}
...
async function runTrackedLlmDiscoverySweep(homeDir, providers, discover) {
    ...
    const statusKey = node_path_1.default.resolve(homeDir);
...
function startBackgroundLlmDiscoverySweep(homeDir, providers, discover) {
    ...
    if (llmDiscoverySweepStatusByHomeDir.get(node_path_1.default.resolve(homeDir))?.running)
        return;
...
function createDefaultMetabotDaemonHandlers(input) {
    ...
    const infrastructureConfigStore = (0, infrastructureConfigStore_1.createInfrastructureConfigStore)(normalizedSystemHomeDir);
    async function resolveInfrastructure() {
        const config = await infrastructureConfigStore.read();
        return (0, metasoInfrastructure_1.resolveMetasoInfrastructureEndpoints)(config.metasoP2PBaseUrl);
    }
    async function resolveChatApiBaseUrl() {
        return normalizeText(input.chatApiBaseUrl)
            || (await resolveInfrastructure()).chatApiBaseUrl;
    }
    async function resolveSocketPresenceApiBaseUrl() {
    ...
    async function resolveFileUploadNetworkForHome(rawNetwork, homeDir) {
    ...
    async function resolveMvcSponsorUploadClientForHome(homeDir, network) {
        if (network !== 'mvc') {
            return undefined;
        }
        const config = await (0, configStore_1.createConfigStore)(homeDir).read();
    ...
    async function updateConfigDefaultWriteNetwork(targetConfigStore, rawInput) {
        ...
        if (chainInput.mvcSponsorUploadEnabled !== undefined
            && typeof chainInput.mvcSponsorUploadEnabled !== 'boolean') {
            return (0, commandResult_1.commandFailed)('invalid_argument', 'chain.mvcSponsorUploadEnabled must be a boolean.');
        }
    ...
    const serviceOrderPaymentVerifier = input.serviceOrderPaymentVerifier ?? servicePaymentVerification_1.verifyServiceOrderPayment;
    ...
    const conversationProfileWarmupLocals = new Map();
    const conversationProfileUpdateListeners = new Map();
    function subscribeConversationProfileUpdates(localGlobalMetaId, listener) {
    ...
    function publishConversationProfileUpdate(localGlobalMetaId) {
        const event = {
    ...
    function readChainProfileLlm(data) {
    ...
    async function resolveChainProfile(globalMetaId) {
        ...
        const pending = (async () => {
            ...
            try {
                const resp = await conversationProfileFetch(`https://so.metaid.io/api/info/globalmetaid/${encodeURIComponent(gmid)}`, { signal: controller.signal });
                if (resp.ok) {
    ...
    const callerTimeoutFinalizations = new Set();
    ...
    async function registerIdentityProfile(identity, profileHomeDir = input.homeDir) {
        ...
        await (0, twinRole_1.applyTwinInvariant)(normalizedSystemHomeDir, {}).catch(() => null);
    }
    ...
    function isInlineAvatarPayload(value) {
     ...
     */
    function mergeConversationRowAvatar(currentAvatar, profile) {
    ...
    async function resolveActorWriteContext(rawActor) {
        ...
        return {
            ...
            runtimeStateStore: normalizedProfileHomeDir === node_path_1.default.resolve(input.homeDir)
                ? runtimeStateStore
    ...
    async function runConversationGuidanceTurn(inputForTurn) {
        ...
        const profileSessionStateStore = profileHomeDir === node_path_1.default.resolve(input.homeDir)
            ? sessionStateStore
            : (0, sessionStateStore_1.createSessionStateStore)(profileHomeDir);
        ...
        void (async () => {
            try {
                ...
                console.warn('[conversations guidance] guided turn failed:', error instanceof Error ? error.message : String(error));
            }
        })();
    ...
    async function resolveLlmProfileForActor(inputProfile = {}) {
        ...
        const profiles = await (0, identityProfiles_1.listIdentityProfiles)(normalizedSystemHomeDir).catch(() => []);
        if (requestedSelector) {
            const resolved = (0, profileNameResolution_1.resolveProfileNameMatch)(requestedSelector, profiles);
            if (resolved.status === 'not_found' || resolved.status === 'ambiguous') {
                return {
                    failure: (0, commandResult_1.commandFailed)(resolved.status === 'ambiguous' ? 'identity_profile_ambiguous' : 'profile_not_found', resolved.message),
                };
            }
            return {
        ...
        return {
            slug: activeProfile?.slug || node_path_1.default.basename((0, paths_1.resolveMetabotPaths)(activeHomeDir).profileRoot),
            homeDir: activeHomeDir,
        };
    }
    async function resolveScopedServicesForActor(rawActor) {
        const requestedSlug = normalizeText(rawActor);
        if (!requestedSlug) {
            return {};
        }
        ...
        const profileHomeDir = node_path_1.default.resolve(selectedProfile.homeDir);
        if (profileHomeDir === node_path_1.default.resolve(input.homeDir)) {
            return {};
        }
    ...
    async function resolveScopedProviderForActor(rawActor) {
        const requestedSlug = normalizeText(rawActor);
        if (!requestedSlug) {
            return {};
        }
    ...
    async function resolveScopedTraceForActor(rawActor) {
        const requestedSlug = normalizeText(rawActor);
        if (!requestedSlug) {
            return {};
        }
    ...
    function getPublishedServicePinIds(service) {
        return [...new Set([
                ...(Array.isArray(service.chainPinIds) ? service.chainPinIds : []),
                service.sourceServicePinId,
                service.id,
    ...
    function publishedServiceMatchesId(service, serviceId) {
    ...
    function isSamePublishedService(left, right) {
    ...
    function normalizeMyServicesPage(value, fallback) {
        return normalizePositiveInteger(value) ?? fallback;
    }
    function readMyServiceDraftInput(rawInput, currentService) {
        ...
        if (!serviceName || !displayName || !description || !providerSkill || !price || !currency || !outputType) {
            return {
                error: (0, commandResult_1.commandFailed)('invalid_service_payload', 'Service payload is missing one or more required fields.'),
            };
        }
        if (!/^\d+(?:\.\d+)?$/u.test(price) || !Number.isFinite(Number(price)) || Number(price) < 0) {
            return {
                error: (0, commandResult_1.commandFailed)('invalid_service_payload', 'Service price must be a non-negative decimal number.'),
            };
        }
        const settlement = (0, publishService_1.resolvePublishedServiceSettlement)(currency);
        if (!['SPACE', 'BTC', 'DOGE', 'BTC-OPCAT'].includes(settlement.currency)) {
            return {
                error: (0, commandResult_1.commandFailed)('invalid_service_payload', 'Service currency must be one of BTC, SPACE, DOGE, or BTC-OPCAT.'),
            };
        }
        if (!['text', 'image', 'video', 'audio', 'other'].includes(outputType.toLowerCase())) {
            return {
                error: (0, commandResult_1.commandFailed)('invalid_service_payload', 'Service outputType must be one of text, image, video, audio, or other.'),
            };
        }
        return {
            draft: {
    ...
    async function listMyServicesProfileRecords() {
        const byHomeDir = new Map();
        const profiles = await (0, identityProfiles_1.listIdentityProfiles)(normalizedSystemHomeDir).catch(() => []);
        for (const profile of profiles) {
            const homeDir = normalizeText(profile.homeDir);
            if (!homeDir)
                continue;
            byHomeDir.set(node_path_1.default.resolve(homeDir), {
                slug: normalizeText(profile.slug) || node_path_1.default.basename(homeDir),
                name: normalizeText(profile.name) || node_path_1.default.basename(homeDir),
                homeDir,
            });
        }
        const activeHomeDir = node_path_1.default.resolve(input.homeDir);
        if (!byHomeDir.has(activeHomeDir)) {
            ...
            byHomeDir.set(activeHomeDir, {
                slug: node_path_1.default.basename(activeHomeDir),
                name: normalizeText(state?.identity?.name) || node_path_1.default.basename(activeHomeDir),
                homeDir: activeHomeDir,
            });
        }
        return [...byHomeDir.values()];
    }
    async function readMyServicesRatingDetails(profileHomeDir, refresh) {
        const store = (0, ratingDetailState_1.createRatingDetailStateStore)(profileHomeDir);
        if (!refresh) {
    ...
    async function loadMyServicesProfileInputs(refresh) {
        const records = await listMyServicesProfileRecords();
        const profiles = [];
        for (const profile of records) {
            const profileHomeDir = node_path_1.default.resolve(profile.homeDir);
            const store = profileHomeDir === node_path_1.default.resolve(input.homeDir)
                ? runtimeStateStore
                : (0, runtimeStateStore_1.createRuntimeStateStore)(profileHomeDir);
    ...
    async function loadProviderRefundProfileStates() {
        const records = await listMyServicesProfileRecords();
        const states = [];
        for (const profile of records) {
            const profileHomeDir = node_path_1.default.resolve(profile.homeDir);
    ...
    async function selectMyServicesProfilesForRequest(profiles, request) {
        const requestedFrom = normalizeText(request.from);
        if (requestedFrom) {
            const selectedProfile = await (0, metabotProfileManager_1.getMetabotProfile)(normalizedSystemHomeDir, requestedFrom);
            if (!selectedProfile) {
                return {
                    profiles: [],
                    failure: (0, commandResult_1.commandFailed)('profile_not_found', `MetaBot profile not found: ${requestedFrom}`),
                };
            }
            const selectedHomeDir = node_path_1.default.resolve(selectedProfile.homeDir);
            return {
                profiles: profiles.filter((profile) => node_path_1.default.resolve(normalizeText(profile.homeDir)) === selectedHomeDir),
            };
        }
        if (request.all === false) {
            const activeHomeDir = node_path_1.default.resolve(input.homeDir);
            return {
                profiles: profiles.filter((profile) => node_path_1.default.resolve(normalizeText(profile.homeDir)) === activeHomeDir),
            };
        }
        return { profiles };
    }
    async function resolveMyServiceMutationTarget(serviceId, actor) {
        const normalizedServiceId = normalizeText(serviceId);
        ...
        if (requestedFrom) {
            ...
            if (!selectedProfile) {
                ...
                const selectedHomeDir = node_path_1.default.resolve(selectedProfile.homeDir);
                records = records.filter((profile) => node_path_1.default.resolve(profile.homeDir) === selectedHomeDir);
            }
        }
        for (const profile of records) {
            const profileHomeDir = node_path_1.default.resolve(profile.homeDir);
            ...
            const service = state.services.find((candidate) => (publishedServiceMatchesId(candidate, normalizedServiceId)
                && (!normalizeText(state.identity?.globalMetaId)
                    || !normalizeText(candidate.providerGlobalMetaId)
                    || normalizeText(state.identity?.globalMetaId) === normalizeText(candidate.providerGlobalMetaId)))) ?? null;
            if (!service)
                continue;
            return {
                target: {
    ...
    async function sendProviderOrderStatusNoticeBestEffort(inputNotice) {
        ...
        try {
            const wireContent = (0, orderProtocol_1.buildOrderStatusMessage)(inputNotice.orderTxid, content);
    ...
    async function cleanupProviderRunWorkspace(runnerResult) {
        const attemptWorkspaceCwd = normalizeText(readObject(runnerResult?.metadata)?.attemptWorkspaceCwd);
        if (!attemptWorkspaceCwd) {
            return;
        }
        await (0, providerWorkspaceCleanup_1.removeProviderRunWorkspace)(runtimeStateStore.paths.profileRoot, attemptWorkspaceCwd)
            .catch(() => undefined);
    }
    ...
    function buildRefundRequestPayloadForTrace(inputRefund) {
        ...
        const corePayload = (0, serviceRefundProtocol_1.buildServiceRefundRequestPayload)({
            version: 1,
            serviceOrderPinId,
    ...
    async function runBuyerRefundRequestLifecycleForDueTraces(nowMs = Date.now()) {
        const state = await runtimeStateStore.readState();
        return (0, serviceRefundLifecycle_1.runBuyerRefundRequestLifecycle)({
            traces: state.traces,
            nowMs,
            localGlobalMetaId: state.identity?.globalMetaId,
            writer: {
    ...
    function isSellerOrderEligibleForLocalBuyerRefundRequest(order) {
        if (normalizeText(order.refundRequestPinId)) {
            return false;
        }
    ...
    function readBuyerTraceProviderGlobalMetaId(trace) {
    ...
    function buyerTraceMatchesSellerOrder(inputRefund) {
        ...
        if (normalizeText(order.providerGlobalMetaId)
    ...
    function withSellerOrderRefundRequest(inputRefund) {
        ...
        const nextSellerOrders = inputRefund.state.sellerOrders.map((order) => {
            ...
            return {
                ...order,
                state: order.state === 'failed' ? 'refund_pending' : order.state,
                refundRequestPinId,
                refundRequestTxid,
    ...
    function sellerOrderHasLocalRefundSettlement(order) {
    ...
    function buyerTraceAlreadyHasSellerRefundSettlement(inputRefund) {
        const traceOrder = (inputRefund.trace.order ?? {});
    ...
    async function applyLocalBuyerRefundSettlementForSellerOrder(inputRefund) {
        ...
        const profiles = await (0, metabotProfileManager_1.listMetabotProfiles)(normalizedSystemHomeDir).catch(() => []);
        for (const profile of profiles) {
            if (node_path_1.default.resolve(profile.homeDir) === node_path_1.default.resolve(input.homeDir)) {
                continue;
            }
            const buyerRuntimeStateStore = (0, runtimeStateStore_1.createRuntimeStateStore)(profile.homeDir);
            ...
            for (const trace of matchingTraces) {
                ...
                await persistTraceRecord(buyerRuntimeStateStore, withBuyerRefundSettlementFromSellerOrder({ trace, order }));
                updated += 1;
            }
        }
        return updated;
    }
    async function reconcileLocalRefundLedgersAcrossProfiles() {
        const records = await listMyServicesProfileRecords();
        const sellerOrders = [];
        for (const profile of records) {
            const profileHomeDir = node_path_1.default.resolve(profile.homeDir);
    ...
    async function prepareLocalBuyerRefundRequestForSellerOrder(inputRefund) {
        ...
        for (const profile of profiles) {
            ...
            const preparedOrder = (preparedTrace.order ?? {});
            ...
            const nextOrder = (0, providerOperations_1.findSellerOrderBySelector)(nextProviderState, { orderId: inputRefund.order.id }).order
                ?? inputRefund.order;
            return {
    ...
    function emptyServiceRefundSyncResponse() {
        return {
            scanned: {
                requestPins: 0,
                finalizePins: 0,
    ...
    function mergeServiceRefundSyncResponses(left, right) {
        return {
            scanned: {
    ...
    function normalizeRefundAsset(value) {
    ...
    async function verifyServiceRefundFinalizeTransfer(finalize) {
        ...
        const refundRequestPayload = (0, serviceRefundProtocol_1.parseRefundProtocolContent)(refundRequestDetail.content);
        if (!refundRequestPayload) {
            return false;
        }
        const expectedAmount = normalizeRefundAmount(refundRequestPayload.refundAmount
            ?? refundRequestPayload.amount
            ?? refundRequestPayload.paymentAmount);
        const expectedAsset = normalizeRefundAsset(refundRequestPayload.refundCurrency
    ...
    async function syncServiceRefundsForCurrentProfile(nowMs = Date.now()) {
        return withRefundMutationLock(input.homeDir, async () => {
            ...
            const reader = (0, serviceRefundChainReader_1.createServiceRefundChainReader)({
                chainApiBaseUrl: input.chainApiBaseUrl,
            });
            ...
            const identity = {
                ...
                localMetabotSlug: node_path_1.default.basename(node_path_1.default.resolve(input.homeDir)),
                localGlobalMetaId: normalizeText(stateAfterLifecycle.identity?.globalMetaId),
            };
            const requestSync = (0, serviceRefundSync_1.applyServiceRefundRequestsToState)({
                state: stateAfterLifecycle,
                requests: requestPins,
                identity,
                nowMs,
            });
            const finalizeSync = await (0, serviceRefundSync_1.applyServiceRefundFinalizationsToState)({
            ...
            if (requestSync.nextState !== stateAfterLifecycle || finalizeSync.nextState !== requestSync.nextState) {
                ...
                await runtimeStateStore.writeState((0, serviceRefundSync_1.mergeServiceRefundSyncState)({
                    currentState,
        ...
        for (const peerGlobalMetaId of candidatePeers) {
            ...
            if (orderReference && conversation) {
                const matchingMessage = conversation.messages.find((message) => (message.kind === 'order_protocol'
                    ...
                    && extractOrderReferenceLineValue(message.content) === orderReference));
    ...
    async function failInboundProviderOrderValidation(inputFailure) {
        const state = inputFailure.state;
        const service = inputFailure.service ?? null;
        const orderTxid = normalizeText(inputFailure.orderTxid);
        const orderReference = normalizeText(inputFailure.orderReference);
        const paymentTxid = normalizeText(inputFailure.paymentTxid);
        const servicePinId = normalizeText(service?.currentPinId)
            || normalizeText(inputFailure.servicePinId);
        const serviceName = normalizeText(service?.displayName)
            || normalizeText(service?.serviceName)
            || null;
        const outputType = normalizeText(inputFailure.outputType)
            || normalizeText(service?.outputType);
        const orderMessageId = normalizeText(inputFailure.orderMessageId)
            || normalizeText(inputFailure.orderPinId)
            || orderTxid
            || orderReference
            || `validation-${inputFailure.timestamp}`;
        const failureText = normalizeText(inputFailure.message) || 'Inbound ORDER failed validation.';
        const traceId = orderTxid
            ? `trace-provider-${sanitizeServiceSegment(orderTxid.slice(0, 16))}`
            : `trace-provider-${sanitizeServiceSegment(orderMessageId.slice(0, 16))}`;
        let peerChatPublicKey = '';
        try {
            peerChatPublicKey = await resolvePeerChatPublicKey(inputFailure.buyerGlobalMetaId) ?? '';
        }
        catch {
            peerChatPublicKey = '';
        }
        const orderEndContent = (0, orderProtocol_1.buildOrderEndMessage)(orderTxid, 'failed', failureText, orderReference || null);
        let orderEndWrite = null;
        let orderEndSendFailure = null;
        if (peerChatPublicKey) {
            try {
                orderEndWrite = await sendProviderOrderPrivateMessage({
                    toGlobalMetaId: inputFailure.buyerGlobalMetaId,
                    peerChatPublicKey,
        ...
        await (0, conversationPersistence_1.persistA2AConversationMessageBestEffort)({
            ...
            peer: {
                globalMetaId: inputFailure.buyerGlobalMetaId,
                chatPublicKey: peerChatPublicKey || null,
            },
            message: {
                messageId: normalizeText(inputFailure.orderPinId) || orderMessageId,
                direction: 'incoming',
                content: inputFailure.content,
                pinId: normalizeText(inputFailure.orderPinId) || null,
                txid: orderTxid || normalizeText(inputFailure.orderPinId) || null,
                txids: orderTxid ? [orderTxid] : [],
                chain: 'mvc',
                orderTxid: orderTxid || null,
                serviceOrderPinId: orderReference || null,
                paymentTxid: paymentTxid || null,
                timestamp: inputFailure.timestamp,
                raw: {
                    protocol: 'ORDER',
                    validationFailure: inputFailure.code,
                },
            },
            orderSession: orderTxid ? {
                role: 'provider',
                state: 'received',
                orderTxid,
                serviceOrderPinId: orderReference || null,
                paymentTxid: paymentTxid || null,
                servicePinId: servicePinId || null,
                serviceName,
                outputType: outputType || null,
                createdAt: inputFailure.timestamp,
            } : null,
        }, a2aConversationPersister);
        await (0, conversationPersistence_1.persistA2AConversationMessageBestEffort)({
            paths: runtimeStateStore.paths,
            ...
            orderSession: orderTxid ? {
                ...
                outputType: outputType || null,
                endedAt: Date.now(),
                endReason: inputFailure.code,
                failureReason: failureText,
            } : null,
        }, a2aConversationPersister);
        if (service && orderTxid) {
            const received = sessionEngine.receiveProviderTask({
                traceId,
                servicePinId: service.currentPinId,
                callerGlobalMetaId: inputFailure.buyerGlobalMetaId,
                providerGlobalMetaId: state.identity.globalMetaId,
                userTask: inputFailure.userTask,
                taskContext: '',
            });
            const receivedStatus = await persistSessionMutation(sessionStateStore, received);
            const failedResult = (0, serviceRunnerContracts_1.createServiceRunnerFailedResult)(inputFailure.code, failureText);
            ...
            await appendA2ATranscriptItems(sessionStateStore, [
                ...
                {
                    id: `${traceId}-provider-validation-failure`,
                    sessionId: received.session.sessionId,
                    ...
                    metadata: {
                        ...
                        orderTxid,
                        paymentTxid: paymentTxid || null,
                        refundRequired: Boolean(paymentTxid),
                    },
                },
                {
                    ...
                    metadata: {
                        ...
                        orderEndPinId: orderEndWrite?.pinId ?? null,
                        txids: orderEndWrite?.txids ?? [],
                        publicStatus: failedStatus.status,
                        event: failedApplied.event,
                        refundRequired: Boolean(paymentTxid),
            ...
            await persistProviderFailureTrace({
                ...
                orderReference: orderReference || null,
                serviceOrderPinId: orderReference || null,
                paymentCurrency: normalizeText(inputFailure.paymentCurrency) || service.currency,
                paymentAmount: normalizeText(inputFailure.paymentAmount) || service.price,
                paymentChain: inputFailure.paymentChain || null,
                settlementKind: inputFailure.settlementKind || null,
                mrc20Ticker: inputFailure.mrc20Ticker || null,
                mrc20Id: inputFailure.mrc20Id || null,
                ...
                userTask: inputFailure.userTask,
                failureText,
                failureCode: inputFailure.code,
            });
        }
        const data = {
            handled: true,
            notified: Boolean(orderEndWrite?.pinId || orderEndWrite?.txids?.length),
            orderTxid: orderTxid || null,
            paymentTxid: paymentTxid || null,
            orderReference: orderReference || null,
        };
        if (orderEndSendFailure) {
            data.notificationError = orderEndSendFailure;
        }
        return (0, commandResult_1.commandFailed)(inputFailure.code, failureText, { data });
    }
    async function handleInboundProviderOrderMessage(inputMessage) {
        ...
        const orderTxid = (0, metawebReplyWaiter_1.normalizeOrderProtocolReference)(inputMessage.messagePinId)
            ...
            || (0, metawebReplyWaiter_1.normalizeOrderProtocolReference)(extractOrderLineValue(content, 'order pin id'))
            || '';
        ...
        const service = servicePinId
            ? state.services.find((entry) => (entry.available === 1
                && entry.providerGlobalMetaId === state.identity.globalMetaId
                && (entry.currentPinId === servicePinId || entry.sourceServicePinId === servicePinId))) ?? null
            : null;
        const failValidation = (code, message) => failInboundProviderOrderValidation({
            state,
            service,
            ...
            outputType: outputType || null,
            paymentCurrency: amountLine.currency || service?.currency || null,
            paymentAmount: amountLine.amount || service?.price || null,
        ...
        if (!orderTxid) {
            return failValidation('order_reference_missing', 'Inbound ORDER must include a chain order txid or pin id.');
        }
        if (!servicePinId) {
            return failValidation('order_service_missing', 'Inbound ORDER must include a service id.');
        }
        if (!service) {
            return failValidation('service_not_found', `Published service was not found: ${servicePinId}`);
        }
        if (providerSkill && providerSkill !== service.providerSkill) {
            return failValidation('order_provider_skill_mismatch', 'Inbound ORDER provider skill does not match the local service.');
        }
        ...
        if (!paymentTxid && !orderReference) {
            return failValidation('order_payment_unverified', 'Inbound ORDER is missing payment txid or free order reference.');
        }
        ...
        if (serviceIsFree) {
            if (!orderReference || paymentTxid) {
                return failValidation('order_payment_unverified', 'Free inbound ORDER must use an order reference instead of payment txid.');
            }
        }
        else if (!paymentTxid) {
            return failValidation('order_payment_unverified', 'Paid inbound ORDER must include a payment txid.');
        }
        if (!orderAmount || orderAmount !== serviceAmount || !orderCurrency || orderCurrency !== serviceCurrency) {
            return failValidation('order_payment_unverified', 'Inbound ORDER payment terms do not match the local service.');
        }
        if (!serviceIsFree && !paymentChain) {
            return failValidation('order_payment_unverified', 'Paid inbound ORDER must include payment chain metadata.');
        }
        ...
        if (!paymentVerification.verified && paymentVerification.outcome === 'error') {
            ...
            return (0, commandResult_1.commandFailed)('order_payment_verification_unavailable', 'Inbound ORDER payment could not be verified because the chain API is unavailable; the order was not failed and can be retried.', {
                data: {
                    handled: true,
                    transient: true,
                    retryable: true,
        ...
        if ((0, providerDeliveryArtifacts_1.classifyProviderOutputType)(service.outputType) === 'video') {
            try {
        ...
        const providerRunner = (0, providerServiceRunner_1.createProviderServiceRunner)({
            ...
            sessionTimeoutMs: (0, providerServiceRunner_1.resolveProviderOrderExecutionTimeoutMs)(service),
        });
        ...
        const runProviderExecutionWithProgressHeartbeat = async (executeRun) => {
        ...
        const runnerResult = await runProviderExecutionWithProgressHeartbeat(() => providerRunner
            ...
            .catch((error) => (0, serviceRunnerContracts_1.createServiceRunnerFailedResult)(readErrorCode(error, 'provider_execution_failed'), readErrorMessage(error, 'provider_execution_failed', 'Provider execution failed before a runner result was produced.'))));
        ...
        if (runnerResult.state !== 'completed') {
            ...
            return runnerResult.state === 'failed'
                ...
                : (0, commandResult_1.commandFailed)('clarification_not_supported', failureText);
        }
        ...
        if (!(0, providerDeliveryArtifacts_1.isTextLikeProviderOutputType)(service.outputType)) {
            const resolveOrderDeliveryArtifacts = async (deliveryResult) => {
                ...
                return (0, providerDeliveryArtifacts_1.resolveProviderDeliveryArtifacts)({
                    ...
                    mvcSponsorClient: await resolveMvcSponsorUploadClientForHome(runtimeStateStore.paths.profileRoot, artifactNetwork),
            ...
            let deliveryPreparationError = null;
            ...
            try {
                ...
                if (readErrorCode(initialError, 'provider_artifact_delivery_failed') === 'provider_artifact_missing'
                    ...
                    const continuationResult = await runProviderExecutionWithProgressHeartbeat(() => providerRunner
                        ...
                        .catch((error) => (0, serviceRunnerContracts_1.createServiceRunnerFailedResult)(readErrorCode(error, 'provider_execution_failed'), readErrorMessage(error, 'provider_execution_failed', 'Provider continuation execution failed before a runner result was produced.'))));
            ...
            if (resolvedDelivery) {
                ...
                await cleanupProviderRunWorkspace(runnerResult);
                return (0, commandResult_1.commandFailed)(failureCode, failureText, data ? { data } : undefined);
            }
        }
        const deliveryMessage = (0, orderProtocol_1.buildDeliveryMessage)({
            paymentTxid: paymentTxid || null,
        ...
        try {
            artifacts = await (0, transcriptExport_1.exportSessionArtifacts)({
                trace,
                transcript: {
                    sessionId: trace.session.id,
                    title: trace.session.title,
    ...
    async function applyInboundSellerOrderRated(inputRating) {
        ...
        const orderTxid = (0, metawebReplyWaiter_1.normalizeOrderProtocolReference)(inputRating.orderEnd.orderTxid);
        ...
        const sellerOrder = state.sellerOrders.find((entry) => {
            ...
            return Boolean((orderTxid && (0, metawebReplyWaiter_1.normalizeOrderProtocolReference)(entry.orderTxid) === orderTxid)
                || (orderPinId && (normalizeText(entry.serviceOrderPinId) === orderPinId
        ...
        if (sellerOrder.state === 'rating_pending') {
            await runtimeStateStore.updateState((current) => {
                ...
                return {
                    ...current,
                    sellerOrders: (0, sellerOrderState_1.upsertSellerOrderRecord)(current.sellerOrders, {
                        ...currentOrder,
                        state: 'completed',
                        updatedAt: Date.now(),
                    }),
    ...
    async function runSellerRatingTimeoutSweep(nowMs = Date.now()) {
        const state = await runtimeStateStore.readState();
        if (!state.identity) {
        ...
        const staleOrders = state.sellerOrders.filter((entry) => {
            ...
            return Number.isFinite(requestedAt) && requestedAt + orderLifecycle_1.SERVICE_ORDER_RATING_TIMEOUT_MS <= nowMs;
        });
        ...
        for (const staleOrder of staleOrders) {
            try {
                const serviceOrderPinId = normalizeText(staleOrder.serviceOrderPinId)
                    ...
                    || null;
                const lookupRating = (serviceId) => ratingState && serviceId
                    ? (0, ratingDetailSync_1.findRatingDetailByServicePayment)(ratingState, {
                        serviceId,
                        serviceOrderPinId,
                ...
                if (!ratingDetail && orderTxid) {
                    const orderEndContent = (0, orderProtocol_1.buildOrderEndMessage)(orderTxid, 'rating_timeout', 'Rating wait timed out; the delivered order is now closed.', serviceOrderPinId);
                    let peerChatPublicKey = '';
                    try {
    ...
    async function runBuyerOrderDeadlineSweep(nowMs = Date.now()) {
        const state = await runtimeStateStore.readState();
        if (!state.identity) {
        ...
        for (const session of waitingSessions) {
            ...
            scanned += 1;
            const deadlines = (0, orderLifecycle_1.resolveServiceOrderDeadlines)({
                firstResponseDeadlineAt: order?.firstResponseDeadlineAt,
                deliveryDeadlineAt: order?.deliveryDeadlineAt,
                createdAt: trace.createdAt,
            });
            const transition = (0, orderLifecycle_1.getServiceOrderDeadlineTimeout)({
                ...deadlines,
    ...
    async function handleInboundOrderProtocolMessage(inputMessage) {
        ...
        const orderStatus = delivery || needsRating || orderEnd
            ? null
            : (0, orderProtocol_1.parseOrderStatusMessage)(content);
    ...
    function resolveCallerReplyTimeoutReason(trace, nowMs = Date.now()) {
        const order = (trace.order ?? {});
        const deadlines = (0, orderLifecycle_1.resolveServiceOrderDeadlines)({
            firstResponseDeadlineAt: order.firstResponseDeadlineAt,
            deliveryDeadlineAt: order.deliveryDeadlineAt,
        ...
        return (0, orderLifecycle_1.getServiceOrderDeadlineTimeout)({
            ...deadlines,
            firstResponseReceivedAt: order.firstResponseReceivedAt,
    ...
    async function applyCallerOrderEndFailure(inputFailure) {
        ...
        const failedResult = (0, serviceRunnerContracts_1.createServiceRunnerFailedResult)(failureCode, failureText);
    ...
    function scheduleCallerReplyContinuation(input) {
        ...
        const waitTimeoutMs = Number.isFinite(input.waiterInput.timeoutMs)
            ...
            : orderLifecycle_1.SERVICE_ORDER_DELIVERY_TIMEOUT_MS;
    ...
    async function resumePendingCallerReplyContinuations(inputResume) {
        ...
        if (localProfileSlug) {
            const selectedProfile = await (0, metabotProfileManager_1.getMetabotProfile)(normalizedSystemHomeDir, localProfileSlug);
            if (!selectedProfile) {
                return report;
            }
            const profileHomeDir = node_path_1.default.resolve(selectedProfile.homeDir);
        ...
        for (const session of waitingSessions) {
            ...
            try {
                ...
                const orderDeadlines = (0, orderLifecycle_1.resolveServiceOrderDeadlines)({
                    firstResponseDeadlineAt: order?.firstResponseDeadlineAt,
                    deliveryDeadlineAt: order?.deliveryDeadlineAt,
                    createdAt: trace.createdAt,
                });
                ...
                const privateChatIdentity = await signer.getPrivateChatIdentity();
                const providerChatPublicKey = (session.providerGlobalMetaId === identity.globalMetaId
                ...
                scheduleCallerReplyContinuation({
                    ...
                    waiterInput: {
                        ...
                        paymentTxid: normalizeText(order?.paymentTxid),
                        orderTxid: (0, metawebReplyWaiter_1.normalizeOrderProtocolReference)(order?.orderTxid)
                            || (0, metawebReplyWaiter_1.normalizeOrderProtocolReference)(order?.orderPinId)
                            || null,
        ...
        return withRefundMutationLock(input.homeDir, async () => {
            ...
            const settlement = await (0, serviceRefundSettlement_1.processSellerRefundSettlement)({
                ...
                fetchRefundRequestPin: async (pinId) => {
                    if (prepared.localRefundRequestDetail
                ...
                writeRefundFinalizePin: async ({ payload }) => signer.writePin({
                    operation: 'create',
                    path: serviceRefundProtocol_1.SERVICE_REFUND_FINALIZE_PATH,
                    encryption: '0',
                    version: '1.0.0',
                    contentType: 'application/json',
                    payload: JSON.stringify(payload),
    ...
    const metaAppWriteGuard = (0, writeGuard_1.createMetaAppWriteGuard)();
    ...
    const metaAppStageHub = (0, stageEvents_1.createMetaAppStageHub)();
    let daemonHandlers = null;
    function emitMetaAppOpTerminal(opId, result) {
        if (!opId)
            return;
    ...
    async function readMetaAppRecordForUpdate(actorHomeDir, targetPinId) {
        ...
        const local = localRecords.find((record) => record.pinId === targetPinId || record.firstPinId === targetPinId);
        if (local)
            return local;
        ...
        const state = await (0, runtimeStateStore_1.createRuntimeStateStore)(actorHomeDir).readState().catch(() => null);
        const mvcAddress = normalizeText(state?.identity?.addresses?.mvc) || normalizeText(state?.identity?.mvcAddress);
        if (!mvcAddress)
            return null;
        let cursor = '';
        for (let page = 0; page < 3; page += 1) {
            ...
            if (match)
                return galleryRecordFromOwnerListRecord(match);
    ...
    function galleryRecordFromOwnerListRecord(record) {
        return {
            pinId: record.pinId,
    ...
    const handlers = {
        ...
        metaapp: {
            ...
            publishProject: async (rawInput) => {
                ...
                const onStage = opId
                    ? (stage, detail) => metaAppStageHub.publish(opId, { stage, ...detail })
                    : undefined;
                const finish = (result) => {
                ...
                try {
                    ...
                    if (result.ok && result.data && typeof result.data === 'object') {
                        ...
                        if (pinId) {
                            ...
                            return finish((0, commandResult_1.commandSuccess)({
                                ...data,
                                localUiUrl,
                            }));
                        }
                    ...
                    const data = readLargeFileUploadFailureData(error);
                    return finish((0, commandResult_1.commandFailed)('metaapp_publish_failed', error instanceof Error ? error.message : String(error), data ? { data } : undefined));
                }
            },
            updateProject: async (rawInput) => {
                ...
                try {
                    ...
                    const data = readLargeFileUploadFailureData(error);
                    return finish((0, commandResult_1.commandFailed)('metaapp_update_failed', error instanceof Error ? error.message : String(error), data ? { data } : undefined));
                }
            },
            publish: async (rawInput) => {
            ...
            fork: async (rawInput) => {
                ...
                const pinId = (0, pinId_1.normalizeMetaAppPinIdOrUri)(rawInput.pinId);
                if (!pinId) {
                    return (0, commandResult_1.commandFailed)('invalid_argument', 'pinId must be a MetaApp pin id or a metaapp://<pinId> URI.');
                }
                ...
                const outDir = node_path_1.default.join(actor.homeDir, 'workspace', 'metaapps', `${slugifyMetaAppForkTitle(title)}-${pinId.slice(0, 8)}-${Date.now()}`);
                try {
                    const infrastructure = await infrastructureConfigStore.read();
                    const materialize = input.metaAppSourceMaterialize ?? metaAppSource_1.materializeMetaAppSource;
                    return await materialize({ pinId, outDir }, {
                        homeDir: actor.homeDir,
                    ...
                    return (0, commandResult_1.commandFailed)('metaapp_fork_failed', error instanceof Error ? error.message : String(error));
                }
            },
            list: async (rawInput) => {
            ...
            create: async ({ name, host, profileSlug }) => {
                ...
                try {
                    ...
                    if (error instanceof RequestedMetabotHostUnavailableError) {
                        return (0, commandResult_1.commandFailed)('requested_host_unavailable', error.message);
                    }
                    return (0, commandResult_1.commandFailed)('metabot_provider_selection_failed', error instanceof Error ? error.message : String(error));
                }
                ...
                if (nextState.identity && (bootstrap.success || bootstrap.canSkip)) {
                    ...
                    try {
                        const profile = await (0, metabotProfileManager_1.createMetabotProfileFromIdentity)(normalizedSystemHomeDir, {
                            ...
                            systemDefaultProviderRoles: [
                                ...(defaultProviders.primaryProvider ? ['primary'] : []),
                                ...(defaultProviders.fallbackProvider ? ['fallback'] : []),
                            ],
                        });
        ...
        provider: {
            ...
            getInitiatedRefunds: async (request = {}) => {
                ...
                if (!requestedFrom && request.all === true) {
                    ...
                    return (0, commandResult_1.commandSuccess)(mergeInitiatedRefundsPayloads(states.map((state) => buildInitiatedRefundsPayload({ state }))));
                }
                const state = await runtimeStateStore.readState();
        ...
        services: {
            listMyServices: async (request) => {
                ...
                return (0, commandResult_1.commandSuccess)((0, myServices_1.buildMyServiceSummaries)({
            ...
            listMyServiceOrders: async (request) => {
                ...
                if (!serviceId) {
                    return (0, commandResult_1.commandFailed)('invalid_service_request', 'My service orders request must include serviceId.');
                }
                const loadedProfiles = await loadMyServicesProfileInputs(Boolean(request.refresh));
                const selected = await selectMyServicesProfilesForRequest(loadedProfiles, request);
                if (selected.failure) {
                ...
                return (0, commandResult_1.commandSuccess)((0, myServices_1.buildMyServiceOrderDetails)({
                    serviceId,
                    profiles: selected.profiles,
                    page: normalizeMyServicesPage(request.page, 1),
                    pageSize: normalizeMyServicesPage(request.pageSize, 20),
            ...
            modifyMyService: async (rawInput) => {
                ...
                if (!serviceId) {
                    return (0, commandResult_1.commandFailed)('invalid_service_request', 'My service modify request must include serviceId.');
                }
                const resolved = await resolveMyServiceMutationTarget(serviceId, rawInput.from);
                const validation = (0, myServices_1.validateMyServiceMutation)({
                    action: 'modify',
                    target: resolved.target,
                });
                if (!validation.ok || !resolved.target.service || !resolved.target.identity || !resolved.store || !resolved.state) {
                    return (0, commandResult_1.commandFailed)(validation.errorCode ?? 'my_service_modify_unavailable', validation.error ?? 'Service is not available for modification.');
                }
                ...
                const providerSkillValidation = await (0, servicePublishValidation_1.validateServicePublishProviderSkills)({
                    ...
                    bindingStore: resolved.target.profileHomeDir === node_path_1.default.resolve(input.homeDir)
                        ? llmBindingStore
                        : (0, llmBindingStore_1.createLlmBindingStore)(resolved.target.profileHomeDir),
                ...
                if (!providerSkillValidation.ok) {
                    return (0, commandResult_1.commandFailed)(providerSkillValidation.code, providerSkillValidation.message);
                }
                try {
                    const now = Date.now();
                    ...
                    const icon = await (0, servicePublishChain_1.uploadServiceIconDataUrl)({
                        signer: profileSigner,
                    ...
                    const paymentAddress = (0, myServices_1.resolveMyServicePaymentAddress)({
                        identity: resolved.target.identity,
                        currency: preparedDraft.currency,
                        fallback: resolved.target.service.paymentAddress,
                    });
                    const payload = (0, myServices_1.buildMyServicePayload)({
                        draft: preparedDraft,
                        providerGlobalMetaId: resolved.target.identity.globalMetaId,
                        paymentAddress,
                    });
                    const payloadJson = JSON.stringify(payload);
                    const chainWrite = await profileSigner.writePin((0, myServices_1.buildMyServiceModifyChainWrite)({
                        targetPinId: resolved.target.service.currentPinId,
                        payloadJson,
                        network,
                    }));
                    const updatedRecord = (0, myServices_1.buildMyServiceModifyRecord)({
                    ...
                    return (0, commandResult_1.commandFailed)('service_modify_failed', error instanceof Error ? error.message : String(error));
                }
            },
            revokeMyService: async (rawInput) => {
                const serviceId = normalizeText(rawInput.serviceId ?? rawInput.servicePinId ?? rawInput.currentPinId);
                if (!serviceId) {
                    return (0, commandResult_1.commandFailed)('invalid_service_request', 'My service revoke request must include serviceId.');
                }
                const resolved = await resolveMyServiceMutationTarget(serviceId, rawInput.from);
                const validation = (0, myServices_1.validateMyServiceMutation)({
                    action: 'revoke',
                ...
                if (!validation.ok || !resolved.target.service || !resolved.store || !resolved.state) {
                    return (0, commandResult_1.commandFailed)(validation.errorCode ?? 'my_service_revoke_unavailable', validation.error ?? 'Service is not available for revocation.');
                }
                try {
                    const now = Date.now();
                    const network = await resolveWriteNetworkForHome(rawInput.network, resolved.target.profileHomeDir);
                    const profileSigner = createSignerForProfileHome(resolved.target.profileHomeDir);
                    const chainWrite = await profileSigner.writePin((0, myServices_1.buildMyServiceRevokeChainWrite)({
                        targetPinId: resolved.target.service.currentPinId,
                        network,
                    }));
                    const updatedRecord = (0, myServices_1.buildMyServiceRevokeRecord)({
                        service: resolved.target.service,
                        now,
                    });
                    await resolved.store.writeState({
            ...
            syncRefunds: async (request = {}) => {
                ...
                if (!requestedFrom && request.all === true) {
                    ...
                    for (const profile of records) {
                        ...
                        if (!result?.ok) {
                            return (0, commandResult_1.commandFailed)(result?.code ?? 'service_refund_sync_failed', result?.message ?? `Failed to sync refunds for profile: ${profile.slug}`);
                        }
                    ...
                    return (0, commandResult_1.commandSuccess)(merged);
                }
                return (0, commandResult_1.commandSuccess)(await syncServiceRefundsForCurrentProfile());
            },
            inspectOrder: async ({ from, orderId, paymentTxid }) => {
                const scoped = await resolveScopedProviderForActor(from);
                if (scoped.failure) {
            ...
            listPublishSkills: async (request = {}) => {
                ...
                const chatSkillResolution = await (0, chatSkillPolicy_1.readChatSkillResolution)(profileRuntimeStateStore.paths.chatSkillResolutionPath).catch(() => null);
                return (0, commandResult_1.commandSuccess)({
                    metaBotSlug,
                    identity: {
                        metabotId: state.identity.metabotId,
            ...
            call: async (rawInput) => {
                ...
                const persistCallerTraceSnapshot = async (failure) => {
                    ...
                    const deadlines = (orderDeadlines ??= (0, orderLifecycle_1.computeServiceOrderDeadlines)(Date.now()));
                ...
                if (!request.providerDaemonBaseUrl) {
                    ...
                    const deliveryDeadlineAt = trace.order?.deliveryDeadlineAt
                        ?? (0, orderLifecycle_1.computeServiceOrderDeadlines)(Date.now()).deliveryDeadlineAt;
                    ...
                    return (0, commandResult_1.commandWaiting)('order_sent_awaiting_provider', 'Order sent to provider. Waiting for response...', 3000, {
                        localUiUrl: (0, conversationUrl_1.buildConversationLocalUiUrl)(daemon, state.identity?.globalMetaId, plan.service.providerGlobalMetaId)
                ...
                return (0, commandResult_1.commandSuccess)({
                    ...
                    localUiUrl: (0, conversationUrl_1.buildConversationLocalUiUrl)(input.getDaemonRecord(), state.identity?.globalMetaId, plan.service.providerGlobalMetaId)
            ...
            execute: async (rawInput) => {
                ...
                const serviceCurrency = (0, publishService_1.normalizePublishedServiceCurrency)(service.currency);
                const orderCurrency = (0, publishService_1.normalizePublishedServiceCurrency)(execution.payment.paymentCurrency);
                const serviceIsFree = Number(serviceAmount || '0') === 0;
                ...
                if (serviceIsFree) {
                    if (paymentTxid) {
                        return (0, commandResult_1.commandFailed)('order_payment_unverified', 'Free service execution must not include a payment txid.');
                    }
                }
                else if (!paymentTxid) {
                    return (0, commandResult_1.commandFailed)('order_payment_unverified', 'Paid service execution must include a payment txid.');
                }
                else if (!paymentChain) {
                    return (0, commandResult_1.commandFailed)('order_payment_unverified', 'Paid service execution must include payment chain metadata.');
                }
                else if (!orderAmount || orderAmount !== serviceAmount || !orderCurrency || orderCurrency !== serviceCurrency) {
                    return (0, commandResult_1.commandFailed)('order_payment_unverified', 'Service execution payment terms do not match the local service.');
                }
                const paymentVerification = await serviceOrderPaymentVerifier({
                    adapters,
                    paymentTxid: paymentTxid || null,
                ...
                if (!paymentVerification.verified && paymentVerification.outcome === 'error') {
                    ...
                    return (0, commandResult_1.commandFailed)('order_payment_verification_unavailable', 'Service execution payment could not be verified because the chain API is unavailable; the execution was not started and can be retried.', {
                        data: {
                            handled: true,
                            transient: true,
                            retryable: true,
                ...
                if (!paymentVerification.verified) {
                    return (0, commandResult_1.commandFailed)('order_payment_unverified', 'Service execution payment could not be verified on chain.');
                }
        ...
        chat: {
            ...
            private: async (rawInput) => {
                ...
                return (0, commandResult_1.commandSuccess)({
                    ...
                    a2aSessionId,
                    traceId: trace.traceId,
                    localUiUrl: (0, conversationUrl_1.buildConversationLocalUiUrl)(input.getDaemonRecord(), state.identity?.globalMetaId, request.to)
                        ?? buildDaemonLocalUiUrl(input.getDaemonRecord(), '/ui/trace', a2aSessionId
            ...
            privateChatMessages: async (msgInput) => {
                ...
                const messages = await actor.privateChatStateStore.getRecentMessages(normalizeText(msgInput.conversationId), typeof msgInput.limit === 'number' && Number.isFinite(msgInput.limit)
        ...
        file: {
            ...
            uploadLarge: async (rawInput) => {
                ...
                try {
                    ...
                    const data = readLargeFileUploadFailureData(error);
                    if (code) {
                        return (0, commandResult_1.commandFailed)(code, message, data ? { data } : undefined);
                    }
                    return (0, commandResult_1.commandFailed)('file_upload_failed', message, data ? { data } : undefined);
                }
            },
        },
        trace: {
            ...
            listSessions: async (request = {}) => {
                let profiles = await (0, identityProfiles_1.listIdentityProfiles)(normalizedSystemHomeDir).catch(() => []);
                const requestedFrom = normalizeText(request.from);
                if (requestedFrom) {
                    const selectedProfile = await (0, metabotProfileManager_1.getMetabotProfile)(normalizedSystemHomeDir, requestedFrom);
                    if (!selectedProfile) {
                        return (0, commandResult_1.commandFailed)('profile_not_found', `MetaBot profile not found: ${requestedFrom}`);
                    }
                    const selectedHomeDir = node_path_1.default.resolve(selectedProfile.homeDir);
                    profiles = profiles.filter((profile) => node_path_1.default.resolve(profile.homeDir) === selectedHomeDir);
                }
                else if (request.all === false) {
                    const activeHomeDir = node_path_1.default.resolve(input.homeDir);
                    profiles = profiles.filter((profile) => node_path_1.default.resolve(profile.homeDir) === activeHomeDir);
                }
                const results = [];
                ...
                const buildSellerRefundTraceSession = (order, profile) => {
                ...
                await Promise.all(profiles.map(async (profile) => {
                    ...
                    try {
                        const runtimeState = await (0, runtimeStateStore_1.createRuntimeStateStore)(profile.homeDir).readState();
                        const refundOrderIds = new Set((0, providerOperations_1.buildSellerReceivedRefundItems)(runtimeState).map((item) => normalizeText(item.orderId)));
                ...
                for (const profile of profiles) {
                    ...
                    try {
                        ...
                        try {
                            ...
                            trace = runtimeState.traces.find((entry) => (normalizeText(entry.traceId) === normalizedSessionId
                                || normalizeText(entry.a2a?.sessionId) === normalizedSessionId
                                || normalizeText(entry.session?.id) === normalizedSessionId
                                || (session && normalizeText(entry.traceId) === normalizeText(session.traceId)))) ?? null;
                        }
                        catch {
                            trace = null;
                        }
                        ...
                        return (0, commandResult_1.commandSuccess)({
                            ...
                            orderTxids: [],
                            paymentTxid: null,
                            localUiUrl: (0, conversationUrl_1.buildConversationLocalUiUrl)(input.getDaemonRecord(), profile.globalMetaId, peerGlobalMetaId)
                                ?? buildDaemonLocalUiUrl(input.getDaemonRecord(), '/ui/trace', {
                                    traceId,
                                    sessionId: normalizedSessionId,
                                }),
        ...
        bot: {
            ...
            listProfiles: async () => {
                ...
                const twinSlug = await (0, twinRole_1.resolveCurrentTwinSlug)(normalizedSystemHomeDir).catch(() => null);
                const fallbackHomeDir = node_path_1.default.resolve(input.homeDir);
                const profilesWithSetup = await Promise.all(profiles.map(async (profile) => {
                    ...
                    return {
                        ...
                        isActive: twinSlug !== null
                            ...
                            : node_path_1.default.resolve(profile.homeDir) === fallbackHomeDir,
            ...
            createProfile: async (body) => {
                ...
                let systemDefaultProviderRoles = [];
                try {
                ...
                try {
                    ...
                    if (pendingProviders.length > 0 && !createTimeLlmDiscoveryDisabled(input.discoverLlmRuntimes)) {
                        ...
                        startBackgroundLlmDiscoverySweep(profileHomeDir, pendingProviders, input.discoverLlmRuntimes ?? llmRuntimeDiscovery_1.discoverLlmRuntimes);
                    }
                    return (0, commandResult_1.commandSuccess)({
                        profile,
                        identity,
                        ...
                        twinInvariant: await (0, twinRole_1.applyTwinInvariant)(normalizedSystemHomeDir, createInput.botType === 'twin' ? { preferredTwinSlug: profile.slug } : {}).catch(() => null),
                    });
                }
                catch (error) {
            ...
            updateProfile: async (body) => {
                ...
                try {
                    ...
                    if (update.name !== undefined && update.name !== current.name) {
                        const profiles = await (0, identityProfiles_1.listIdentityProfiles)(normalizedSystemHomeDir).catch(() => []);
                        const duplicate = (0, profileNameResolution_1.resolveProfileNameConflict)(update.name, profiles.filter((profile) => profile.slug !== current.slug));
                        ...
                        if (duplicate.status === 'ambiguous') {
                            return (0, commandResult_1.commandFailed)('name_taken', duplicate.message);
                        }
                    }
                    ...
                    const primaryProviderChanged = (update.primaryProvider !== undefined
                        && update.primaryProvider !== (current.primaryProvider ?? null));
                    if (effectiveAllowChatSkills.length > 0
                        && (update.allowChatSkills !== undefined || primaryProviderChanged)) {
                        const runtimeStore = (0, llmRuntimeStore_1.createLlmRuntimeStore)(current.homeDir);
                        const bindingStore = (0, llmBindingStore_1.createLlmBindingStore)(current.homeDir);
                        ...
                        const validation = await (0, chatSkillPolicy_1.validateAllowChatSkills)({
                ...
                const updateInfoTargets = (0, metabotProfileManager_1.buildMetabotInfoPublishTargets)(chainProfile, calculateMetabotUpdateInfoFields(current, update));
                ...
                if (chatSkillsOnlyUpdate) {
                    try {
                        ...
                        let chatSkillChainWrites = [];
                        let chainSync = { ok: true };
                        if (updateInfoTargets.length > 0 && current.globalMetaId) {
                            try {
                                const profileSigner = createSignerForProfileHome(current.homeDir);
                                chatSkillChainWrites = await (0, metabotProfileManager_1.syncMetabotInfoToChain)(profileSigner, profile, updateInfoTargets, {
                                    deferPublishStateWrite: true,
                                });
                                await (0, metabotProfileManager_1.recordMetabotInfoPublishResults)(profile, updateInfoTargets, chatSkillChainWrites);
                            }
                            catch (error) {
                                chainSync = {
                                    ok: false,
                ...
                if (updateInfoTargets.length > 0) {
                    try {
                        const profileSigner = createSignerForProfileHome(current.homeDir);
                        chainWrites = await (0, metabotProfileManager_1.syncMetabotInfoToChain)(profileSigner, chainProfile, updateInfoTargets, {
                            deferPublishStateWrite: true,
                        });
                    }
                    catch (error) {
            ...
            uploadHomepageFile: async (body) => {
                ...
                try {
                    ...
                    return (0, commandResult_1.commandFailed)('homepage_upload_failed', message, data ? { data } : undefined);
                }
            },
            ...
            deleteProfile: async ({ slug }) => {
                try {
                    ...
                    const twinInvariant = await (0, twinRole_1.applyTwinInvariant)(normalizedSystemHomeDir, {})
                        .catch(() => null);
                    return (0, commandResult_1.commandSuccess)({
                        deleted: true,
            ...
            testRuntime: async (request) => {
                ...
                if (!runtime) {
                    return (0, commandResult_1.commandFailed)('runtime_not_found', `LLM runtime was not found: ${runtimeId || '<missing>'}`);
                }
                const testedRuntime = await (input.testLlmRuntimeReadiness ?? llmRuntimeDiscovery_1.testLlmRuntimeReadiness)(runtime, {
                    env: process.env,
                });
        ...
        llm: {
            ...
            upsertBindings: async ({ from, slug, bindings }) => {
                ...
                const normalizedBindings = bindings
                    ...
                    return (0, llmTypes_1.normalizeLlmBinding)({
                        ...b,
                        id,
    ...
    handlers.resumePendingCallerReplyContinuations = resumePendingCallerReplyContinuations;
