...
const artifactCache_1 = require("../metaapp/artifactCache");
const pinId_1 = require("../metaapp/pinId");
...
function normalizeIndexFile(value) {
    ...
    if (!text || text.includes('\\') || node_path_1.default.posix.isAbsolute(text) || node_path_1.default.win32.isAbsolute(text)) {
        return null;
    }
...
function isZipContent(contentType, contentReference) {
...
function firstContentReference(...values) {
...
function downloadUrlsForReference(reference) {
...
async function defaultMakeTempDir() {
    return node_fs_1.promises.mkdtemp(node_path_1.default.join(node_os_1.default.tmpdir(), 'metabot-browser-metaapp-'));
}
async function defaultCreatePreviewSession(input) {
    return {
        localPreviewUrl: `file://${node_path_1.default.join(input.artifactDir, input.indexFile)}`,
    };
}
async function findArtifactRootForIndexFile(rootDir, indexFile) {
    const directPath = node_path_1.default.join(rootDir, indexFile);
    try {
        const stat = await node_fs_1.promises.stat(directPath);
        if (stat.isFile()) {
            return rootDir;
        }
    }
    ...
    try {
        const stat = await node_fs_1.promises.stat(node_path_1.default.join(nestedRoot, indexFile));
...
async function resolveMetaAppPinToRecord(input) {
    ...
    if (!pinId) {
        return (0, commandResult_1.commandFailed)('invalid_browser_uri', 'metaapp:// requires a 64-hex pinId ending in i0.');
    }
    const fetchImpl = input.fetch ?? globalThis.fetch;
    if (!fetchImpl) {
        return (0, commandResult_1.commandFailed)('browser_resolve_failed', 'A fetch implementation is required to resolve MetaApp pins.');
    }
    ...
    try {
        ...
        if (!response.ok || !response.json) {
            return (0, commandResult_1.commandFailed)(response.status === 404 ? 'browser_resource_not_found' : 'browser_resolve_failed', response.status === 404 ? 'MetaApp pin was not found.' : `MetaApp pin lookup failed with HTTP ${response.status}.`);
        }
        const pinRecord = unwrapPinRecord(await response.json());
        if (!pinRecord) {
            return (0, commandResult_1.commandFailed)('browser_resolve_failed', 'MetaApp pin lookup returned an invalid response.');
        }
        const protocolPath = normalizeText(pinRecord.path);
        if (protocolPath !== METAAPP_PROTOCOL_PATH) {
            return (0, commandResult_1.commandFailed)('browser_protocol_mismatch', `Pin path ${protocolPath || '(empty)'} is not a MetaApp protocol pin.`, { data: { expectedPath: METAAPP_PROTOCOL_PATH, actualPath: protocolPath || null } });
        }
        const protocol = parseJsonObject(pinRecord.contentSummary ?? pinRecord.contentBody ?? pinRecord.content);
        if (!protocol) {
            return (0, commandResult_1.commandFailed)('browser_resolve_failed', 'MetaApp protocol contentSummary is not valid JSON.');
        }
        ...
        if (!indexFile) {
            return (0, commandResult_1.commandFailed)('browser_resolve_failed', 'MetaApp indexFile must be a relative path inside the app package.');
        }
        if (!contentReference) {
            return (0, commandResult_1.commandFailed)('browser_resolve_failed', 'MetaApp protocol is missing a content reference.');
        }
        let localPreviewUrl;
        let rendererContentType = sourceContentType;
        if (isZipContent(sourceContentType, contentReference)) {
            const modifyHistory = (0, artifactCache_1.normalizeMetaAppModifyHistory)(pinRecord.modify_history ?? pinRecord.modifyHistory);
            ...
            if (cachedArtifact) {
                ...
                if (!archive) {
                    return (0, commandResult_1.commandFailed)('browser_resolve_failed', 'MetaApp ZIP content could not be downloaded.');
                }
                if (input.artifactCache) {
                    ...
                    await (0, zipArchive_1.extractMetaAppZipArchive)({ archive, outDir: tempDir });
        ...
        return (0, commandResult_1.commandSuccess)(buildRecord({
            pinId,
            manApiBaseUrl,
            pinRecord,
            protocol,
        ...
        return (0, commandResult_1.commandFailed)('browser_resolve_failed', error instanceof Error ? error.message : String(error));
    }
}
