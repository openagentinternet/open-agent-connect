...
exports.validateLoomPayload = validateLoomPayload;
...
function requireObject(object, key, errors, prefix = '') {
...
function requireNonEmptyString(object, key, errors, prefix = '') {
    const path = fieldPath(prefix, key);
    if (!hasOwn(object, key)) {
        addError(errors, path, 'required', `${path} is required.`);
        return;
...
function requirePinId(object, key, errors, prefix = '') {
    const path = fieldPath(prefix, key);
...
function validateOptionalTimestamp(object, key, errors, prefix = '') {
...
function requireEnum(object, key, values, errors, prefix = '') {
    const path = fieldPath(prefix, key);
    requireNonEmptyString(object, key, errors, prefix);
    const value = object[key];
...
function validateMetafileUriArray(object, key, errors, prefix = '') {
    const path = fieldPath(prefix, key);
    if (!hasOwn(object, key) || object[key] === undefined || object[key] === null) {
        return;
    }
...
function validateStringArray(object, key, errors, prefix = '') {
    const path = fieldPath(prefix, key);
    if (!hasOwn(object, key) || object[key] === undefined || object[key] === null) {
        return;
    }
...
function validateClaim(payload, errors) {
...
function validateStatus(payload, errors) {
    requirePinId(payload, 'taskPinId', errors);
...
function validateDelivery(payload, errors) {
    requirePinId(payload, 'taskPinId', errors);
    requirePinId(payload, 'claimPinId', errors);
...
function validateAcceptance(payload, errors) {
    requirePinId(payload, 'taskPinId', errors);
...
function validateClaimReject(payload, errors) {
    requirePinId(payload, 'taskPinId', errors);
    requirePinId(payload, 'claimPinId', errors);
    requireNonEmptyString(payload, 'reason', errors);
    validateMetafileUriArray(payload, 'attachments', errors);
}
function validateLoomPayload(protocol, payload) {
