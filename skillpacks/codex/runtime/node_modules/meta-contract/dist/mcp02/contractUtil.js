...
class ContractUtil {
    static init(config) {
        if (config) {
            this.transferCheckCodeHashArray = config.transferCheckCodeHashArray.map((v) => new mvc_scrypt_1.Bytes(v));
            this.unlockContractCodeHashArray = config.unlockContractCodeHashArray.map((v) => new mvc_scrypt_1.Bytes(v));
            ...
            this.transferCheckCodeHashArray = getTokenTransferCheckCodeHashArray().map((v) => new mvc_scrypt_1.Bytes(v));
            this.unlockContractCodeHashArray = getTokenUnlockContractCheckCodeHashArray().map((v) => new mvc_scrypt_1.Bytes(v));
        ...
        let genesisContract = tokenGenesis_1.TokenGenesisFactory.getDummyInstance();
