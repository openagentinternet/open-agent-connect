...
exports.resolvePinUriToResource = resolvePinUriToResource;
const types_js_1 = require("./types.js");
const pinUri_js_1 = require("./pinUri.js");
...
function parsePayload(content, contentType) {
    if (record(content) || Array.isArray(content))
        return content;
...
function contentBodyText(pinRecord, contentType) {
...
function meaningfulPayloadSource(pinRecord, contentType) {
    ...
    if (record(pinRecord.contentSummary) || text(pinRecord.contentSummary))
        return pinRecord.contentSummary;
    return '';
}
...
function pinTxid(pinRecord) {
    return text(pinRecord.genesisTransaction ?? pinRecord.genesis_transaction ?? pinRecord.txid);
}
async function resolvePinUriToResource(input) {
    const parsed = (0, pinUri_js_1.parsePinUri)(input.uri);
    const fetchImpl = fetchImplementation(input.fetch);
    if (!fetchImpl) {
        return (0, types_js_1.browserCommandFailed)('browser_resolve_failed', 'A fetch implementation is required to resolve pin resources.');
    }
    ...
    if (!response.ok || !response.json) {
        return (0, types_js_1.browserCommandFailed)(response.status === 404 ? 'browser_resource_not_found' : 'browser_resolve_failed', response.status === 404 ? 'Pin was not found.' : `Pin lookup failed with HTTP ${response.status}.`);
    }
    const pinRecord = unwrapPinRecord(await response.json());
    if (!pinRecord) {
        return (0, types_js_1.browserCommandFailed)('browser_resource_not_found', 'Pin was not found.');
    }
    ...
    const title = text(payloadRecord.title
        ?? payloadRecord.name
        ?? payloadRecord.displayName
        ?? payloadRecord.serviceName) || `Pin ${shortId(resolvedPinId)}`;
...
//# sourceMappingURL=pinResolver.js.map