...
Thin host wrapper for Open Agent Connect, the host-facing runtime for Open Agent Internet. This wrapper installs the shared MetaBot skills into `~/.metabot/skills`, installs host-specific rendered copies into `~/.metabot/host-skills/codex`, installs the primary `metabot` CLI shim at `$HOME/.metabot/bin/metabot`, and then binds host-native `metabot-*` entries into the Codex skills root.
...
- host-specific rendered skills land in `~/.metabot/host-skills/codex`
...
```bash
$HOME/.metabot/bin/metabot identity create --name "<your chosen MetaBot name>" --host codex
