...
exports.renderPinInspectorHtml = renderPinInspectorHtml;
...
function versionValue(resource) {
    return record(data(resource).version);
}
function jsonBlock(value, className = 'browser-protocol-json') {
...
function shortReference(value) {
    ...
    if (separator > 0) {
        ...
        if (body.length > 28)
            return `${prefix}${body.slice(0, 10)}...${body.slice(-10)}`;
    }
...
function currentPinIds(resource) {
    const pin = pinValue(resource);
    const recordValue = rawPinRecord(resource);
...
function convertGlobalMetaIdBits(values, fromBits, toBits) {
    ...
    if (bitCount >= fromBits)
        return null;
    if (((accumulator << (toBits - bitCount)) & maxValue) !== 0)
        return null;
    return result;
}
function hasValidGlobalMetaIdPayloadLength(versionChar, bytes) {
    ...
    if (versionChar === 'r' || versionChar === 'y')
...
function normalizeGlobalMetaId(value) {
    ...
    if (!/^id[qpzryt]1/u.test(normalized))
        return '';
...
function shortGlobalMetaId(value) {
    const normalized = text(value);
    return normalized.length > 17 ? `${normalized.slice(0, 8)}...${normalized.slice(-6)}` : normalized;
}
function collectGlobalMetaIds(value, output, seen = new WeakSet()) {
...
function creatorGlobalMetaId(resource) {
    const owner = record(resource.owner);
    const pin = pinValue(resource);
    const recordValue = rawPinRecord(resource);
    return normalizeGlobalMetaId(owner.globalMetaId
        ?? owner.metaid
...
function relatedEntityIds(resource) {
...
function entityInitials(value) {
    const normalized = text(value) || 'Bot';
    return normalized.slice(0, 2).toUpperCase();
}
function renderEntityCard(globalMetaId, name = '') {
...
function renderEntityRow(role, ids, fallbackNames = new Map()) {
    if (!ids.length)
        return '';
...
function renderRelatedEntities(resource) {
...
function linkHtml(value, label, extraAttributes = '', className = '') {
...
function sectionBlock(title, bodyHtml, intro = '') {
...
const markdownRenderer = new marked_1.Marked();
function renderMarkdownReference(href, content, title) {
...
function renderMarkdownMetafileImage(href, alt, title) {
    const normalizedHref = text(href);
    const titleAttribute = title ? ` title="${escapeHtml(title)}"` : '';
...
function parseJsonPayload(payload, rawPayload) {
...
function contentTypeValue(resource) {
...
function markdownPayloadContent(jsonPayload) {
...
function payloadIntro(resource) {
...
function renderJsonRows(value) {
    return Object.entries(value).map(([key, item]) => (`<div class="browser-pin-json-row${typeof item === 'string' && isLongJsonString(item) ? ' browser-pin-json-row-longtext' : ''}"><div class="browser-pin-json-key">${escapeHtml(key)}</div><div class="${jsonValueClass(item)}">${renderJsonValue(item)}</div></div>`)).join('');
}
...
function renderRawPayload(resource) {
    const rawPayload = data(resource).rawPayload;
    const payload = payloadValue(resource);
    const jsonPayload = parseJsonPayload(payload, rawPayload);
...
function isExtensionlessMetafileImageReference(uri) {
    const value = text(uri);
    if (!value.startsWith('metafile://'))
        return false;
    let path = value.slice('metafile://'.length).split(/[?#]/, 1)[0];
    if (path.startsWith('image/'))
        path = path.slice('image/'.length);
    return /^[0-9a-f]{64}i[0-9]+$/i.test(path);
}
function isImageReference(uri, sourceKey = '') {
    if (IMAGE_MEDIA_KEYS.has(sourceKey))
        return true;
    return /\.(png|jpe?g|gif|webp|avif|svg)(?:[?#].*)?$/iu.test(uri)
        || isExtensionlessMetafileImageReference(uri);
}
function isVideoReference(uri, sourceKey = '') {
    if (VIDEO_MEDIA_KEYS.has(sourceKey))
        return true;
    return VIDEO_EXTENSION_PATTERN.test(uri) || VIDEO_PATH_PATTERN.test(uri);
}
function isAudioReference(uri, sourceKey = '') {
    if (AUDIO_MEDIA_KEYS.has(sourceKey))
        return true;
    return AUDIO_EXTENSION_PATTERN.test(uri) || AUDIO_PATH_PATTERN.test(uri);
}
function mediaKind(uri, sourceKey = '') {
    if (isImageReference(uri, sourceKey))
        return 'image';
    if (isVideoReference(uri, sourceKey))
        return 'video';
    if (isAudioReference(uri, sourceKey))
        return 'audio';
    return 'file';
}
function isMediaReferenceUri(uri) {
    return EXTERNAL_URL_PATTERN.test(uri) || uri.startsWith('metafile://');
}
function mediaReference(value, sourceKey = '') {
    if (typeof value === 'string') {
        const uri = text(value);
        if (!uri || !isMediaReferenceUri(uri))
            return null;
...
function collectBarePinIdUris(value, output, ignoredPinIds) {
    ...
    while ((match = PIN_ID_PATTERN.exec(value)) !== null) {
        ...
        if (value.slice(Math.max(0, pinIdStart - 3), pinIdStart) === '://')
            continue;
        if (ignoredPinIds.has(pinId))
            continue;
        output.add(`pin://${pinId}`);
    }
}
function collectBrowserUris(value, output, seen = new WeakSet(), includeExternal = false, ignoredPinIds = new Set()) {
    if (typeof value === 'string') {
...
function extractMetafilePinId(reference) {
    ...
    if (!value || !value.startsWith('metafile://'))
        return '';
    let path = value.slice('metafile://'.length).split(/[?#]/, 1)[0];
...
function isDownloadableMediaReference(reference) {
    const uri = text(reference);
    if (!uri)
        return false;
    if (EXTERNAL_URL_PATTERN.test(uri))
        return true;
    if (!uri.startsWith('metafile://'))
        return false;
...
function collectMediaItems(payload) {
    ...
    collectBrowserUris(payload, discoveredUris, new WeakSet(), true);
...
function mediaSlotPlaceholder(item) {
    if (item.kind === 'video') {
...
function renderMediaItems(resource) {
    const contentType = contentTypeValue(resource);
    const payload = payloadValue(resource);
    const rawPayload = data(resource).rawPayload;
    const jsonPayload = parseJsonPayload(payload, rawPayload);
...
function renderRelatedLinks(resource) {
    const contentType = contentTypeValue(resource);
    const payload = payloadValue(resource);
    const rawPayload = data(resource).rawPayload;
    const jsonPayload = parseJsonPayload(payload, rawPayload);
    const source = contentType.includes('json')
        ...
        : payload;
    const uris = new Set();
    collectBrowserUris(source, uris, new WeakSet(), false, currentPinIds(resource));
    if (!uris.size) {
...
function versionLabel(version, pin, recordValue) {
    ...
    if (selector === 'history-index')
        return `history version ${text(version.historyIndex) || '0'}`;
    return text(pin.version) ? `version ${text(pin.version)}` : selector;
}
function renderPinInspectorHtml(resource, headingOverride = '') {
    const pin = pinValue(resource);
    const version = versionValue(resource);
    const recordValue = rawPinRecord(resource);
...
//# sourceMappingURL=pinInspector.js.map