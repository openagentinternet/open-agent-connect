...
const utxoConverter_js_1 = require('./utxoConverter.cjs');
/**
...
class Psbt {
    ...
    constructor(opts = {}, data = new bip174_1.Psbt(new PsbtTransaction())) {
        ...
        for (let i = 0; i < this.__CACHE.__TX.inputs.length; i++) {
            ...
            const opcatUtxo = (0, utxoConverter_js_1.parseInputOutputFromPsbt)(this.data.inputs[i]);
            input.output = new opcat_1.Transaction.Output({
                satoshis: Number(opcatUtxo.value),
                script: new opcat_1.Script(Buffer.from(opcatUtxo.script)),
     ...
     */
    getInputOutput(inputIndex) {
        return (0, utxoConverter_js_1.parseInputOutputFromPsbt)(this.data.inputs[inputIndex]);
    }
    combine(...those) {
    ...
    addInput(inputData) {
        ...
        this.data.addUnknownKeyValToInput(inputIndex, (0, utxoConverter_js_1.encode)(inputData.opcatUtxo));
    ...
    addOutput(outputData) {
        ...
        if (typeof address === 'string') {
            const script = tools.fromHex(opcat_1.Script.fromAddress(address).toHex());
    ...
    finalizeAllInputs() {
        (0, bip174_1.checkForInput)(this.data.inputs, 0); // making sure we have at least one
        range(this.data.inputs.length).forEach(idx => this.finalizeInput(idx));
        return this;
    }
    finalizeInput(inputIndex, finalScriptsFunc) {
        const input = (0, bip174_1.checkForInput)(this.data.inputs, inputIndex);
    ...
    outputHasHDKey(outputIndex, root) {
        const output = (0, bip174_1.checkForOutput)(this.data.outputs, outputIndex);
        const derivationIsMine = bip32DerivationIsMine(root);
    ...
    _signInput(inputIndex, keyPair, sighashTypes = [opcat_1.crypto.Signature.SIGHASH_ALL]) {
        const { hash, sighashType } = getHashAndSighashType(this.data.inputs, inputIndex, this.__CACHE, sighashTypes);
        const partialSig = [
            {
    ...
    _setOpcatKV() {
        ...
        const findGlobalOpcatKV = (this.data.globalMap.unknownKeyVals || []).find(kv => (utxoConverter_js_1.OPCAT_KEY_BUF).equals(Buffer.from(kv.key)));
        if (!findGlobalOpcatKV) {
            this.data.addUnknownKeyValToGlobal({
                key: utxoConverter_js_1.OPCAT_KEY_BUF,
                value: Buffer.from([1])
            });
        }
        // add output data to output
        this.data.outputs.forEach((output, outputIndex) => {
            const opcatKV = (output.unknownKeyVals || []).find(kv => (utxoConverter_js_1.OPCAT_KEY_BUF).equals(Buffer.from(kv.key)));
            if (opcatKV) {
...
class PsbtTransaction {
    ...
    addOutput(output) {
        ...
        this.tx.addOutput(new opcat_1.Transaction.Output({
            script: opcat_1.Script.fromBuffer(Buffer.from(output.script)),
...
function getHashAndSighashType(inputs, inputIndex, cache, sighashTypes) {
    const input = (0, bip174_1.checkForInput)(inputs, inputIndex);
    const { hash, sighashType } = getHashForSig(inputIndex, input, cache, sighashTypes);
...
function getHashForSig(inputIndex, input, cache, sighashTypes) {
    ...
    const sighashType = input.sighashType || (sighashTypes && sighashTypes.length > 0
        ? sighashTypes[0]
        : opcat_1.crypto.Signature.SIGHASH_ALL);
        ...
        case 'multisig':
            {
                ...
                unlockingScript = opcat_1.Script.fromASM(`${sigs.map(sig => tools.toHex(sig)).join(' ')}`);
                break;
            }
        case 'pubkey':
            {
...
function getScriptFromInput(_inputIndex, input, _cache) {
    ...
    res.script = (0, utxoConverter_js_1.parseInputOutputFromPsbt)(input).script;
    return res;
}
...
function pubkeyInOutput(pubkey, outputIndex, cache) {
    ...
    const { meaningfulScript } = getMeaningfulScript(script.toBuffer());
    return (0, psbtutils_js_1.pubkeyInScript)(pubkey, meaningfulScript);
}
...
function txFromBitcoinBuffer(buf) {
    const inputFromBufferReader = (reader) => {
        ...
        const input = new opcat_1.Transaction.Input({
            prevTxId,
            outputIndex,
            script,
            sequenceNumber
    ...
    const outputFromBufferReader = (reader) => {
        const satoshis = reader.readUInt64LEBN();
        const script = reader.readVarLengthBuffer();
        const output = new opcat_1.Transaction.Output({
            satoshis,
            script,
            data: Buffer.from([])
        });
    ...
    const tx = new opcat_1.Transaction();
    const reader = new opcat_1.encoding.BufferReader(buf);
...
function txToBitcoinBuffer(tx) {
    const writer = new opcat_1.encoding.BufferWriter();
    ...
    tx.outputs.forEach(output => {
        writer.write(Buffer.from((0, byteString_js_1.intToByteString)(output.satoshis, 8n), 'hex'));
