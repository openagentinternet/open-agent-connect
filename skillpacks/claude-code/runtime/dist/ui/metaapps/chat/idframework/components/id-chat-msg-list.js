...
class IdChatMsgList extends HTMLElement {
  ...
  get messages() {
    return this._viewerMessages.slice();
  }

  set messages(value) {
    this._viewerMessages = Array.isArray(value) ? value.slice() : [];
    this._scheduleViewerUpdate();
  }

  get loading() {
    return this._viewerLoading;
  }

  set loading(value) {
    this._viewerLoading = value === true || value === 'true' || value === '';
    this._scheduleViewerUpdate();
  }

  get error() {
    return this._viewerError;
  }

  set error(value) {
    this._viewerError = String(value || '');
    this._scheduleViewerUpdate();
  }
  ...
  attributeChangedCallback(name, oldValue, newValue) {
    ...
    if (name === 'messages') {
      this._viewerMessages = this._parseMessagesAttribute(newValue);
    }
    if (name === 'loading') {
      this._viewerLoading = newValue !== null && newValue !== 'false';
    }
    if (name === 'error') {
      this._viewerError = String(newValue || '');
    }
    this._scheduleViewerUpdate();
  }
  ...
  _isStandaloneViewer() {
    const mode = String(this.getAttribute('viewer-mode') || '').trim().toLowerCase();
    return mode === 'standalone' || mode === 'viewer';
  }

  _parseMessagesAttribute(value) {
  ...
  _standaloneSnapshot() {
    const rawMode = String(this.getAttribute('mode') || 'private').trim().toLowerCase();
    const conversationType = rawMode === 'group' || rawMode === 'public' || rawMode === '1' ? '1' : '2';
    const peerGlobalMetaId = String(this.getAttribute('peer-global-metaid') || '').trim();
    const conversationId = String(this.getAttribute('conversation-id') || peerGlobalMetaId || '').trim();
    const attrMessages = this.hasAttribute('messages')
      ? this._parseMessagesAttribute(this.getAttribute('messages'))
      : null;
    return {
      ready: true,
      ...
      messages: attrMessages || this._viewerMessages.slice(),
      isLoading: this._viewerLoading || this.hasAttribute('loading'),
      error: this._viewerError || String(this.getAttribute('error') || ''),
      selfGlobalMetaId: String(this.getAttribute('self-global-metaid') || '').trim(),
      selfMetaId: String(this.getAttribute('self-metaid') || '').trim(),
    };
  }

  _snapshot() {
    if (this._isStandaloneViewer()) return this._standaloneSnapshot();
  ...
  _debugTrace(eventName, payload) {
    ...
    if (window.__IDCHAT_SCROLL_DEBUG_CONSOLE__ && typeof console !== 'undefined' && typeof console.debug === 'function') {
      console.debug('[id-chat-msg-list]', trace);
    }
  }
  ...
  async _loadOlderMessages() {
    const snapshot = this._snapshot();
    if (snapshot.viewerMode === 'standalone') return false;
    const conversationId = String(snapshot.currentConversation || '').trim();
    if (!conversationId) return false;
    if (this._noMoreOlderByConversation[conversationId]) return false;
    ...
    const idf = (typeof window !== 'undefined' && window.IDFramework) ? window.IDFramework : null;
    if (!idf || typeof idf.dispatch !== 'function') return false;
  ...
  render(forcedSnapshot) {
    ...
    this.shadowRoot.innerHTML = `
      ...
      <div class="messages-container">
        ${this._isLoadingOlder ? `<div class="history-loader">${this._escapeHtml(this._t('chat.msg.loadingOlder', 'Loading older messages...'))}</div>` : ''}
        ${showInlineSyncing ? `<div class="history-loader">${this._escapeHtml(this._t('chat.msg.syncing', 'Syncing messages...'))}</div>` : ''}
        ${showLoadingState ? `
          <div class="state">${this._escapeHtml(this._t('chat.msg.loadingMessages', 'Loading messages...'))}</div>
        ` : snapshot.error ? `
          <div class="state error">${this._escapeHtml(this._t('chat.msg.errorPrefix', 'Error:'))} ${this._escapeHtml(snapshot.error)}</div>
        ` : !snapshot.currentConversation ? `
          <div class="state">${this._escapeHtml(this._t('chat.msg.selectConversation', 'Select a conversation to view messages'))}</div>
        ` : !snapshot.messages.length ? `
          <div class="state">${this._escapeHtml(this._t('chat.msg.noMessagesYet', 'No messages yet'))}</div>
        ` : `
          <div class="list">
      ...
      <button
        ...
        type="button"
        aria-label="${this._escapeHtml(this._t('chat.msg.scrollToBottom', 'Scroll to latest messages'))}"
...
if (!customElements.get('id-chat-msg-list')) {
  customElements.define('id-chat-msg-list', IdChatMsgList);
}

export default IdChatMsgList;
