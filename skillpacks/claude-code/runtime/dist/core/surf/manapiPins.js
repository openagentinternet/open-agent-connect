...
exports.DEFAULT_MANAPI_BASE_URL = void 0;
exports.listPinsByPath = listPinsByPath;
exports.DEFAULT_MANAPI_BASE_URL = 'https://manapi.metaid.io';
...
function resolveOptions(options) {
    ...
    return {
        baseUrl: (options?.baseUrl || exports.DEFAULT_MANAPI_BASE_URL).replace(/\/+$/, ''),
        fetchImpl,
        timeoutMs: options?.timeoutMs ?? DEFAULT_TIMEOUT_MS,
    };
}
/** GET /api/pin/path/list?path=…&size=… — newest first. */
async function listPinsByPath(params, options) {
    const { baseUrl, fetchImpl, timeoutMs } = resolveOptions(options);
    const path = params.path.trim();
    if (!path)
        throw new Error('path is required to list pins by path.');
