...
exports.isSellerOrderActiveForChatSuppression = isSellerOrderActiveForChatSuppression;
exports.isBuyerTraceOrderActiveForChatSuppression = isBuyerTraceOrderActiveForChatSuppression;
exports.createHasActiveOrderWithPeer = createHasActiveOrderWithPeer;
...
// buyer-role only).
const SELLER_ORDER_STATES_ACTIVE_FOR_CHAT_SUPPRESSION = new Set([
    'received',
    'acknowledged',
    'in_progress',
    'rating_pending',
...
const CALLER_SESSION_STATES_WAITING_FOR_CHAT_SUPPRESSION = new Set([
    'requesting_remote',
    'remote_received',
    'remote_executing',
]);
...
function isSellerOrderActiveForChatSuppression(order) {
...
function isBuyerTraceOrderActiveForChatSuppression(order) {
...
function createHasActiveOrderWithPeer(input) {
    return async (peerGlobalMetaId) => {
        ...
        if (runtimeState?.sellerOrders.some((order) => (normalizeText(order.buyerGlobalMetaId) === peer
            && isSellerOrderActiveForChatSuppression(order)))) {
            return true;
        }
        const traces = runtimeState?.traces ?? [];
        const findTrace = (traceId) => (traces.find((entry) => entry.traceId === traceId) ?? null);
