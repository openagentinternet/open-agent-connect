...
exports.tryQaQuestionBrowserPath = tryQaQuestionBrowserPath;
...
function qaQuestionAppBrowserPath(pinId) {
...
async function tryQaQuestionBrowserPath(uri) {
    ...
    try {
        await (0, recall_1.qaQuestionDetail)(pinId, {
...
function createDefaultCliDependencies(context) {
    ...
    async function runQandaSearch(input) {
        try {
            const q = normalizeEnvText(typeof input.query === 'string' ? input.query : undefined);
            if (!q)
                return (0, commandResult_1.commandFailed)('missing_query', '--query is required.');
            ...
            return (0, commandResult_1.commandSuccess)({ ...page, formatted });
        }
        catch (error) {
            const message = error instanceof Error ? error.message : String(error);
            return (0, commandResult_1.commandFailed)('qanda_search_failed', message);
        }
    }
    async function runQandaLatest(input) {
        try {
            ...
            const message = error instanceof Error ? error.message : String(error);
            return (0, commandResult_1.commandFailed)('qanda_latest_failed', message);
        }
    }
    async function runQandaDetail(input) {
        try {
            const pinId = normalizeEnvText(typeof input.pinId === 'string' ? input.pinId : undefined);
            if (!pinId)
                return (0, commandResult_1.commandFailed)('missing_pin', '--pin is required.');
            const detail = await (0, recall_1.qaQuestionDetail)(pinId, metawebServiceOptions());
            return (0, commandResult_1.commandSuccess)({
                ...detail,
                formatted: (0, format_2.formatQaQuestionDetail)({ question: detail.question, answers: detail.answers }),
            });
        }
        catch (error) {
            if (error instanceof recall_1.QaRecallNotFoundError) {
                return (0, commandResult_1.commandFailed)('question_not_found', error.message);
            }
            const message = error instanceof Error ? error.message : String(error);
            return (0, commandResult_1.commandFailed)('qanda_detail_failed', message);
        }
    }
    async function runQandaAnswers(input) {
        try {
            const pinId = normalizeEnvText(typeof input.pinId === 'string' ? input.pinId : undefined);
            if (!pinId)
                return (0, commandResult_1.commandFailed)('missing_pin', '--pin is required.');
            const page = await (0, recall_1.qaQuestionAnswers)({
                pinId,
                ...(normalizeEnvText(typeof input.publisher === 'string' ? input.publisher : undefined) ? { publisher: normalizeEnvText(input.publisher) } : {}),
                ...(readPositiveField(input.size) ? { size: readPositiveField(input.size) } : {}),
                ...(normalizeEnvText(typeof input.cursor === 'string' ? input.cursor : undefined) ? { cursor: normalizeEnvText(input.cursor) } : {}),
            }, metawebServiceOptions());
            const bullets = (0, format_2.formatQaAnswerBullets)(page.items);
            return (0, commandResult_1.commandSuccess)({
                ...page,
            ...
            const message = error instanceof Error ? error.message : String(error);
            return (0, commandResult_1.commandFailed)('qanda_answers_failed', message);
        }
    }
    ...
    function mapSkillInstallError(error) {
        ...
        const message = error instanceof Error ? error.message : String(error);
        return (0, commandResult_1.commandFailed)('skill_install_failed', message);
    }
    /** Best-effort rebind of every auto-bind host root after the skills root changed. */
    async function rebindHostSkillRoots() {
        try {
            return await (0, hostSkillBinding_1.bindPlatformSkills)({
    ...
    async function unbindSkillFromHostRoots(name) {
        const systemHomeDir = normalizeSystemHomeDir(context.env, context.cwd);
        const results = [];
        for (const root of (0, platformRegistry_1.getInstallSkillRoots)()) {
            if (root.autoBind === 'manual')
                continue;
            let hostSkillRoot;
            try {
                hostSkillRoot = (0, platformRegistry_1.resolvePlatformSkillRootPath)(root, systemHomeDir, context.env);
            }
            catch {
                continue;
            }
            const linkPath = node_path_1.default.join(hostSkillRoot, name);
            try {
                const stat = await node_fs_1.default.promises.lstat(linkPath);
                if (stat.isSymbolicLink()) {
                    await node_fs_1.default.promises.unlink(linkPath);
    ...
    async function runSkillsInstall(input) {
        try {
            ...
            if (!pinId && !directUri) {
                return (0, commandResult_1.commandFailed)('invalid_argument', 'Pass --pin <skill pinId> or --uri <package metafile:// or https URI>.');
            }
            let descriptor = null;
            let creator;
            if (pinId) {
                const pin = await (0, pinRead_1.readMetawebPin)(pinId, metawebServiceOptions());
                descriptor = (0, skillInstall_1.extractSkillPinDescriptor)(pin);
                if (!descriptor && !directUri) {
                    return (0, commandResult_1.commandFailed)('invalid_skill_pin', `Pin ${pinId} (protocol ${pin.protocol || 'unknown'}) does not carry a skill package payload (needs \`name\` + \`skill-file\`). Read it with \`metabot metaweb read --pin ${pinId}\` first.`);
                }
                creator = {
            ...
            if (!confirm) {
                ...
                return (0, commandResult_1.commandFailed)('confirm_required', plan);
            }
            const installed = await (0, skillInstall_1.installSkillFromReference)({
            ...
            const boundRoots = Array.isArray(rebind?.boundRoots)
                ? rebind.boundRoots
            ...
            return (0, commandResult_1.commandSuccess)({ skill: installed, rebind, formatted });
        }
        catch (error) {
    ...
    async function runSkillsList() {
        try {
            const skills = await (0, skillInstall_1.listInstalledSkills)(sharedSkillsRoot());
            ...
            return (0, commandResult_1.commandSuccess)({ skills, formatted });
        }
        catch (error) {
            return mapSkillInstallError(error);
        }
    ...
    async function runSkillsRead(input) {
        try {
            const name = normalizeEnvText(typeof input.name === 'string' ? input.name : undefined);
            if (!name)
                return (0, commandResult_1.commandFailed)('invalid_argument', '--name is required.');
            const skill = await (0, skillInstall_1.readInstalledSkill)({ skillsRoot: sharedSkillsRoot(), name });
            return (0, commandResult_1.commandSuccess)({ ...skill, formatted: skill.skillMd });
        }
        catch (error) {
            return mapSkillInstallError(error);
        }
    ...
    async function runSkillsUninstall(input) {
        try {
            const name = normalizeEnvText(typeof input.name === 'string' ? input.name : undefined);
            if (!name)
                return (0, commandResult_1.commandFailed)('invalid_argument', '--name is required.');
            if (input.confirm !== true) {
                return (0, commandResult_1.commandFailed)('confirm_required', `Uninstalling skill "${name}" removes its directory under ${sharedSkillsRoot()} and its host symlinks. Re-run with --confirm to uninstall.`);
            }
            const removed = await (0, skillInstall_1.uninstallInstalledSkill)({ skillsRoot: sharedSkillsRoot(), name });
     ...
     */
    async function runSkillsPublish(input) {
        const skillDir = normalizeEnvText(typeof input.skillDir === 'string' ? input.skillDir : undefined);
        if (!skillDir) {
            return (0, commandResult_1.commandFailed)('invalid_argument', 'Pass --dir <skill directory> (its root, or its single subdirectory, must carry SKILL.md).');
        }
        return requestJsonForSelectedActor('POST', '/api/skills/publish', typeof input.from === 'string' ? input.from : undefined, {
            ...input,
    ...
    return {
        ...
        qanda: {
            question: async (input) => requestJsonForSelectedActor('POST', '/api/qanda/question', typeof input.from === 'string' ? input.from : undefined, input),
            answer: async (input) => requestJsonForSelectedActor('POST', '/api/qanda/answer', typeof input.from === 'string' ? input.from : undefined, input),
            like: async (input) => requestJsonForSelectedActor('POST', '/api/qanda/like', typeof input.from === 'string' ? input.from : undefined, input),
