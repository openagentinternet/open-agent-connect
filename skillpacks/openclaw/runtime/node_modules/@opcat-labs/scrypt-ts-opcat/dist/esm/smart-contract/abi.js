import { arrayTypeAndSize, buildScriptHex, isArrayType, resolveType, subArrayType, subscript, } from './abiutils.js';
import { ABIEntityType, } from './types/artifact.js';
import { serializeArgToHex } from './serializer.js';
import { SymbolType, isScryptType } from './types/abi.js';
 ...
 * @ignore
 */
export function buildTypeResolverFromArtifact(artifact) {
 ...
 * @ignore
 */
export function hasGeneric(entity) {
 ...
 * @ignore
 */
export function buildTypeResolver(_contract, _alias, structs, library, contracts = [], _statics = []) {
    const resolvedTypes = {};
...
export class ABICoder {
     ...
     */
    encodeConstructorCall(ctorArgs) {
        const constructorABI = this.artifact.abi.filter((entity) => entity.type === ABIEntityType.CONSTRUCTOR)[0];
        ...
        flatteredArgs.forEach((arg) => {
            ...
            hexTemplateArgs.set(`<${arg.name}>`, serializeArgToHex(arg.value));
        });
    ...
    isPubFunction(method) {
        return (this.artifact.abi.findIndex((entity) => entity.name === method && entity.type === ABIEntityType.FUNCTION) > -1);
    }
    /**
     ...
     */
    encodePubFunctionCall(method, args) {
        const resolver = buildTypeResolverFromArtifact(this.artifact);
        const methodABI = this.artifact.abi.find((entity) => entity.name === method && entity.type === ABIEntityType.FUNCTION);
        if (!methodABI) {
            throw new Error(`no public function named '${method}' found in contract '${this.artifact.contract}'`);
        }
        ...
        let unlockingScriptHex = flatteredArgs.map((a) => serializeArgToHex(a.value)).join('');
     ...
     */
    decodePubFunctionCall(unlockingScript) {
        let unlockingScriptHex;
        ...
        const args = cParams.map(param => deserializeArgfromHex(resolver, Object.assign(param, {
     ...
     */
    flattenStruct(arg, type, ignoreValue = false) {
        const resolver = buildTypeResolverFromArtifact(this.artifact);
        return flatternStruct(arg, {
            name: 'flattened_struct',
            type,
        }, resolver, { state: false, ignoreValue });
    }
    /**
