...
exports.buildBrowserClientScript = buildBrowserClientScript;
const menuModel_js_1 = require("./menuModel.js");
function jsonScript(value) {
    return JSON.stringify(value).replace(/</g, '\\u003c');
}
function buildBrowserClientScript(input) {
    return `(() => {
  ...
  const browserMenuSections = ${jsonScript(menuModel_js_1.BROWSER_MENU_SECTIONS)};
  const browserSettingsTabs = ${jsonScript(menuModel_js_1.BROWSER_SETTINGS_TABS)};
  const browserBaseUrlFields = ${jsonScript(menuModel_js_1.BROWSER_BASE_URL_FIELDS)};
  const browserBotHomepageTemplates = ${jsonScript(menuModel_js_1.BROWSER_BOT_HOMEPAGE_TEMPLATES)};
...
//# sourceMappingURL=browserClientScript.js.map