...
function baseWorkflowState(overrides = {}) {
  return {
    version: 1,
    taskPinId,
    claimPinId,
...
test('resolves workflow paths under profile runtime loom root', async () => {
  ...
  assert.match(normalizePathForAssert(paths.stagingRoot), /\.runtime\/loom\/staging$/);
  assert.match(normalizePathForAssert(paths.workspacesRoot), /\.runtime\/loom\/workspaces$/);
  assert.match(normalizePathForAssert(paths.logsRoot), /\.runtime\/loom\/logs$/);
  assert.match(normalizePathForAssert(paths.taskLogsRoot), /\.runtime\/loom\/logs\/a+.*$/);
  assert.match(normalizePathForAssert(paths.workflowPath), /\.runtime\/loom\/workflows\/a+.*\/b+.*\.json$/);
  assert.match(normalizePathForAssert(paths.workspaceRepoPath), /\.runtime\/loom\/workspaces\/a+.*\/b+.*\/repo$/);
  assert.match(normalizePathForAssert(paths.stagingRepoPath), /\.runtime\/loom\/staging\/a+.*\/run-1\/repo$/);
});
...
test('resolves pending claim preview paths with run fallback', async () => {
  ...
  assert.match(normalizePathForAssert(paths.stagingRepoPath), /\.runtime\/loom\/staging\/a+.*\/run\/repo$/);
});
...
test('workflow store normalizes missing statuses on read', async () => {
  ...
  await writeFile(workflowPath, `${JSON.stringify(baseWorkflowState({ statuses: undefined }))}\n`, 'utf8');
  const read = await store.read(taskPinId, claimPinId);
...
test('workflow store returns null for malformed state', async () => {
  ...
  const workflowPath = store.resolve(taskPinId, claimPinId).workflowPath;
  await mkdir(path.dirname(workflowPath), { recursive: true });
  await writeFile(workflowPath, `${JSON.stringify({ version: 1, taskPinId, claimPinId })}\n`, 'utf8');
...
test('workflow store returns null when statuses is not an array', async () => {
  ...
  const workflowPath = store.resolve(taskPinId, claimPinId).workflowPath;
  await mkdir(path.dirname(workflowPath), { recursive: true });
  await writeFile(workflowPath, `${JSON.stringify(baseWorkflowState({
    statuses: 'started',
  }))}\n`, 'utf8');
  assert.equal(await store.read(taskPinId, claimPinId), null);
});
...
test('workflow store write leaves no temp file after success', async () => {
  ...
  const workflowPath = store.resolve(taskPinId, claimPinId).workflowPath;
  await store.write(baseWorkflowState());
  const entries = await readdir(path.dirname(workflowPath));
