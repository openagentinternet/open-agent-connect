...
test('local cross-host demo harness creates two local runtimes and produces a discoverable remote call plan', async () => {
  ...
  let result;
  try {
    result = await runLocalCrossHostDemo({
      callerHost: 'codex',
    ...
    assert.match(result.artifacts.traceJsonPath, /\/\.runtime\/exports\/traces\/.*\.json$/);
    assert.match(result.remoteExecution.traceJsonPath, /\/\.runtime\/exports\/traces\/.*\.json$/);
  } finally {
