...
test('GET /api/network/services forwards query filters to network.listServices', async (t) => {
  ...
  const response = await fetch(`${server.baseUrl}/api/network/services?online=true&query=tarot`);
  const payload = await response.json();

  assert.equal(response.status, 200);
  assert.deepEqual(server.calls.networkServices, [{ online: true, query: 'tarot' }]);
  assert.equal(payload.ok, true);
