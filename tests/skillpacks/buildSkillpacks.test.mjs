...
test('repository tracked shared and host-wrapper skillpack artifacts stay in sync with a fresh build', async (t) => {
  ...
  assert.deepEqual(
    missingTrackedSharedSkills.filter((skillName) => skillName !== 'metabot-upload-largefile'),
    [],
    'unexpected tracked shared skillpack artifacts are missing',
  );
  if (missingTrackedSharedSkills.includes('metabot-upload-largefile')) {
    t.skip('metabot-upload-largefile tracked skillpack artifacts are generated by Task 8');
    return;
  }
