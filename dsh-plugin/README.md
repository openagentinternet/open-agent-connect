...
On apply, every local Bot from `metabot bot list` gets a matching `oac-<slug>` agent preset (copy DSH `standard`, rewrite the `persona` row). Delete removes that preset. Non-`oac-*` presets are left alone — with one legacy exception: the bare shared `oac` preset installed by pre-per-Bot releases is removed on reconcile (a dangling `agent-presets.default: oac` heals to `standard`). An `oac-*` agent's per-agent tool set installs idempotently when the agent is created, and sessions that select the preset afterwards (the create-then-select flow) get the same set installed on the `agent-preset/selected` session event — no conversation starts without its tools.
