/**
 * skill_tool — native install/list/read for on-chain metabot-skill packages,
 * OAC port of the IDBots skillAgentTools subset the MetaWeb learning loop
 * needs. All actions go through the OAC CLI: the DSH agent sandbox cannot
 * write ~/.metabot, and the CLI owns download, safe extraction, the install
 * registry, and host re-binding. Installs ask DSH `ctx.approval` before the
 * CLI's --confirm (same posture as bot_browser_publish_app).
 */
  ...
  return [
    {
      ...
      description:
        ...
        + 'under the shared skills root, and rebinds installed hosts. list_installed_skills lists what is '
        + 'installed; read_skill loads one skill\'s SKILL.md so you can follow it immediately.',
      parameters: {
        type: 'object',
        properties: {
          action: {
            type: 'string',
            enum: ['install_skill', 'list_installed_skills', 'read_skill'],
          ...
          name: { type: 'string', description: 'read_skill: installed skill name (from list_installed_skills).' },
      ...
      execute: async (args: Record<string, unknown>, exec: HostToolExec) => {
        ...
        try {
          ...
          return `Unknown action "${action}". Use install_skill, list_installed_skills, or read_skill.`
        } catch (error) {
