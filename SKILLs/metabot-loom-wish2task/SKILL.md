...
Do not treat `{{METABOT_CLI}} loom draft-task --wish ...` as the whole workflow. The skill must own clarification, quality control, final JSON assembly, explicit confirmation, publish, and handoff. Use the CLI draft helper only as an optional assistant, never as a substitute for asking missing blocking questions.
...
```bash
{{METABOT_CLI}} loom validate --protocol task --payload-file <task-payload.json>
```

Preview the publish without writing chain data:

```bash
{{METABOT_CLI}} loom post-task --from <bot-slug> --payload-file <task-payload.json> --dry-run
```

When the human explicitly chooses MVC, BTC, DOGE, or OPCAT for the Loom record, pass the matching write-chain flag:

```bash
{{METABOT_CLI}} loom post-task --from <bot-slug> --payload-file <task-payload.json> --chain mvc --dry-run
{{METABOT_CLI}} loom post-task --from <bot-slug> --payload-file <task-payload.json> --chain btc --dry-run
{{METABOT_CLI}} loom post-task --from <bot-slug> --payload-file <task-payload.json> --chain doge --dry-run
{{METABOT_CLI}} loom post-task --from <bot-slug> --payload-file <task-payload.json> --chain opcat --dry-run
```
...
```bash
{{METABOT_CLI}} loom post-task --from <bot-slug> --payload-file <task-payload.json>
```
...
```bash
{{METABOT_CLI}} ui open --page loom --from <bot-slug>
```
