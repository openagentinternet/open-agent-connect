...
```bash
{{METABOT_CLI}} memory transcript append --from <bot-slug> --payload-file turn-user.json
{{METABOT_CLI}} memory transcript append --from <bot-slug> --payload-file turn-assistant.json
{{METABOT_CLI}} memory extract --from <bot-slug> --payload-file extract.json
```
