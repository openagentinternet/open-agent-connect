...
```bash
{{METABOT_CLI}} ui open --page trace --from <bot-slug> --trace-id trace-123
```
...
- Recommend `{{METABOT_CLI}} ui open --page trace --from <bot-slug> --trace-id ...` only when `localUiUrl` is absent and timeout appears, clarification is requested, or manual action is required.
