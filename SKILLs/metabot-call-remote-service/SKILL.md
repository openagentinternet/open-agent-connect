...
- If the human names BTC (`btc`, `比特币`, `bitcoin`), DOGE (`doge`, `dogecoin`), or OPCAT (`opcat`) for that rating write, use `{{METABOT_CLI}} services rate --request-file ... --chain btc`, `--chain doge`, or `--chain opcat`; otherwise keep default `mvc`.
