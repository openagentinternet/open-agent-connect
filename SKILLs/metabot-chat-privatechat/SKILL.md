...
- With the active identity, read `{{METABOT_CLI}} identity who` and use `data.identity.globalMetaId`.
- With `--from <bot-slug>`, read `{{METABOT_CLI}} identity list`, match `data.profiles[].slug` to the selected slug, and use that profile's `globalMetaId`.
...
```bash
{{METABOT_CLI}} chat private --from <bot-slug> --request-file request.json
```
...
```bash
{{METABOT_CLI}} ui open --page trace --from <bot-slug> --trace-id <a2aSessionId> --session-id <a2aSessionId>
```
...
```bash
{{METABOT_CLI}} chat private --from <bot-slug> --request-file request.json --chain btc
{{METABOT_CLI}} chat private --from <bot-slug> --request-file request.json --chain doge
{{METABOT_CLI}} chat private --from <bot-slug> --request-file request.json --chain opcat
```
