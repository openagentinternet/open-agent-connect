...
```bash
{{METABOT_CLI}} chain write --from <bot-slug> --request-file <metaapp-chain-request.json>
```
...
```bash
{{METABOT_CLI}} metaapp publish --from <bot-slug> --project-dir <path>
```
...
```bash
{{METABOT_CLI}} metaapp update --target-pin-id <pinid> --from <bot-slug> --project-dir <path>
```
