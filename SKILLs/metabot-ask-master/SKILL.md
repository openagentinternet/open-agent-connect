...
```bash
{{METABOT_CLI}} master ask --from <bot-slug> --request-file master-request.json
{{METABOT_CLI}} master ask --from <bot-slug> --trace-id trace-master-123 --confirm
```

Inspect Ask Master trace details when needed:

```bash
{{METABOT_CLI}} master trace --from <bot-slug> --id trace-master-123
```
...
- Always call `{{METABOT_CLI}} master ask --from <bot-slug> --request-file ...` first so the runtime can build the preview for the selected Bot actor.
...
- After approval, reuse the returned trace id and run `{{METABOT_CLI}} master ask --from <bot-slug> --trace-id ... --confirm`.
...
- When a structured response is returned, surface these fields directly:
  ...
  - `confidence`
  - `followUpQuestion`
- If the confirmed ask returns a trace id without a finished structured response, or the human asks for more evidence, follow with `{{METABOT_CLI}} master trace --from <bot-slug> --id ...`.
...
- In one caller terminal or fresh host session, verify both public lanes:
  - `manual`: preview with `{{METABOT_CLI}} master ask --from <bot-slug> --request-file ...`, then confirm with `{{METABOT_CLI}} master ask --from <bot-slug> --trace-id ... --confirm`.
...
- After a real run, inspect the result with `{{METABOT_CLI}} master trace --from <bot-slug> --id ...`.
