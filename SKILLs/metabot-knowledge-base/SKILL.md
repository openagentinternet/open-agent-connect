...
```bash
{{METABOT_CLI}} knowledge-base add-document --from <bot-slug> \
...
```bash
{{METABOT_CLI}} knowledge-base query --from <bot-slug> --text "wallet recovery steps"
```

Example — assign a long-horizon learning topic for the coming nights:

```bash
{{METABOT_CLI}} knowledge-base study enqueue --from <bot-slug> \
