...
```bash
{{METABOT_CLI}} media describe <image|video|audio> --path <file-or-url> [--question <text>]
```
...
```bash
{{METABOT_CLI}} media describe image --path /abs/path/screenshot.png --question "Extract the error code"
{{METABOT_CLI}} media describe audio --path https://example.com/voicemail.mp3
```
