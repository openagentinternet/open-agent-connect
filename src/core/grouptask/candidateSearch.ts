...
import { GROUP_TASK_SEAT_ROLES } from './staffing';
...
export function scoreOpenTeamCandidate(
  item: { name?: string; bio?: string; chatSkills?: string[] },
  tokens: string[],
): number {
...
export interface SeatImpressionSnapshot {
  ...
  collaborationFacts?: Array<{ title: string; outcome: string; seatRole?: string }>;
}
...
export interface GroupTaskCandidateSearchDeps {
  ...
  ): Promise<SeatImpressionSnapshot | null>;
...
function normalizeGmid(value: string | null | undefined): string | null {
...
export function evaluateImpressionForSeat(
  snapshot: SeatImpressionSnapshot | null,
  roleHint: GroupTaskSeatRole | null,
...
async function toLocalCandidate(
  worker: GroupTaskCandidateSearchLocalWorker,
  tokens: string[],
  roleHint: GroupTaskSeatRole | null,
  snapshot: SeatImpressionSnapshot | null,
): Promise<GroupTaskSeatCandidate | null> {
  if (!worker.enabled || worker.botType === 'twin') return null;
...
export async function searchGroupTaskSeatCandidates(
  ...
  try {
    const searchRemote = deps.searchRemote ?? ((remoteInput: SearchGroupTaskRemoteInput) =>
      searchRemoteBotsForSeat(remoteInput, deps.botSearch));
    ...
    const excludeGlobalMetaIds = [
      ...new Set([
        ...(observer ? [observer] : []),
        ...workers.map((worker) => normalizeGmid(worker.globalMetaId)).filter((id): id is string => Boolean(id)),
      ].map((id) => String(id))),
    ];
    ...
    for (const remote of found) {
      remotes.push(toRemoteCandidate(
        remote,
        tokens,
        roleHint,
        await readSnapshot(deps, observer, remote.globalMetaId),
      ));
    }
