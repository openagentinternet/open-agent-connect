...
export function isStaffingProposalExpired(createdAt: number, nowMs: number): boolean {
...
export function localSeatSlugs(plan: GroupTaskStaffingPlan): string[] {
  return [...new Set(
    plan.seats
      .filter((seat) => seat.source === 'local' && seat.candidateSlug)
      .map((seat) => seat.candidateSlug as string),
  )];
}
