...
import { DEFAULT_METASO_P2P_BASE_URL } from '../network/metasoInfrastructure';
...
export function extractDeliverablePinId(uri: string | null | undefined): string | null {
  const match = /([0-9a-f]{64}i\d+)/i.exec(uri ?? '');
  return match ? match[1]!.toLowerCase() : null;
}

/** metaso-p2p pin existence check; envelope code 0 = found, 40400 = missing. */
export function createMetasoPinVerifier(options: {
  baseUrl?: string;
  fetchImpl?: typeof fetch;
  timeoutMs?: number;
} = {}): PinVerifier {
  const baseUrl = (options.baseUrl ?? DEFAULT_METASO_P2P_BASE_URL).replace(/\/+$/, '');
  ...
  return async (pinId) => {
    ...
    try {
      ...
      if (code === 40400) return 'not_found';
      return 'error';
    } catch {
      return 'error';
    } finally {
...
export async function verifyTaskDeliverables(
  ...
  for (const row of rows) {
    ...
    if (verdict === 'found') {
      report.confirmed += 1;
      await store.updateDeliverableVerification(
        row.id,
        JSON.stringify({ sources: [{ source: 'metaso', result: 'found' }], checkedAt: now() }),
        'confirmed',
        row.status === 'pending' ? 'delivered' : undefined,
      );
    } else {
      report.stillUnconfirmed += 1;
      options.log?.(
        `[GroupTaskEngine] Deliverable ${row.id} of task ${taskId} still unverified (${verdict})`,
      );
    }
  }
  return report;
