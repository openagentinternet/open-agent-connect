...
import { decryptGroupContent } from '../appSession/groupChat';
import { fetchGroupHistoryPage, type GroupTaskTransportOptions } from './transport';
 ...
 */
export async function syncGroupMessages(input: SyncGroupMessagesInput): Promise<SyncGroupMessagesResult> {
  ...
  const result: SyncGroupMessagesResult = { fetched: 0, inserted: 0 };
  ...
  while (result.fetched < maxRows) {
    ...
    const page = await fetchGroupHistoryPage(input.groupId, startIndex, size, input.transport);
    if (page.length === 0) break;

    const messages: GroupTaskMessage[] = page.map((item) => {
      const gmid = normalizeGmid(item.globalMetaId);
      const content = item.encryption === 'aes'
        ? decryptGroupContent(item.content, input.groupId)
        : item.content;
      return {
        index: item.index ?? -1,
    ...
    if (page.length < size) break; // short page: no more history
