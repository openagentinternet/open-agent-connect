...
export const OMNI_READ_MAX_RESULT_CHARS = 20_000;
const DEFAULT_TIMEOUT_MS = 10_000;

export const OMNI_READ_ACTIONS = [
  'user_info',
  'search_users',
  'buzz_newest',
  'buzz_recommended',
...
export type OmniReadResult = { text: string; isError: boolean };
...
function formatData(data: unknown): string {
  ...
  if (text.length > OMNI_READ_MAX_RESULT_CHARS) {
    return `${truncateUtf16Units(text, OMNI_READ_MAX_RESULT_CHARS)}\n...(truncated, narrow the query with cursor/size)`;
  }
  return text;
}
...
async function fetchJson(url: string, deps: OmniReadDeps): Promise<unknown> {
...
async function fetchText(url: string, deps: OmniReadDeps): Promise<string> {
  const fetchImpl = deps.fetchImpl ?? globalThis.fetch;
  const controller = new AbortController();
  const timer = setTimeout(() => controller.abort(), deps.timeoutMs ?? DEFAULT_TIMEOUT_MS);
  try {
...
export async function runOmniReadAction(rawArgs: unknown, deps: OmniReadDeps = {}): Promise<OmniReadResult> {
  const args = (rawArgs && typeof rawArgs === 'object' ? rawArgs : {}) as Record<string, unknown>;
  const action = asString(args.action) as OmniReadAction;
  const fail = (message: string): OmniReadResult => ({ text: message, isError: true });
  const ok = (text: string): OmniReadResult => ({ text, isError: false });
  try {
    switch (action) {
      case 'user_info': {
      ...
      case 'search_users': {
        ...
        if (!keyword) return fail('omni_read search_users requires keyword.');
      ...
      case 'buzz_search': {
        ...
        if (!key) return fail('omni_read buzz_search requires key.');
      ...
      case 'buzz_info': {
        ...
        if (!pinId) return fail('omni_read buzz_info requires pinId.');
      ...
      case 'notifications': {
        ...
        if (!address) return fail('omni_read notifications requires address.');
      ...
      case 'following': {
        ...
        if (!metaid) return fail(`omni_read ${action} requires metaid.`);
      ...
      case 'pin': {
        const pinId = asString(args.pinId);
        if (!pinId) return fail('omni_read pin requires pinId.');
      ...
      case 'pin_version': {
        const pinId = asString(args.pinId);
        if (!pinId) return fail('omni_read pin_version requires pinId.');
      ...
      case 'pins_by_path': {
        ...
        if (!path) return fail('omni_read pins_by_path requires path (e.g. /protocols/simplebuzz).');
        const size = toInt(args.size);
      ...
      case 'pins_by_metaid': {
        const metaid = asString(args.metaid);
        if (!metaid) return fail('omni_read pins_by_metaid requires metaid.');
      ...
      case 'pins_by_address': {
        const address = asString(args.address);
        if (!address) return fail('omni_read pins_by_address requires address.');
        const path = asString(args.path);
        if (!path) return fail('omni_read pins_by_address requires path.');
      ...
      case 'pin_content': {
        const pinId = asString(args.pinId);
        if (!pinId) return fail('omni_read pin_content requires pinId.');
        const url = buildUrl(MANAPI_BASE, `content/${encodeURIComponent(pinId)}`);
        const body = await fetchText(url, deps);
        const text = body.length > OMNI_READ_MAX_RESULT_CHARS
          ? `${truncateUtf16Units(body, OMNI_READ_MAX_RESULT_CHARS)}\n...(truncated, narrow the query with cursor/size)`
          : body;
        return ok(text);
      }
      ...
      case 'file_info': {
        const pinId = asString(args.pinId);
        if (!pinId) return fail('omni_read file_info requires pinId.');
      ...
      case 'file_latest': {
        ...
        if (!firstPinId) return fail('omni_read file_latest requires firstPinId.');
      ...
      case 'files_by_creator': {
        const address = asString(args.address);
        if (!address) return fail('omni_read files_by_creator requires address.');
      ...
      case 'files_by_metaid': {
        const metaid = asString(args.metaid);
        if (!metaid) return fail('omni_read files_by_metaid requires metaid.');
      ...
      case 'files_by_extension': {
        ...
        if (!extension) return fail('omni_read files_by_extension requires extension (e.g. .jpg).');
        const metaid = asString(args.metaid);
        const path = metaid
...
export const OMNI_READ_TOOL_DESCRIPTION = [
