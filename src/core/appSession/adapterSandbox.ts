...
export function sandboxSha256Hex(value: string): string {
  return createHash('sha256').update(value).digest('hex');
}

function normalizeAdapterHashHex(value: string): string {
 ...
 */
export function createAdapterSandbox(options: AdapterSandboxOptions): SandboxedAdapter {
  ...
  if (computedHash !== requestedHash) {
    throw new Error(
      `adapterHash mismatch: expected ${requestedHash}, computed ${computedHash}`,
    );
  }
  ...
  function spawnWorker(): Worker {
    const spawned = new Worker(WORKER_BOOTSTRAP, {
      eval: true,
      workerData: { code },
      resourceLimits: {
    ...
    spawned.on('message', (payload: { id?: unknown; ok?: unknown; result?: unknown; error?: unknown }) => {
      ...
      if (payload?.ok === true) {
        ...
        (error as { code?: string }).code = typeof errorRecord.code === 'string'
          ? errorRecord.code
          : 'adapter_error';
        entry.reject(error);
      }
  ...
  return {
    call<T = unknown>(method: string, args: unknown[] = []): Promise<T> {
      ...
      return new Promise<T>((resolve, reject) => {
        const entryResolve = (value: unknown) => resolve(value as T);
