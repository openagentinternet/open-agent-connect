...
export const AGENT_GAME_PROTOCOL = 'agent-game/1';
export const APP_SESSION_TYPE = 'agent-game';

/** Default protocol paths a session grant may write (docs/09 5.1). */
export const DEFAULT_AGENT_GAME_PROTOCOL_PATHS = [
...
export function createAppSessionError(
  code: AppSessionErrorCode,
  message: string,
  details?: Record<string, unknown>,
): AppSessionError {
...
export function isAppSessionError(value: unknown): value is AppSessionError {
  return Boolean(
    value
    && typeof value === 'object'
