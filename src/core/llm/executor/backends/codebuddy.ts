...
function buildCodeBuddyArgs(request: LlmExecutionRequest): string[] {
  const args = ['-p', request.prompt, '--output-format', 'stream-json', '--dangerously-skip-permissions'];
      ...
      const processResult = await runJsonLineProcess({
        ...
        onJson(message) {
          ...
          if (type === 'system/error' || (type === 'system' && subtype === 'error') || type === 'error') {
            ...
            protocolError = String(message.message ?? message.error ?? message.detail ?? 'codebuddy error');
          ...
          if (type === 'result' || (type === 'system' && subtype === 'result')) {
            ...
            if (message.is_error === true || message.status === 'error' || message.subtype === 'error') {
              protocolStatus = 'failed';
              protocolError = candidateOutput || String(message.error ?? message.detail ?? 'codebuddy result failed');
            }
          }
        },
      });
