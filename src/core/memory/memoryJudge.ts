...
import { isQuestionLikeMemoryText, type MemoryGuardLevel } from './memoryExtractor';
...
export const MEMORY_JUDGE_SYSTEM_PROMPT = [
...
export function parseLlmJudgePayload(text: string): { accepted: boolean; confidence: number; reason: string } | null {
...
async function judgeWithLlm(
  ...
  if (!input.judgeComplete) return null;
  ...
  try {
    const text = await input.judgeComplete(MEMORY_JUDGE_SYSTEM_PROMPT, userPrompt);
