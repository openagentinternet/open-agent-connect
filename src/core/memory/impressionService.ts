...
export async function buildDreamImpressionSubjects(input: {
  ...
  const selected = [...accumulators.values()]
    ...
    .slice(0, limit);
  const result: DreamImpressionPromptSubject[] = [];
...
export async function applyDreamImpressionUpdates(input: {
  impressionStore: ImpressionStore;
  observerGlobalMetaId: string;
  dreamDate: string;
  dreamVersion: number;
  ...
  const result: DreamImpressionApplyResult = { accepted: 0, created: 0, rejected: 0, rebuilt: 0 };
  ...
  for (const subject of rebuiltSubjects) {
    try {
      if (await input.impressionStore.rebuildSnapshot(observer, subject)) result.rebuilt += 1;
    } catch {
