...
export function procedureTitleFingerprint(title: string): string {
...
function latinTokensOf(text: string): string[] {
 ...
 */
export function scoreProceduresForQuery(
  procedures: ProcedureRecord[],
  query: string,
): Array<{ procedure: ProcedureRecord; score: number }> {
  ...
  const scored: Array<{ procedure: ProcedureRecord; score: number }> = [];
  for (const procedure of procedures) {
    if (procedure.status !== 'active') continue;
    ...
    for (const token of latin) {
      total += 1;
      if (titleText.includes(token)) coverage += 1;
    }
    for (const bg of bigrams) {
      total += 1;
      if (titleText.includes(bg)) coverage += 1;
    }
    for (const ch of singles) {
      total += 1;
      if (procedure.title.toLowerCase().includes(ch)) coverage += 1;
    }
...
export interface ProcedureStore {
  upsertProcedure(input: UpsertProcedureInput): Promise<{ procedure: ProcedureRecord; created: boolean }>;
  listProcedures(options?: { status?: ProcedureRecord['status'] }): Promise<ProcedureRecord[]>;
...
function normalizeProcedure(value: unknown): ProcedureRecord | null {
  if (!value || typeof value !== 'object') return null;
  const row = value as Record<string, unknown>;
  if (typeof row.title !== 'string' || !row.title.trim()) return null;
  const toList = (input: unknown, cap = 40): string[] => (
    Array.isArray(input)
...
export function createProcedureStore(paths: MetabotPaths): ProcedureStore {
  const filePath = path.join(paths.workspaceRoot, 'memory', 'procedures.json');
  ...
  async function readFile(): Promise<ProcedureFile> {
    try {
      const raw = await fs.readFile(filePath, 'utf8');
      const parsed = JSON.parse(raw) as Partial<ProcedureFile>;
      return {
        seq: Number.isInteger(parsed?.seq) ? (parsed!.seq as number) : 0,
        procedures: Array.isArray(parsed?.procedures)
          ? (parsed!.procedures as unknown[]).map(normalizeProcedure).filter((row): row is ProcedureRecord => row !== null)
          : [],
      };
    } catch {
  ...
  async function writeFile(state: ProcedureFile): Promise<void> {
  ...
  return {
    upsertProcedure: (input) => enqueue(async () => {
      ...
      if (!title) throw new ProcedureStoreError('title_required', 'Procedure title is required.');
      const steps = (input.steps ?? []).map((step) => String(step ?? '').trim().slice(0, 500)).filter(Boolean);
      if (!steps.length) throw new ProcedureStoreError('steps_required', 'At least one step is required.');
      ...
      if (existing) {
        ...
        if (input.confidence != null) existing.confidence = Math.max(0, Math.min(1, input.confidence));
