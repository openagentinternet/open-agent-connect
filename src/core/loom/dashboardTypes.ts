...
export type LoomDashboardStateTone =
  | 'neutral'
  | 'info'
  | 'progress'
  | 'review'
...
export type LoomDashboardColumnId =
  | 'open'
  | 'claimed'
  | 'working'
  | 'review'
...
export type LoomDashboardTimelineEventKind =
  | 'task'
  | 'claim'
  | 'status'
  | 'delivery'
...
export type LoomDashboardBotRole =
  | 'requester'
  | 'developer'
  | 'reviewer'
  | 'unknown';
...
export interface LoomDashboardTaskCard {
  ...
  developer?: LoomDashboardBotIdentity;
  bounty?: { amount?: string; currency?: string };
  repo?: { repoUri?: string; baseBranch?: string };
  tags: string[];
  createdAt: number;
  updatedAt: number;
