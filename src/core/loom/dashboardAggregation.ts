...
import {
  ...
  type LoomWorkflowTaskInvalidBuckets,
  type LoomWorkflowTaskStateBuckets,
} from './workflowState';
...
const BOARD_COLUMNS: Array<{ id: LoomDashboardColumnId; title: string; states: LoomDerivedTaskState[] }> = [
...
const TIMELINE_PRIORITY: Record<LoomDashboardTimelineEventKind, number> = {
  task: 0,
  claim: 1,
  status: 2,
  delivery: 3,
...
function columnIdForState(state: LoomDerivedTaskState): LoomDashboardColumnId {
  return BOARD_COLUMNS.find((column) => column.states.includes(state))?.id ?? 'open';
}

function toneForState(state: LoomDerivedTaskState): LoomDashboardStateTone {
  switch (state) {
    case 'open':
      return 'neutral';
    case 'claimed':
...
function activeBuckets(valid: LoomWorkflowTaskStateBuckets): LoomWorkflowTaskStateBuckets {
...
function deriveDashboardState(valid: LoomWorkflowTaskStateBuckets): LoomDerivedTaskState {
...
function localEvidenceFromWorkflow(state: LoomWorkflowState): LoomDashboardLocalEvidence {
  ...
  return {
    ...
    processLogUris: state.statuses
      ...
      .filter((logUri): logUri is string => Boolean(logUri)),
    commits: state.statuses.flatMap((status) => status.commits),
  };
}

function warningFromInvalid(
  taskPinId: string,
  invalid: LoomWorkflowTaskInvalidBuckets[keyof LoomWorkflowTaskInvalidBuckets][number],
): LoomDashboardWarning {
  return {
    taskPinId,
...
function flattenWarnings(taskPinId: string, invalid: LoomWorkflowTaskInvalidBuckets): LoomDashboardWarning[] {
  return [
    ...invalid.tasks,
    ...invalid.claims,
    ...invalid.statuses,
...
function missingPaymentWarning(taskPinId: string, acceptance: LoomCachedRecord | undefined): LoomDashboardWarning | undefined {
  if (!acceptance) {
    return undefined;
  }
...
function eventForRecord(taskPinId: string, kind: LoomDashboardTimelineEventKind, record: LoomCachedRecord): LoomDashboardTimelineEvent {
...
function eventForWarning(warning: LoomDashboardWarning): LoomDashboardTimelineEvent {
  return {
...
function eventForLocalWorkflow(taskPinId: string, local: LoomDashboardLocalEvidence): LoomDashboardTimelineEvent {
...
function localWorkflowTimestamp(local: LoomDashboardLocalEvidence): number {
  const timestamp = Date.parse(local.updatedAt);
  return Number.isFinite(timestamp) ? timestamp : 0;
}

function sortTimeline(events: LoomDashboardTimelineEvent[]): LoomDashboardTimelineEvent[] {
...
function actorContextForTask(
  actor: LoomDashboardActorContext | undefined,
  requesterGlobalMetaId: string | undefined,
  ...
  const isRequester = Boolean(
    (actorGlobalMetaId && requesterGlobalMetaId && actorGlobalMetaId === requesterGlobalMetaId)
...
function claimSummary(
  ...
  return {
    ...
    developer: projectLoomDashboardBotIdentity({
      role: 'developer',
      author: claim,
...
function bountyFromTask(task: LoomCachedRecord): { amount?: string; currency?: string } | undefined {
...
function repoFromTask(task: LoomCachedRecord): { repoUri?: string; baseBranch?: string } | undefined {
...
function latestActivity(task: LoomCachedRecord, timeline: LoomDashboardTimelineEvent[]): number {
  return Math.max(task.timestamp, ...timeline.map((event) => event.timestamp));
}

function makeColumns(tasks: LoomDashboardTaskCard[]): LoomDashboardColumn[] {
...
function matchesFilters(card: LoomDashboardTaskCard, detail: LoomDashboardTaskDetail, filters: LoomDashboardFilters): boolean {
  if (filters.state) {
...
function summarize(tasks: LoomDashboardTaskCard[], warnings: LoomDashboardWarning[]) {
  return {
...
export function buildLoomDashboard(
  rawState: LoomRawCacheState,
  options: BuildLoomDashboardOptions = {},
): LoomDashboardStateModel {
  const workflowStates = options.workflowStates ?? [];
  const cards: LoomDashboardTaskCard[] = [];
  const details: LoomDashboardTaskDetail[] = [];

  for (const task of sortRecords(rawState.records.task)) {
    const workflowState = buildLoomWorkflowTaskState(rawState, task.pinId);
    ...
    const claimSummaries = sortRecords(valid.claims).map((claim) => claimSummary(
      claim,
      activeClaimIds.has(claim.pinId),
      options,
    ));
    const requester = projectLoomDashboardBotIdentity({
      role: 'requester',
      author: task,
      identityMap: options.identityMap,
    });
    ...
    const actorContext = actorContextForTask(
    ...
    const card: LoomDashboardTaskCard = {
    ...
    const detail: LoomDashboardTaskDetail = {
      taskPinId: task.pinId,
      state,
      columnId,
      title: card.title,
...
export function findLoomDashboardTaskDetail(
  dashboard: LoomDashboardStateModel,
  taskPinId: string,
): LoomDashboardTaskDetail | undefined {
