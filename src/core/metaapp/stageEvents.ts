...
export interface MetaAppStageHub {
  publish(op: string, event: { stage: string } & Record<string, unknown>): void;
...
interface OpState {
  events: MetaAppStageEvent[];
  listeners: Set<(event: MetaAppStageEvent) => void>;
  touchedAt: number;
  terminal: boolean;
}

type MetaAppStageListener = (event: MetaAppStageEvent) => void;

export function createMetaAppStageHub(input?: {
  now?: () => number;
  bufferSize?: number;
  ttlMs?: number;
  maxOps?: number;
  ...
  // evict anything.
  const pending = new Map<string, Set<MetaAppStageListener>>();

  const prune = (): void => {
    const at = now();
    for (const [op, state] of ops) {
      if (at - state.touchedAt >= ttlMs) ops.delete(op);
    }
  ...
  const touch = (op: string): OpState => {
    ...
    const created: OpState = { events: [], listeners: new Set(), touchedAt: now(), terminal: false };
    ops.set(op, created);
    return created;
  };
  ...
  return {
    publish(op, event) {
      ...
      const record: MetaAppStageEvent = { op: key, stage, at: now(), ...detail };
    ...
    subscribe(op, listener) {
      ...
      const unsubscribe = (): void => {
        ...
        if (waiting) {
          ...
          if (waiting.size === 0) pending.delete(key);
        }
      };
