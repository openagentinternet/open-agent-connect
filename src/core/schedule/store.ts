...
export type ScheduleClaimResult =
  | { ok: true; run: ScheduledTaskRun; task: ScheduledTask }
  | { ok: false; code: 'task_not_found' | 'task_expired' | 'already_running' };

export type ScheduleCompleteResult =
  | { settled: boolean; run: ScheduledTaskRun; task: ScheduledTask | null }
  | { notFound: true };

export interface ScheduleStore {
  listTasks(): Promise<ScheduledTask[]>;
  getTask(id: string): Promise<ScheduledTask | null>;
  createTask(input: CreateScheduleTaskInput, options?: { now?: number }): Promise<ScheduledTask>;
  updateTask(
    id: string,
    partial: Partial<CreateScheduleTaskInput>,
    options?: { now?: number },
  ): Promise<{ task: ScheduledTask; warnings: ScheduleEnableWarning[] } | { notFound: true }>;
  ...
  listDue(options?: { now?: number }): Promise<ScheduledTask[]>;
  /** Atomically create the run row and mark the task running. */
  claim(
    id: string,
    input: { trigger: ScheduleRunTrigger; executor: ScheduleRunExecutor | null },
    options?: { now?: number },
  ): Promise<ScheduleClaimResult>;
  /** Settle a claimed run; applies auto-disable/prune rules. */
  complete(
    runId: string,
    input: { error?: string | null; durationMs?: number | null },
    options?: { now?: number },
  ): Promise<ScheduleCompleteResult>;
  listRuns(options?: { taskId?: string; limit?: number }): Promise<ScheduledTaskRun[]>;
}
...
export const SCHEDULE_HOST_LEASE_MS = 3 * 60 * 1000;
...
export const MAX_RUNS_LIST_LIMIT = 1000;
export const CRASH_RECOVERY_ERROR = 'Process stopped during execution';
export const STALE_RUNNING_RESET_ERROR = 'stale running run reset';

const CHANNELS: ReadonlySet<string> = new Set(['auto', 'host', 'daemon']);
...
function normalizeFiniteNumber(value: unknown, fallback: number | null = null): number | null {
...
export function validateScheduleSpec(value: unknown): { spec: ScheduleSpec | null; error: string | null } {
  ...
  if (record.type === 'interval') {
    ...
    if (intervalMs === null || !Number.isInteger(intervalMs) || intervalMs < MIN_INTERVAL_MS) {
      return { spec: null, error: `schedule.interval intervalMs must be an integer >= ${MIN_INTERVAL_MS}.` };
    }
...
function normalizeChannel(value: unknown): ScheduleChannel {
  return CHANNELS.has(value as string) ? value as ScheduleChannel : 'auto';
}
...
function normalizeTaskState(value: unknown): ScheduledTask['state'] {
...
function normalizeTask(value: unknown): ScheduledTask | null {
  ...
  if (!id || !name || !prompt || !schedule) return null;
...
function normalizeRun(value: unknown, nowMs: number): ScheduledTaskRun | null {
  if (!value || typeof value !== 'object' || Array.isArray(value)) return null;
  const record = value as Record<string, unknown>;
  const id = normalizeText(record.id);
  ...
  let runStatus: ScheduleRunStatus = status === 'success' || status === 'error' ? status : 'running';
...
function utcToday(nowMs: number): string {
  return new Date(nowMs).toISOString().slice(0, 10);
}

/** An expired task (`expiresAt <=` UTC today) never fires. */
function isExpired(task: ScheduledTask, nowMs: number): boolean {
  return task.expiresAt !== null && task.expiresAt <= utcToday(nowMs);
}

/** Local wall-clock epoch of an `at` datetime (no timezone suffix). */
function atDatetimeMs(task: ScheduledTask): number {
  if (task.schedule.type !== 'at') return NaN;
  return Date.parse(task.schedule.datetime);
}

export function computeNextRunAtMs(task: ScheduledTask, nowMs: number): number | null {
  ...
  if (schedule.type === 'interval') {
    return nowMs + Math.max(MIN_INTERVAL_MS, Math.floor(schedule.intervalMs));
  }
  return nextCronOccurrence(schedule.expression, nowMs);
}

/** Warnings for enabling a task: expired, or a one-shot `at` in the past. */
export function enableWarnings(task: ScheduledTask, nowMs: number): ScheduleEnableWarning[] {
  const warnings: ScheduleEnableWarning[] = [];
  if (isExpired(task, nowMs)) warnings.push('TASK_EXPIRED');
...
function pruneRuns(runs: ScheduledTaskRun[]): ScheduledTaskRun[] {
  const counts = new Map<string, number>();
  const kept: ScheduledTaskRun[] = [];
  // Newest first so the per-task cap keeps the most recent runs.
  const sorted = [...runs].sort(
    (left, right) => (parseIsoMs(right.startedAt) ?? 0) - (parseIsoMs(left.startedAt) ?? 0),
  );
  ...
  return kept.sort(
    (left, right) => (parseIsoMs(left.startedAt) ?? 0) - (parseIsoMs(right.startedAt) ?? 0),
  );
}

export function createScheduleStore(paths: MetabotPaths): ScheduleStore {
  ...
  async function writeJsonAtomic(file: ScheduleFile): Promise<void> {
  ...
  async function readScheduleFile(nowMs: number): Promise<ScheduleFile> {
    try {
      ...
      return {
        ...
        tasks: Array.isArray(record.tasks)
          ? record.tasks.map(normalizeTask).filter((task): task is ScheduledTask => task !== null)
          : [],
        runs: Array.isArray(record.runs)
          ? record.runs.map((run) => normalizeRun(run, nowMs)).filter((run): run is ScheduledTaskRun => run !== null)
          : [],
      };
    } catch (error) {
   ...
   */
  async function sweepStaleClaims(file: ScheduleFile, nowMs: number): Promise<boolean> {
    let changed = false;
    for (const task of file.tasks) {
      if (task.state.runningAtMs === null) continue;
      if (task.state.runningAtMs > nowMs - STALE_RUNNING_MS) continue;
  ...
  return {
    ...
    async createTask(input, options = {}) {
      ...
      if (!spec) throw new Error(error ?? 'invalid schedule');
    ...
    async updateTask(id, partial, options = {}) {
      ...
      return enqueue(async () => {
        ...
        if (partial.description !== undefined) task.description = normalizeText(partial.description);
        if (partial.prompt !== undefined) {
          const prompt = normalizeText(partial.prompt);
          if (!prompt) throw new Error('prompt must not be empty.');
          task.prompt = prompt;
        }
        if (partial.workingDirectory !== undefined) task.workingDirectory = normalizeText(partial.workingDirectory);
        if (partial.channel !== undefined) {
          if (!CHANNELS.has(partial.channel)) throw new Error('channel must be auto, host, or daemon.');
    ...
    async deleteTask(id) {
      return enqueue(async () => {
        ...
        if (file.tasks.length === before) return { deleted: false };
    ...
    async listDue(options = {}) {
      ...
      return enqueue(async () => {
        ...
        if (swept) await writeJsonAtomic(file);
        return file.tasks
          .filter((task) => (
            task.enabled
    ...
    async claim(id, input, options = {}) {
      ...
      return enqueue(async () => {
        ...
        if (swept) await writeJsonAtomic(file);
        const task = file.tasks.find((entry) => entry.id === id);
        if (!task) return { ok: false as const, code: 'task_not_found' as const };
        if (isExpired(task, nowMs)) return { ok: false as const, code: 'task_expired' as const };
    ...
    async complete(runId, input, options = {}) {
      ...
      return enqueue(async () => {
        ...
        if (!isClaimHolder) return { settled: false, run, task };
    ...
    async listRuns(options = {}) {
      ...
      const limit = options.limit === undefined
        ? MAX_RUNS_LIST_LIMIT
        : Math.max(1, Math.min(MAX_RUNS_LIST_LIMIT, Math.floor(options.limit)));
