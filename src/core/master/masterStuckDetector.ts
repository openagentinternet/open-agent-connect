...
function normalizeStuckLevel(score: number): MasterAskWorthinessAssessment['stuckLevel'] {
  if (score >= 8) {
    return 'critical'
  }
  if (score >= 5) {
...
export function assessMasterAskWorthiness(
  observation: MasterHostObservationFrame,
  options: MasterStuckDetectorOptions = {}
): MasterAskWorthinessAssessment {
  const minNoProgressWindowMs = normalizeInteger(
    options.minNoProgressWindowMs,
    DEFAULT_MIN_NO_PROGRESS_WINDOW_MS
  )
  ...
  let opportunityType: MasterAskWorthinessAssessment['opportunityType'] = 'none'
  if (stuckLevel !== 'none') {
  ...
  let candidateMasterKind: string | null = observation.hints.candidateMasterKindHint
