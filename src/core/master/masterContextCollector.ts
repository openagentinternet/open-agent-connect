...
function extractStderrHighlights(result: MasterContextToolResult): string[] {
...
function createArtifact(
  label: string,
  content: string,
  source: CollectedMasterContextArtifact['source'],
  filePath: string | null = null
): CollectedMasterContextArtifact | null {
...
function collectArtifacts(input: {
  ...
  toolResults: MasterContextToolResult[];
  diffSummary: string | null;
  fileExcerpts: MasterContextFileExcerpt[];
}): CollectedMasterContextArtifact[] {
  const artifacts: CollectedMasterContextArtifact[] = [];
  const seen = new Set<string>();

  const pushArtifact = (artifact: CollectedMasterContextArtifact | null) => {
    if (!artifact) {
      return;
    }
...
function composeWorkspaceSummary(input: {
  ...
  const safeRelevantFiles = sanitizeRelevantFiles(input.relevantFiles, 3);
...
export function collectMasterContext(input: MasterContextCollectionInput | Record<string, unknown>): CollectedMasterContext {
