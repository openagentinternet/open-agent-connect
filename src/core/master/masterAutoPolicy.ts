...
function normalizeSensitivity(value: Partial<MasterPayloadSafetySummary> | null | undefined): MasterPayloadSafetySummary {
  ...
  return {
    ...
    autoPolicy: {
      ...
      minNoProgressWindowMs: normalizeNonNegativeInteger(
        config?.autoPolicy?.minNoProgressWindowMs,
        defaults.autoPolicy.minNoProgressWindowMs
      ),
      perTraceLimit: Math.max(
        1,
        normalizeNonNegativeInteger(config?.autoPolicy?.perTraceLimit, defaults.autoPolicy.perTraceLimit)
      ),
      globalCooldownMs: normalizeNonNegativeInteger(
        config?.autoPolicy?.globalCooldownMs,
...
export function evaluateMasterAutoPolicy(input: {
