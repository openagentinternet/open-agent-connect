...
import { METAWEB_CITATION_RULE, markdownSelfLink } from '../metaweb/uri';
import type {
  MetaProtocolListItem,
...
export function truncateMetaprotocolText(value: string, max: number): string {
...
export function formatMetaprotocolDate(ts: number): string {
  return ts > 0 ? new Date(ts * 1000).toISOString().slice(0, 10) : '';
}

export function metaprotocolAuthorLabel(author: { name: string; globalMetaId: string; metaid: string; address: string }): string {
  return author.name || author.globalMetaId || author.metaid || author.address || 'unknown';
}

export function renderMetaprotocolList(page: MetaProtocolListPage): string {
...
function renderMetaprotocolListItem(item: MetaProtocolListItem): string[] {
  ...
  if (item.intro) lines.push(`  ${truncateMetaprotocolText(flattenInline(item.intro), 160)}`);
...
export function renderMetaprotocolRead(record: MetaProtocolRecord): string {
  ...
  if (record.intro) lines.push(`- intro: ${truncateMetaprotocolText(flattenInline(record.intro), 240)}`);
...
export function renderMetaprotocolPinVersions(versions: MetaProtocolPinVersions, label: string): string {
  const lines = [
...
export function renderMetaprotocolFallbackList(registrations: MetaProtocolManapiRegistration[], query: string): string {
...
export function renderMetaprotocolFallbackRead(entry: MetaProtocolManapiRegistration): string {
  const payload = entry.payload ?? {};
  ...
  if (intro) lines.push(`- intro: ${truncateMetaprotocolText(intro, 240)}`);
...
export function renderMetaprotocolFallbackVersions(versions: MetaProtocolManapiVersion[], label: string): string {
  const lines = [
    '(degraded: registry fallback)',
