...
const ZH_BROWSER_TEXT_REPLACEMENTS: Array<[RegExp, string]> = [
...
function normalizeBrowserLanguage(languagePreference?: string | null): string {
...
export function renderBrowserPageHtml(
  ...
  return renderAbcBrowserPageHtml(definition, languagePreference).then((html) => {
    const language = normalizeBrowserLanguage(languagePreference);
    if (language !== 'zh-CN') {
      return html;
    }
    return ZH_BROWSER_TEXT_REPLACEMENTS.reduce(
      (output, [pattern, replacement]) => output.replace(pattern, replacement),
      html.replace(/<html lang="[^"]+">/, '<html lang="zh-CN">'),
    );
  });
}
