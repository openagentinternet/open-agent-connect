...
export const handleLlmRoutes: RouteHandler = async (context) => {
  ...
  if (url.pathname === '/api/llm/host-executor/status' && req.method === 'GET') {
    const result = handlers.llm?.hostExecutorStatus
      ...
      : commandFailed('not_implemented', 'Host LLM executor handler not configured.');
    context.sendJson(200, result);
    return true;
  }
  ...
  if (url.pathname === '/api/llm/host-executor/events' && req.method === 'GET') {
    const stream = handlers.llm?.hostExecutorEvents
      ...
      : null;
    if (!stream) {
      context.sendJson(404, commandFailed('llm_host_executor_not_available', 'Host LLM executor stream is not configured.'));
      return true;
    }
