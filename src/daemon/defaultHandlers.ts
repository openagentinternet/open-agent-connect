...
import { isRuntimePlatformId } from '../core/platform/platformRegistry';
import {
  isLlmProvider,
  normalizeLlmBinding,
...
import {
  ...
  TrafficApiError,
  type TrafficAccountService,
  type TrafficFreeGrantCampaignStatus,
...
import type { A2AConversationActor, A2AConversationMessage, A2AConversationSession } from '../core/a2a/conversationTypes';
import {
...
import {
  ...
  type ListPeerConversationSummariesResult,
  type PeerConversationSummary,
  type ReadPeerConversationMessagesResult,
} from '../core/a2a/conversationProjection';
import { updatePeerConversationMeta } from '../core/a2a/conversationMeta';
import {
...
interface LocalSocketPresenceStatus {
  enabled: boolean;
...
async function readSocketPresenceStatusForGlobalMetaId(input: {
  ...
  socketPresenceApiBaseUrl?: string;
  socketPresenceFailureMode?: 'throw' | 'assume_service_providers_online';
}): Promise<LocalSocketPresenceStatus> {
...
function buildProviderSummaryPayload(input: {
  state: RuntimeState;
  presence: LocalSocketPresenceStatus;
...
function extractOrderReferenceLineValue(content: string): string {
...
interface LlmDiscoverySweepStatus {
  running: boolean;
  lastStartedAt?: string;
  lastFinishedAt?: string;
  /** Queued-or-running sweep count; internal only, never exposed to clients. */
  activeSweeps: number;
}
...
const llmDiscoverySweepStatusByHomeDir = new Map<string, LlmDiscoverySweepStatus>();
...
const llmDiscoverySweepChainByHomeDir = new Map<string, Promise<unknown>>();

// Public view exposed through the runtimes API; internal counters stay hidden.
...
function normalizeDiscoveryProviders(value: unknown): LlmProvider[] {
  ...
  return value.filter((entry): entry is LlmProvider => isRuntimePlatformId(entry));
}

async function runLlmDiscoverySweep(
  homeDir: string,
  providers: LlmProvider[],
  discover: typeof discoverLlmRuntimes,
): Promise<{ discovered: number; runtimes: LlmRuntime[]; errors: Array<{ provider: string; message: string }> }> {
  ...
  const sweptProviders = providers.length ? new Set<LlmProvider>(providers) : null;
  for (const runtime of previous.runtimes) {
    if (runtime.provider === 'custom') continue;
    if (sweptProviders && !sweptProviders.has(runtime.provider)) continue;
...
async function runTrackedLlmDiscoverySweep(
  homeDir: string,
  providers: LlmProvider[],
  discover: typeof discoverLlmRuntimes,
) {
...
function startBackgroundLlmDiscoverySweep(
  homeDir: string,
  providers: LlmProvider[],
  discover: typeof discoverLlmRuntimes,
): void {
 ...
 */
function trafficCommandFailure(error: unknown, fallbackCode: string): MetabotCommandResult<never> {
  if (error instanceof TrafficApiError) {
    const data: Record<string, unknown> = { featureUnavailable: error.featureUnavailable };
    if (error.errorCode) data.errorCode = error.errorCode;
    if (error.status === undefined || error.status >= 500) data.retryable = true;
    return commandFailed(error.code, error.message, { data });
  }
 ...
 *  an explicit do-not-retry message. Records the attempt for dedup. */
  ...
  function stableJsonForChainWriteHash(value: unknown): string {
    if (Array.isArray(value)) {
  ...
  function metaAppOwnerWriteHash(kind: string, mvcAddress: string, rawInput: Record<string, unknown>): string {
    const { opId: _opId, confirm: _confirm, from: _from, network: _network, ...content } = rawInput;
    return stableChainWriteHash(kind, [
      mvcAddress,
  ...
  async function metaAppOwnerWritePending(
    kind: string,
    contentHash: string,
  ): Promise<MetabotCommandResult<never> | null> {
    const prior = await createChainWriteAttemptStore(normalizedSystemHomeDir)
  ...
  const trafficAccountService = input.trafficAccountService ?? createTrafficAccountService({
  ...
  async function resolveMvcSponsorUploadClientForHome(
    ...
    const client: MvcSponsorV2DirectUploadClient = trafficMode && !input.createMvcSponsorClient
      ? createMvcSponsorV2Client({ baseUrl: await trafficAccountService.getConfiguredTrafficApiBase() })
      : (input.createMvcSponsorClient ?? createMvcSponsorV2Client)();
    if (trafficMode) {
  ...
  const baseA2AConversationPersister = input.a2aConversationPersister ?? persistA2AConversationMessage;
  ...
  type ChainProfileCacheEntry = {
    projection: ConversationProfileProjection | null;
    expiresAt: number;
  };
  ...
  function readChainProfileData(value: unknown): Record<string, unknown> {
    const record = readObject(value) ?? {};
    return readObject(record.data) ?? record;
  }

  function readChainProfileName(data: Record<string, unknown>): string {
  ...
  function readChainProfileAvatarReference(data: Record<string, unknown>): string {
  ...
  async function resolveChainProfile(globalMetaId: string): Promise<ConversationProfileProjection | null> {
    const gmid = normalizeText(globalMetaId);
    if (!gmid) return null;
    ...
    const pending = (async () => {
      let projection: ConversationProfileProjection | null = null;
  ...
  > {
    ...
    const profile = await getMetabotProfile(normalizedSystemHomeDir, requestedSlug);
    if (!profile) {
      return {
        failure: commandFailed('profile_not_found', `MetaBot profile not found: ${requestedSlug || '<missing>'}`),
      };
    }
    const state = await createRuntimeStateStore(profile.homeDir).readState();
    if (!state.identity) {
      return {
        failure: commandFailed('identity_missing', 'No local MetaBot identity is loaded for the selected MetaBot.'),
      };
    }
  ...
  function buildNativeTransferAmountRaw(chain: string, amount: string): string | MetabotCommandResult<never> {
    ...
    if (!unit) {
      return commandFailed(
        'invalid_argument',
        `Unsupported wallet transfer chain: ${normalizedChain || '<missing>'}. Supported chains: ${Object.keys(NATIVE_TRANSFER_UNITS).join(', ')}.`,
      );
    }
    const normalizedAmount = normalizeText(amount);
    if (!normalizedAmount) {
      return commandFailed('invalid_argument', 'Wallet transfer amount is required.');
    }
    return `${normalizedAmount}${unit}`;
  }

  async function resolveMetabotProfileBySelector(selector: unknown): Promise<MetabotProfileFull | null> {
    const requestedSelector = normalizeText(selector);
    if (!requestedSelector) {
      return null;
    }
    const bySlug = await getMetabotProfile(normalizedSystemHomeDir, requestedSelector);
    if (bySlug) {
      return bySlug;
    }
  ...
  function metabotProfileProjection(profile: MetabotProfileFull): ConversationProfileProjection | null {
  ...
  async function loadConversationProfileIndex(
    selectedProfile: MetabotProfileFull,
  ): Promise<Map<string, ConversationProfileProjection>> {
    const profiles = await listMetabotProfiles(normalizedSystemHomeDir).catch(() => [] as MetabotProfileFull[]);
    const index = new Map<string, ConversationProfileProjection>();
  ...
  function resolveConversationProfileProjection(inputProfile: {
    ...
    profileIndex: Map<string, ConversationProfileProjection>;
  }): ConversationProfileProjection | null {
  ...
  async function enrichConversationListResult(
    result: ListPeerConversationSummariesResult,
    selectedProfile: MetabotProfileFull,
  ...
  async function enrichConversationMessagesResult(
    result: ReadPeerConversationMessagesResult,
    selectedProfile: MetabotProfileFull,
  ...
  async function ensurePrivateChatConversationForPeer(inputForConversation: {
    homeDir: string;
    stateStore: ReturnType<typeof createPrivateChatStateStore>;
    localGlobalMetaId: string;
    peerGlobalMetaId: string;
  ...
  async function runConversationGuidanceTurn(inputForTurn: {
    ...
    if (!profile) {
      return commandFailed('profile_not_found', `MetaBot profile not found: ${normalizeText(inputForTurn.local) || '<missing>'}`);
    }
    ...
    if (!state.identity) {
      return commandFailed('identity_missing', 'Create a local MetaBot identity before sending guided conversation turns.');
    }

    const peerGlobalMetaId = normalizeText(inputForTurn.peer);
    if (!peerGlobalMetaId) {
      return commandFailed('missing_peer', 'peer is required.');
    }
    const guidanceText = normalizeText(inputForTurn.guidance);
    if (!guidanceText) {
      return commandFailed('missing_guidance', 'guidance is required.');
    }
    ...
    const profileMetaBotSlug = path.basename(profileRuntimeStateStore.paths.profileRoot);
      ...
      ?? (() => {
        ...
        const runtimeResolver = createLlmRuntimeResolver({
          ...
          getPreferredRuntimeId: async () => {
            try {
              const raw = await fs.readFile(profileRuntimeStateStore.paths.preferredLlmRuntimePath, 'utf8');
  ...
  async function failInboundProviderOrderValidation(inputFailure: {
    state: RuntimeState;
    service?: RuntimeState['services'][number] | null;
    buyerGlobalMetaId: string;
    content: string;
    orderTxid: string;
    orderMessageId?: string | null;
    ...
    const orderEndContent = buildOrderEndMessage(orderTxid, 'failed', failureText, orderReference || null);
    ...
    if (service && orderTxid) {
      ...
      const failedResult = createServiceRunnerFailedResult(inputFailure.code, failureText);
    ...
    return commandFailed(inputFailure.code, failureText, { data });
  }
  ...
  async function handleInboundProviderOrderMessage(inputMessage: {
    ...
    const orderTxid = normalizeOrderProtocolReference(inputMessage.messagePinId)
      ...
      || normalizeOrderProtocolReference(extractOrderLineValue(content, 'order pin id'))
      || '';
    ...
    const service = servicePinId
      ...
      : null;
    const failValidation = (code: string, message: string) => failInboundProviderOrderValidation({
      state,
      service,
  ...
  async function resolveTrafficBotNames(): Promise<Map<string, string>> {
  ...
  function withTrafficBotName<TEntry extends { botAddress?: string }>(
    entry: TEntry,
    botNames: Map<string, string>,
  ): TEntry & { botName?: string } {
  ...
  const handlers: MetabotDaemonHttpHandlers = {
    ...
    traffic: {
      ...
      status: async () => {
        try {
          ...
          const identityRecord = await readOwnerIdentity(normalizedSystemHomeDir).catch(() => null);
          ...
          let account: TrafficAccountRecord | null = null;
          ...
          let freeGrant: TrafficFreeGrantCampaignStatus | null = null;
      ...
      getMode: async () => {
        try {
          return commandSuccess({ mode: await trafficAccountService.getTrafficPinMode() });
        } catch (error) {
      ...
      setMode: async ({ mode }) => {
        ...
        if (normalizedMode !== 'traffic' && normalizedMode !== 'selfpay') {
          return commandFailed('invalid_argument', 'mode must be "traffic" or "selfpay".');
        }
        try {
          await trafficAccountService.setTrafficSettingsSnapshot({ mode: normalizedMode });
          if (normalizedMode !== 'traffic') {
            return commandSuccess({ mode: normalizedMode });
          }
          const bindSummary = await trafficAccountService.bindAllLocalBots();
          return commandSuccess({ mode: normalizedMode, bindSummary });
        } catch (error) {
      ...
      getBalance: async () => {
        try {
          ...
          return commandSuccess({ account, featureUnavailable: false });
        } catch (error) {
          if (error instanceof TrafficApiError && error.featureUnavailable) {
            return commandSuccess({ account: null, featureUnavailable: true });
          }
      ...
      getLedger: async (rawInput) => {
        try {
          const limit = Number.isFinite(rawInput.limit) && (rawInput.limit ?? 0) > 0
            ? Math.floor(rawInput.limit as number)
            : 20;
          const cursor = Number.isFinite(rawInput.cursor) && (rawInput.cursor ?? 0) > 0
            ? Math.floor(rawInput.cursor as number)
            : undefined;
          ...
          return commandSuccess({
            entries: entries.map((entry: TrafficLedgerEntry) => withTrafficBotName(entry, botNames)),
            nextCursor: nextCursor > 0 ? String(nextCursor) : null,
          });
        } catch (error) {
          if (error instanceof TrafficApiError && error.featureUnavailable) {
            return commandSuccess({ entries: [], nextCursor: null, featureUnavailable: true });
          }
      ...
      getUsage: async () => {
        try {
          let summary: TrafficUsageSummary | null = null;
          let featureUnavailable = false;
          try {
      ...
      redeem: async ({ code }) => {
        ...
        if (!normalizedCode) {
          return commandFailed('missing_argument', 'A redeem code is required.');
        }
        try {
          return commandSuccess(await trafficAccountService.redeemTrafficCode(normalizedCode));
        } catch (error) {
      ...
      getApiBase: async () => {
        try {
          ...
          return commandSuccess({
            apiBase: configured ?? '',
            effectiveApiBase: configured ?? DEFAULT_TRAFFIC_API_BASE_URL,
          });
        } catch (error) {
      ...
      setApiBase: async ({ apiBase }) => {
        ...
        try {
          normalizedApiBase = normalizeTrafficApiBase(apiBase);
        } catch (error) {
          return commandFailed('invalid_argument', error instanceof Error ? error.message : String(error));
        }
        if (!normalizedApiBase) {
          return commandFailed('missing_argument', 'An http(s) URL is required (use api-base reset to clear).');
        }
        try {
          await trafficAccountService.setTrafficSettingsSnapshot({ apiBase: normalizedApiBase });
          return commandSuccess({ apiBase: normalizedApiBase, effectiveApiBase: normalizedApiBase });
        } catch (error) {
          return trafficCommandFailure(error, 'traffic_settings_failed');
        }
      },
      resetApiBase: async () => {
        try {
          ...
          return commandSuccess({ apiBase: '', effectiveApiBase: DEFAULT_TRAFFIC_API_BASE_URL });
        } catch (error) {
          return trafficCommandFailure(error, 'traffic_settings_failed');
        }
      },
    ...
    metaapp: {
          ...
          const result = await publishMetaApp(
            ...
            },
            {
              ...
              writeChain: async (writeInput) => {
                ...
                return written as unknown as ChainLikeResult;
              },
              ...
              writeAttempts: createChainWriteAttemptStore(normalizedSystemHomeDir),
              now: Date.now,
              ...(onStage ? { onStage } : {}),
            },
          );
      ...
      publish: async (rawInput) => {
        ...
        try {
          ...
          return finish(await commandFailedOrBroadcastUnknown(
            error,
            'metaapp publish',
            contentHash,
            'metaapp_publish_failed',
      ...
      share: async (rawInput) => {
        try {
          const share = await shareMetaApp({
          ...
          const shareData = share.ok && share.data && typeof share.data === 'object'
            ? share.data as Record<string, unknown>
            : {};
          const bundlePinId = typeof shareData.pinId === 'string' ? shareData.pinId : '';
          const network = await resolveWriteNetworkForHome(rawInput.network, actor.homeDir);
          const announcement = await postBuzzToChain({
      ...
      comment: async (rawInput) => {
        ...
        const state = await actor.runtimeStateStore.readState();
        if (!state.identity) {
          return commandFailed('identity_missing', 'Create a local MetaBot identity before writing comments.');
        }
          ...
          const result = await commentMetaApp(
            ...
            },
            {
              writeChain: async (writeInput) => actor.signer.writePin({
                ...
                payload: typeof writeInput.payload === 'string' ? writeInput.payload : undefined,
                network: await resolveWriteNetworkForHome(rawInput.network, actor.homeDir),
              }) as unknown as Promise<ChainLikeResult>,
            },
          );
          return result;
        } catch (error) {
    ...
    services: {
      ...
      modifyMyService: async (rawInput) => {
        ...
        const providerSkillValidation = await validateServicePublishProviderSkills({
          ...
          providerSkills: selectProviderSkillSource({
            providerSkills: rawInput.providerSkills,
            providerSkill: rawInput.providerSkill,
      ...
      call: async (rawInput) => {
        ...
        let paymentTxid = '';
        let orderReference = '';
        let serviceOrderPinId: string | null = null;
        let serviceOrderTxid: string | null = null;
        let serviceOrderTxids: string[] = [];
        const applyOrderPayment = (payment: A2AOrderPaymentResult) => {
        ...
        const publishServiceOrderRecord = async (payment: A2AOrderPaymentResult): Promise<
          { ok: true; payment: A2AOrderPaymentResult }
          | { ok: false; failure: MetabotCommandResult<never> }
        > => {
          ...
          if (!payload.servicePinId) {
            return {
              ok: false,
              failure: commandFailed(
                'skill_service_order_service_missing',
                'Skill service order requires a servicePinId.'
              ),
            };
          }
          try {
            ...
            if (!pinId) {
              return {
                ok: false,
                failure: commandFailed(
                  'skill_service_order_pin_missing',
                  'Skill service order write did not return a pin id.'
                ),
              };
            }
            const txids = Array.isArray(write.txids)
              ...
              : [];
            const txid = (
              normalizeOrderProtocolReference(txids[0])
              || normalizeOrderProtocolReference(pinId)
              || txids[0]
              || null
            );
            return {
      ...
      execute: async (rawInput) => {
        ...
        const orderMessageId = (
          execution.payment.serviceOrderPinId
    ...
    conversations: {
      list: async (rawInput) => {
        ...
        if (!profile) {
          return commandFailed('profile_not_found', `MetaBot profile not found: ${normalizeText(rawInput.local) || '<missing>'}`);
        }
        try {
          ...
          return commandSuccess(await enrichConversationListResult(result, profile));
        } catch (error) {
          return commandFailed(
      ...
      messages: async (rawInput) => {
        ...
        try {
          ...
          return commandSuccess(await enrichConversationMessagesResult(result, profile));
        } catch (error) {
          return commandFailed(
      ...
      meta: async (rawInput) => {
        ...
        try {
          const result = await updatePeerConversationMeta({
            homeDir: profile.homeDir,
            localGlobalMetaId: profile.globalMetaId,
            peerGlobalMetaId: peer,
          ...
          if (!result) {
            return commandFailed(
              ...
              `No A2A conversation found between ${profile.globalMetaId} and ${peer}.`,
            );
          }
          ...
          return commandFailed(
            'conversation_meta_failed',
            error instanceof Error ? error.message : 'Failed to update conversation meta.',
          );
        }
      },
    ...
    bot: {
      ...
      confirmWalletTransfer: async ({ slug, chain, toAddress, amount }) => {
        ...
        return confirmWalletTransfer({
          ...
          toAddress,
          amountRaw,
          secretStore: createFileSecretStore(resolved.profile.homeDir),
        });
      },
      ...
      discoverRuntimes: async (request = {}) => {
        ...
        const discover = input.discoverLlmRuntimes ?? discoverLlmRuntimes;
        if (request.background === true) {
          startBackgroundLlmDiscoverySweep(profileHomeDir, providers, discover);
          const runtimeStore = createLlmRuntimeStore(profileHomeDir);
          const state = await runtimeStore.read();
          return commandSuccess({ status: 'running', runtimes: state.runtimes });
        }
