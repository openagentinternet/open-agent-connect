...
const COMMAND_HELP_SPECS: CommandHelpSpec[] = [
  ROOT_COMMAND_HELP,
  {
    ...
    subcommands: [
      ...
      { name: 'bind-owner', summary: 'Bind a MetaBot to its owner GlobalMetaID.' },
  ...
  },
  {
    ...
    usage: 'metabot bot create --name <name> [--host <provider>] [--dsh-llm-provider <id>] [--dsh-llm-model <id>] [--dsh-llm-reasoning-effort <effort>] [--dsh-llm-fallback-provider <id>] [--dsh-llm-fallback-model <id>] [--dsh-llm-fallback-reasoning-effort <effort>]',
    requiredFlags: [
    ...
    optionalFlags: [
      ...
      { flag: '--dsh-llm-reasoning-effort', value: '<off|low|high|max>', description: 'Optional DSH reasoning effort stored with the primary DSH model. Ignored by OAC host runtimes.' },
      ...
      { flag: '--dsh-llm-fallback-reasoning-effort', value: '<off|low|high|max>', description: 'Optional DSH reasoning effort stored with the fallback DSH model.' },
      { flag: '--type', value: '<twin|worker>', description: 'Bot role. Creating a twin demotes the previous twin (at most one per machine). Defaults to worker.' },
      { flag: '--owner', value: '<globalMetaId>', description: 'Optional owner GlobalMetaID binding for this Bot.' },
      HELP_JSON_FLAG,
    ],
  ...
  },
  {
    ...
    requestShape: {
      ...
      dshLlmReasoningEffort: 'optional DSH reasoning effort (off|low|high|max) or null',
      ...
      dshLlmFallbackReasoningEffort: 'optional DSH fallback reasoning effort (off|low|high|max) or null',
  ...
  {
    commandPath: ['traffic'],
    summary: 'Manage the traffic (account-quota gas credit) mode, balance, usage, free grant, and redeem codes. All verbs are owner-scoped (no --from).',
    usage: 'metabot traffic <subcommand>',
    subcommands: [
      { name: 'status', summary: 'Show the traffic mode, account, free-grant campaign, and owner identity overview.' },
      { name: 'mode', summary: 'Read or switch the pin billing mode (traffic | selfpay).' },
      { name: 'balance', summary: 'Force-refresh the traffic account balance.' },
      { name: 'ledger', summary: 'List traffic ledger entries with cursor pagination.' },
      { name: 'usage', summary: 'Show per-day per-bot traffic usage with a local-journal fallback.' },
      { name: 'claim', summary: 'Claim the one-time free traffic grant.' },
      { name: 'redeem', summary: 'Redeem a traffic recharge code.' },
      { name: 'api-base', summary: 'Read, override, or reset the traffic service base URL.' },
    ],
    optionalFlags: [HELP_JSON_FLAG],
    examples: [
  ...
  {
    commandPath: ['traffic', 'status'],
    summary: 'Show traffic mode, stored apiBase override, lazily-ensured account, free-grant campaign state, and the public owner identity.',
    usage: 'metabot traffic status',
    successFields: [
      'mode',
      'apiBase',
      'account',
    ...
    examples: ['metabot traffic status'],
    optionalFlags: [HELP_JSON_FLAG],
  },
  {
    commandPath: ['traffic', 'mode'],
    summary: 'Read the pin billing mode, or switch it. Switching to traffic ensures the account and binds every local bot; the result carries a bindSummary.',
    usage: 'metabot traffic mode [traffic|selfpay]',
    successFields: [
      'mode',
      'bindSummary (only when switching to traffic)',
    ],
    failureSemantics: [
      'Fails with invalid_argument for any mode value other than traffic or selfpay.',
      'Fails with traffic_account_failed/traffic_bind_failed when the account ensure or bot binding fails.',
    ],
    examples: [
      'metabot traffic mode',
      'metabot traffic mode traffic',
      'metabot traffic mode selfpay',
    ],
    optionalFlags: [HELP_JSON_FLAG],
  },
  {
    commandPath: ['traffic', 'balance'],
    summary: 'Force-refresh the traffic account balance from the backend.',
    usage: 'metabot traffic balance',
    successFields: [
      'account',
      'featureUnavailable',
      'localUiUrl (the /ui/traffic page; omitted when no daemon base URL is resolvable)',
    ...
    examples: ['metabot traffic balance'],
    optionalFlags: [HELP_JSON_FLAG],
  },
  {
    commandPath: ['traffic', 'ledger'],
    summary: 'List traffic ledger entries (newest page first), enriched with local journal data and bot names.',
    usage: 'metabot traffic ledger [--cursor <cursor>] [--limit <n>]',
    successFields: [
      'entries',
      'nextCursor',
    ],
    failureSemantics: [
      'Fails with invalid_argument when --cursor or --limit is not a non-negative/positive integer.',
      'Returns { entries: [], nextCursor: null, featureUnavailable: true } when the backend has no /v1/traffic/* endpoints.',
    ],
    examples: [
      'metabot traffic ledger',
      'metabot traffic ledger --cursor 128 --limit 50',
    ],
    optionalFlags: [HELP_JSON_FLAG],
  },
  {
    commandPath: ['traffic', 'usage'],
    summary: 'Show traffic usage summary (today/week/month) and per-day per-bot rows; falls back to the local spend journal when the API is unreachable.',
    usage: 'metabot traffic usage',
    successFields: [
      'summary',
      'daily',
      'source (service | local | unavailable)',
    ],
    examples: ['metabot traffic usage'],
    optionalFlags: [HELP_JSON_FLAG],
  },
  {
    commandPath: ['traffic', 'claim'],
    summary: 'Claim the one-time free traffic grant for the local account.',
    usage: 'metabot traffic claim',
    successFields: [
      'grantId',
      'grantBytes',
      'balanceAfter',
    ...
    failureSemantics: [
      'Fails with traffic_campaign_failed; data.errorCode carries backend codes such as CAMPAIGN_DISABLED, ALREADY_CLAIMED, or CLIENT_NOT_ALLOWED.',
    ],
    examples: ['metabot traffic claim'],
    optionalFlags: [HELP_JSON_FLAG],
  },
  {
    commandPath: ['traffic', 'redeem'],
    summary: 'Redeem a traffic recharge code for the local account.',
    usage: 'metabot traffic redeem <code>',
    successFields: [
      'codeId',
      'trafficBytes',
      'balanceAfter',
    ...
    failureSemantics: [
      'Fails with missing_argument when the code is omitted.',
      'Fails with traffic_redeem_failed; data.errorCode carries backend codes such as CODE_NOT_FOUND, CODE_USED, CODE_DISABLED, or CODE_EXPIRED.',
    ],
    examples: ['metabot traffic redeem IDB-XXXX-XXXX-XXXX'],
    optionalFlags: [HELP_JSON_FLAG],
  },
  {
    commandPath: ['traffic', 'api-base'],
    summary: 'Read the traffic service base URL override, set a new http(s) override, or reset to the production default.',
    usage: 'metabot traffic api-base [get|set <url>|reset]',
    successFields: [
      'apiBase (stored override; empty means default)',
      'effectiveApiBase (the URL requests actually use)',
    ],
    failureSemantics: [
      'Fails with invalid_argument when the override is not a valid http(s) URL; nothing is persisted then.',
      'Fails with missing_argument when set is called without a URL.',
    ],
    examples: [
      'metabot traffic api-base',
      'metabot traffic api-base set https://assist.example.com',
      'metabot traffic api-base reset',
    ],
    optionalFlags: [HELP_JSON_FLAG],
  },
  {
  ...
  },
  {
    ...
    subcommands: [
      ...
      { name: 'new', summary: 'Scaffold a publish-ready MetaApp project directory.' },
    ...
    examples: [
      ...
      'metabot metaapp new ./my-app --template demo',
  ...
  {
    commandPath: ['metaapp', 'new'],
    summary: 'Scaffold a publish-ready MetaApp project directory: index.html with relative asset references, the APP.md documentation file, and a .metaapp.json manifest. Runs fully local; requires no daemon, identity, or --confirm.',
    usage: 'metabot metaapp new <dir> [--template <blank|demo>] [--title <title>] [--app-name <slug>] [--force]',
    requestShape: {
      dir: 'target directory, positional or via --dir; created when missing',
      template: 'blank (default, single-file starter) or demo (counter demo with style.css/app.js)',
      title: 'optional display title; defaults to the directory base name',
      appName: 'optional lowercase slug; defaults to a slugified title',
      force: 'overwrite the template files inside a non-empty directory',
    },
    successFields: [
      'data = { projectDir, template, title, appName, files[] }',
    ],
    failureSemantics: [
      'Fails with invalid_flag when the target directory is missing or passed both positionally and via --dir.',
      'Fails with metaapp_scaffold_template_unknown when --template is not blank or demo.',
      'Fails with metaapp_scaffold_invalid_app_name when --app-name is not a lowercase slug.',
      'Fails with metaapp_scaffold_dir_not_empty when the directory exists, is non-empty, and --force is absent; --force only overwrites the template files and never deletes other files.',
    ],
    examples: [
      'metabot metaapp new ./my-app',
      'metabot metaapp new ./my-app --template demo --title "My App"',
      'metabot metaapp new --dir ./my-app --app-name my-app --force',
    ],
    optionalFlags: [
      { flag: '--dir', value: '<dir>', description: 'Target directory when not passed positionally.' },
      { flag: '--template', value: '<blank|demo>', description: 'Project template. Defaults to blank.' },
      { flag: '--title', value: '<title>', description: 'Display title written into index.html, APP.md, and .metaapp.json.' },
      { flag: '--app-name', value: '<slug>', description: 'Lowercase slug appName for the manifest; defaults to a slugified title.' },
      { flag: '--force', description: 'Overwrite the template files inside a non-empty directory.' },
      HELP_JSON_FLAG,
    ],
  },
  {
  ...
  {
    commandPath: ['grouptask'],
    summary: 'Group task commands: one on-chain MetaWeb group chat = one task, chaired by a local Bot (twin preferred).',
    usage: 'metabot grouptask <subcommand>',
    subcommands: [
      { name: 'create', summary: 'Create a group task: on-chain group + members + kickoff message.' },
      { name: 'list', summary: 'List group tasks across all local chair Bots.' },
      { name: 'detail', summary: 'Show one group task (members, deliverables, checkpoints, messages).' },
      { name: 'messages', summary: 'Show the decrypted group transcript for one task.' },
      { name: 'post', summary: 'Post a group message as a member Bot or as the owner (manual chair sends need --confirm-chair while the engine drives).' },
  ...
  },
  {
    ...
    summary: 'Create a group task end to end: chair Bot creates the on-chain group, worker Bots and the owner join, and the chair posts the kickoff message.',
    usage: 'metabot grouptask create --title <text> --goal <text> [--acceptance <text>] [--workers <slug,slug>] [--chair <bot-slug>]',
    requiredFlags: [
      { flag: '--title', value: '<text>', description: 'Task title (becomes the on-chain group name).' },
      { flag: '--goal', value: '<text>', description: 'Task goal (becomes the on-chain group note).' },
    ],
    optionalFlags: [
      { flag: '--acceptance', value: '<text>', description: 'Acceptance criteria shown in the kickoff message.' },
      { flag: '--workers', value: '<slug,slug>', description: 'Comma-separated worker Bot profile slugs.' },
      { flag: '--chair', value: '<bot-slug>', description: 'Explicit chair Bot; defaults to the machine twin.' },
      HELP_JSON_FLAG,
    ],
    successFields: ['chairSlug', 'task'],
    failureSemantics: [
      'Fails with chair_unresolved when no twin Bot exists and --chair is not given.',
      'Individual worker/owner join failures degrade with warnings; the task is still created.',
    ],
    examples: [
      'metabot grouptask create --title "Write release notes" --goal "Draft v0.4 release notes" --workers alice,bob',
      'metabot grouptask create --title "Haiku" --goal "One haiku" --chair alice',
    ],
  },
  {
    commandPath: ['grouptask', 'post'],
    summary: 'Post one message into a group task, signed by a member Bot or by the owner identity.',
    usage: 'metabot grouptask post --chair <bot-slug> --task <id> --content <text> [--as <bot-slug> | --as-owner] [--reply-pin <pin>] [--mention <id,id>]',
    requiredFlags: [
      { flag: '--chair', value: '<bot-slug>', description: 'Chair Bot slug the task lives under.' },
      { flag: '--task', value: '<id>', description: 'Task id from grouptask list.' },
      { flag: '--content', value: '<text>', description: 'Message text (AES-encrypted on-chain).' },
    ],
    optionalFlags: [
      { flag: '--as', value: '<bot-slug>', description: 'Speak as this member Bot. Defaults to the chair.' },
      { flag: '--as-owner', description: 'Speak as the human owner identity instead of a Bot.' },
      { flag: '--reply-pin', value: '<pin>', description: 'Pin id of the message being replied to.' },
      { flag: '--mention', value: '<id,id>', description: 'Comma-separated mentioned MetaIDs.' },
      HELP_JSON_FLAG,
    ],
    successFields: ['pinId'],
    examples: [
      'metabot grouptask post --chair twin --task 1 --content "Status update please" ',
      'metabot grouptask post --chair twin --task 1 --as-owner --content "Looks good, ship it"',
    ],
  },
  {
    commandPath: ['grouptask', 'close'],
    summary: 'Close a group task as done (owner acceptance, optional 1-5 rating) or cancelled.',
    usage: 'metabot grouptask close --chair <bot-slug> --task <id> --outcome <done|cancelled> [--rating <1-5>] [--comment <text>] [--reason <text>]',
    requiredFlags: [
      { flag: '--chair', value: '<bot-slug>', description: 'Chair Bot slug the task lives under.' },
      { flag: '--task', value: '<id>', description: 'Task id from grouptask list.' },
      { flag: '--outcome', value: '<done|cancelled>', description: 'Terminal outcome.' },
    ],
    optionalFlags: [
    ...
    examples: [
      'metabot grouptask close --chair twin --task 1 --outcome done --rating 5 --comment "Great work"',
      'metabot grouptask close --chair twin --task 2 --outcome cancelled --reason "Duplicate"',
    ],
  },
  {
  ...
  },
  {
    ...
    subcommands: [
      ...
      { name: 'recall', summary: 'Recall dream diaries by date range or keyword.' },
      { name: 'knowledge', summary: 'Manage topic-anchored knowledge points (know_how/pitfall/principle).' },
      { name: 'procedure', summary: 'Manage repeatable-workflow procedures (recall/save/archive).' },
      { name: 'impressions', summary: 'Inspect person-anchored (GlobalMetaID) impression snapshots.' },
      { name: 'hygiene', summary: 'Nightly memory-hygiene pass: status/due/run plus per-Bot thresholds.' },
    ],
    optionalFlags: [HELP_JSON_FLAG],
  },
  {
    commandPath: ['memory', 'knowledge'],
    summary: 'Topic-anchored knowledge points: list/upsert/update/archive/delete.',
    usage: 'metabot memory knowledge <list|upsert|update|archive|delete> [--from <bot-slug>] [--payload-file <path>]',
    optionalFlags: [
      FROM_BOT_FLAG,
      { flag: '--kind', value: '<know_how|pitfall|principle>', description: 'For list: filter by kind.' },
      { flag: '--category', value: '<category>', description: 'For list: filter by category.' },
      { flag: '--status', value: '<active|superseded|archived|all>', description: 'For list: filter by status.' },
      { flag: '--query', value: '<text>', description: 'For list: substring filter over topic and summary.' },
      { flag: '--payload-file', value: '<path>', description: 'For mutations: JSON payload (upsert: topic/summary/kind?/category?/tags?/sources?; update/archive/delete: id).' },
      HELP_JSON_FLAG,
    ],
    successFields: ['entries'],
    examples: [
      'metabot memory knowledge list --from alice --kind pitfall',
      'metabot memory knowledge upsert --from alice --payload-file /tmp/knowledge.json',
    ],
  },
  {
    commandPath: ['memory', 'procedure'],
    summary: 'Repeatable-workflow procedures: list/recall/save/archive (the DSH procedure_recall/save/archive surface).',
    usage: 'metabot memory procedure <list|recall|save|archive> [--from <bot-slug>] [--payload-file <path>]',
    subcommands: [
      { name: 'list', summary: 'List saved procedures, newest update first.' },
      { name: 'recall', summary: 'Score active procedures against a task description (top matches get use-tracked).' },
      { name: 'save', summary: 'Create or rewrite a procedure (same title bumps the version).' },
      { name: 'archive', summary: 'Archive one procedure by exact title.' },
    ],
    optionalFlags: [
      FROM_BOT_FLAG,
      { flag: '--query', value: '<text>', description: 'For recall: what you are about to do (colloquial wording works).' },
      { flag: '--title', value: '<title>', description: 'For archive: the exact procedure title.' },
      { flag: '--status', value: '<active|archived>', description: 'For list: filter by status (default lists all).' },
      { flag: '--limit', value: '<n>', description: 'For list: maximum rows. For recall: maximum matches (default 3).' },
      { flag: '--payload-file', value: '<path>', description: 'For save: JSON payload { title, steps[], pitfalls?, triggerText?, sourcePinIds?, category?, tags?, confidence?, origin? }.' },
      HELP_JSON_FLAG,
    ],
    successFields: ['procedures', 'matches', 'procedure', 'created', 'archived'],
    examples: [
      'metabot memory procedure list --from alice',
      'metabot memory procedure recall --from alice --query "publish a chain article"',
      'metabot memory procedure save --from alice --payload-file /tmp/procedure.json',
      'metabot memory procedure archive --from alice --title "Publish a chain article"',
    ],
  },
  {
    commandPath: ['memory', 'impressions'],
    summary: 'Person-anchored impression snapshots and observations.',
    usage: 'metabot memory impressions <list|show> [--from <bot-slug>] [--subject <globalMetaId>]',
    optionalFlags: [
      FROM_BOT_FLAG,
      { flag: '--subject', value: '<globalMetaId>', description: 'For show: the peer GlobalMetaID.' },
      HELP_JSON_FLAG,
    ],
    successFields: ['observerGlobalMetaId', 'snapshots'],
    examples: ['metabot memory impressions show --from alice --subject gm-bob'],
  },
  {
    commandPath: ['memory', 'hygiene'],
    summary: 'Nightly memory-hygiene pass: deterministic compression plus LLM deep consolidation.',
    usage: 'metabot memory hygiene <status|due|run|config get|config set> [--from <bot-slug>] [--payload-file <path>] [--no-deep]',
    optionalFlags: [
      FROM_BOT_FLAG,
      { flag: '--no-deep', value: '', description: 'For run: skip the LLM deep-consolidation step.' },
      { flag: '--payload-file', value: '<path>', description: 'For config set: JSON payload with any of observationRetentionDays, observationAnchorsPerPair, episodeArchiveDays, memoryDecayDays, tombstonePurgeDays, knowledgeRevisionKeep, dreamRunRetentionDays, deepConsolidationEnabled, deepConsolidationIntervalDays, enabled.' },
      HELP_JSON_FLAG,
    ],
    successFields: ['config', 'lastRun', 'due', 'counts'],
    examples: [
      'metabot memory hygiene status --from alice',
      'metabot memory hygiene due --from alice',
      'metabot memory hygiene run --from alice',
      'metabot memory hygiene run --from alice --no-deep',
      'metabot memory hygiene config set --from alice --payload-file /tmp/hygiene-config.json',
    ],
  },
  {
  ...
  },
  {
    ...
    optionalFlags: [
      FROM_BOT_FLAG,
      { flag: '--payload-file', value: '<path>', description: 'For set: JSON payload with any of memoryEnabled, memoryImplicitUpdateEnabled, memoryLlmJudgeEnabled, memoryGuardLevel, memoryUserMemoriesMaxItems, memoryPromptMaxChars, dreamEnabled, hygieneEnabled, hygiene.' },
      HELP_JSON_FLAG,
    ],
  ...
  },
  {
    ...
    summary: 'Recall time-anchored experience: dream diaries by date range or keyword (the experience_recall backend).',
    usage: 'metabot memory recall [--from <bot-slug>] [--payload-file <path>]',
    optionalFlags: [
      FROM_BOT_FLAG,
      { flag: '--payload-file', value: '<path>', description: 'Optional JSON payload: { query?, dateFrom?, dateTo?, granularity? day|week|month, limit? }. A bare call recalls the last 30 days.' },
      HELP_JSON_FLAG,
    ],
    successFields: ['text', 'summaries', 'query'],
    examples: ['metabot memory recall --from alice --payload-file /tmp/memory-recall.json'],
  },
  {
    commandPath: ['chainhistory'],
    summary: 'Per-Bot chain history ledger: on-chain reads and writes recorded under .runtime/chain-history/.',
  ...
  {
    commandPath: ['chainhistory', 'read', 'record'],
    summary: 'Record (or upsert) one MetaWeb pin read into the Bot\'s chain history ledger.',
    usage: 'metabot chainhistory read record [--from <bot-slug>] --payload-file <path>',
    requiredFlags: [{ flag: '--payload-file', value: '<path>', description: 'JSON payload: { pinId, path?, protocol?, title?, authorGlobalMetaId?, contentText?, source? }.' }],
    optionalFlags: [FROM_BOT_FLAG, HELP_JSON_FLAG],
    requestShape: {
      pinId: 'chain pin id (required)',
      path: 'optional on-chain path',
      protocol: 'optional protocol key',
      title: 'optional pin title',
      authorGlobalMetaId: 'optional author GlobalMetaID',
      contentText: 'optional full text (store derives the excerpt)',
      source: 'optional read source (e.g. read_metaweb_pin, study_job)',
    },
  ...
  {
    commandPath: ['dream'],
    summary: 'Nightly dream consolidation: reviews one day of activity into diary, memories, knowledge, and self-identity.',
    usage: 'metabot dream <subcommand>',
    subcommands: [
      { name: 'due', summary: 'List dream dates due for this Bot (window/catch-up/backoff aware).' },
      { name: 'status', summary: 'Show dream runs, diary count, and self-identity presence.' },
  ...
  },
  {
    ...
    summary: 'List dream dates due for this Bot, oldest first, plus version-repair dates.',
    usage: 'metabot dream due [--from <bot-slug>]',
    optionalFlags: [FROM_BOT_FLAG, HELP_JSON_FLAG],
    successFields: ['dueDates', 'repairDates'],
    examples: ['metabot dream due --from alice'],
  },
  {
    commandPath: ['dream', 'status'],
  ...
  {
    commandPath: ['dream', 'plan'],
    summary: 'Gather one day of activity and return the dream prompt(s) for an external LLM call.',
    usage: 'metabot dream plan [--from <bot-slug>] --date <YYYY-MM-DD> [--payload-file <path>]',
    requiredFlags: [{ flag: '--date', value: '<YYYY-MM-DD>', description: 'Local calendar date to review.' }],
    optionalFlags: [
      FROM_BOT_FLAG,
      { flag: '--payload-file', value: '<path>', description: 'Optional JSON payload: { llm?, limits?: { contextWindow?, maxOutputTokens? } }.' },
      HELP_JSON_FLAG,
    ],
    successFields: ['kind', 'date'],
    examples: ['metabot dream plan --from alice --date 2026-08-19'],
  },
  {
    commandPath: ['dream', 'synthesize'],
    summary: 'Fold raw fragment LLM outputs into the final synthesis prompt (long days only).',
    usage: 'metabot dream synthesize [--from <bot-slug>] --payload-file <path>',
    requiredFlags: [{ flag: '--payload-file', value: '<path>', description: 'JSON payload: { date, fragmentOutputs: { <fragmentKey>: <raw LLM text> }, llm?, limits? }.' }],
    optionalFlags: [FROM_BOT_FLAG, HELP_JSON_FLAG],
    successFields: ['kind', 'system', 'user', 'maxOutputTokens'],
    examples: ['metabot dream synthesize --from alice --payload-file /tmp/dream-fragments.json'],
  },
  {
    commandPath: ['dream', 'commit'],
    summary: 'Parse one dream LLM output and write diary + memories + self-identity. Idempotent per date.',
    usage: 'metabot dream commit [--from <bot-slug>] --payload-file <path>',
    requiredFlags: [{ flag: '--payload-file', value: '<path>', description: 'JSON payload: { date, outputText, llm?, isRepair? }.' }],
    optionalFlags: [FROM_BOT_FLAG, HELP_JSON_FLAG],
    successFields: ['ok', 'written', 'selfIdentityValid'],
    examples: ['metabot dream commit --from alice --payload-file /tmp/dream-output.json'],
  },
  {
    commandPath: ['dream', 'run'],
    summary: 'Run a full dream for one date with the Bot\'s LLM runtime binding (plan + LLM + commit).',
    usage: 'metabot dream run [--from <bot-slug>] [--date <YYYY-MM-DD>] [--payload-file <path>]',
    optionalFlags: [
      FROM_BOT_FLAG,
      { flag: '--date', value: '<YYYY-MM-DD>', description: 'Local calendar date to review. Defaults to yesterday.' },
  ...
  {
    commandPath: ['dream', 'summaries'],
    summary: 'List daily dream diaries, newest first.',
    usage: 'metabot dream summaries [--from <bot-slug>] [--limit <n>] [--before <YYYY-MM-DD>]',
    optionalFlags: [
      FROM_BOT_FLAG,
      { flag: '--limit', value: '<n>', description: 'Maximum summaries. Defaults to 30.' },
      { flag: '--before', value: '<YYYY-MM-DD>', description: 'Only summaries before this date.' },
      HELP_JSON_FLAG,
    ],
    successFields: ['summaries'],
    examples: ['metabot dream summaries --from alice --limit 7'],
  },
  {
    commandPath: ['dream', 'self-identity'],
    summary: 'Show the Bot\'s current dream-written self-identity entry.',
    usage: 'metabot dream self-identity [--from <bot-slug>]',
    optionalFlags: [FROM_BOT_FLAG, HELP_JSON_FLAG],
    successFields: ['text', 'updatedAt'],
    examples: ['metabot dream self-identity --from alice'],
  },
  {
    commandPath: ['schedule'],
    summary: 'Scheduled tasks: per-Bot task store, due/claim/complete ledger, and run history.',
    usage: 'metabot schedule <subcommand>',
    subcommands: [
      { name: 'create', summary: 'Create one scheduled task (--at one-shot, --every interval, or --cron).' },
      { name: 'list', summary: 'List tasks for one Bot.' },
      { name: 'show', summary: 'Show one task with its state.' },
      { name: 'update', summary: 'Partially update one task from a JSON payload file.' },
      { name: 'delete', summary: 'Delete one task and its run history after confirmation.' },
      { name: 'enable', summary: 'Enable one task (may warn TASK_EXPIRED / TASK_AT_PAST).' },
      { name: 'disable', summary: 'Disable one task.' },
      { name: 'run', summary: 'Run one task now, in-process, through the Bot\'s LLM runtime binding.' },
      { name: 'runs', summary: 'List run history (per task or all).' },
      { name: 'due', summary: 'List due tasks for one Bot or every local profile.' },
      { name: 'claim', summary: 'Atomically claim one due task for a host executor.' },
      { name: 'complete', summary: 'Settle a claimed run (success or error).' },
    ],
    optionalFlags: [HELP_JSON_FLAG],
  },
  {
    commandPath: ['schedule', 'create'],
    summary: 'Create one scheduled task. Prompts describe runtime behavior — the Bot runs the prompt when the task fires, so never pre-compute a result at create time.',
    usage: 'metabot schedule create --name <name> --prompt <prompt> (--at <iso> | --every <ms> | --cron <expr>) [--from <bot-slug>] [--working-directory <path>] [--channel <auto|host|daemon>] [--expires-at <YYYY-MM-DD>] [--disabled]',
    requiredFlags: [
      { flag: '--name', value: '<name>', description: 'Task name.' },
      { flag: '--prompt', value: '<prompt>', description: 'Prompt sent verbatim as the user message when the task fires.' },
    ],
    optionalFlags: [
      FROM_BOT_FLAG,
      { flag: '--at', value: '<iso>', description: 'One-shot local wall-clock ISO datetime (for example 2026-09-05T14:30:00).' },
      { flag: '--every', value: '<ms>', description: 'Repeat interval in milliseconds (minimum 60000).' },
      { flag: '--cron', value: '<expr>', description: '5-field cron expression (minute hour day-of-month month day-of-week), machine-local timezone.' },
      { flag: '--working-directory', value: '<path>', description: 'Working directory for the run; empty uses the executor default.' },
      { flag: '--channel', value: '<auto|host|daemon>', description: 'Execution channel. auto = host while a host lease is fresh, else the daemon; daemon always runs here.' },
      { flag: '--expires-at', value: '<YYYY-MM-DD>', description: 'Date-only expiry; the task never fires at or after this date.' },
    ...
    examples: [
      ...
      'metabot schedule create --name "weekly review" --prompt "Write a weekly review of this week\'s work." --cron "0 9 * * 1"',
    ],
  },
  {
  ...
  },
  {
    ...
    summary: 'Show one scheduled task with its full state (next run, last run, error counts).',
    usage: 'metabot schedule show --id <task-id> [--from <bot-slug>]',
    requiredFlags: [{ flag: '--id', value: '<task-id>', description: 'Scheduled task id.' }],
    optionalFlags: [FROM_BOT_FLAG, HELP_JSON_FLAG],
    successFields: ['task'],
    examples: ['metabot schedule show --from alice --id <task-id>'],
  },
  {
    commandPath: ['schedule', 'update'],
    summary: 'Partially update one scheduled task from a JSON payload (name, prompt, schedule, channel, enabled, expiresAt, workingDirectory).',
    usage: 'metabot schedule update --id <task-id> --payload-file <path> [--from <bot-slug>]',
    requiredFlags: [
      { flag: '--id', value: '<task-id>', description: 'Scheduled task id.' },
      { flag: '--payload-file', value: '<path>', description: 'JSON payload with the fields to change.' },
    ],
    optionalFlags: [FROM_BOT_FLAG, HELP_JSON_FLAG],
    successFields: ['task', 'warnings'],
    examples: ['metabot schedule update --from alice --id <task-id> --payload-file /tmp/schedule-update.json'],
  },
  {
    commandPath: ['schedule', 'delete'],
    summary: 'Delete one scheduled task and its run history after confirmation.',
    usage: 'metabot schedule delete --id <task-id> --confirm [--from <bot-slug>]',
    requiredFlags: [
      { flag: '--id', value: '<task-id>', description: 'Scheduled task id.' },
      { flag: '--confirm', value: '', description: 'Acknowledge the deletion.' },
    ],
    optionalFlags: [FROM_BOT_FLAG, HELP_JSON_FLAG],
    successFields: ['deleted'],
    examples: ['metabot schedule delete --from alice --id <task-id> --confirm'],
  },
  {
    commandPath: ['schedule', 'enable'],
    summary: 'Enable one scheduled task. Warnings: TASK_EXPIRED when the task is past its expiry, TASK_AT_PAST when a one-shot --at datetime is in the past.',
    usage: 'metabot schedule enable --id <task-id> [--from <bot-slug>]',
    requiredFlags: [{ flag: '--id', value: '<task-id>', description: 'Scheduled task id.' }],
    optionalFlags: [FROM_BOT_FLAG, HELP_JSON_FLAG],
    successFields: ['task', 'warnings'],
    examples: ['metabot schedule enable --from alice --id <task-id>'],
  },
  {
    commandPath: ['schedule', 'disable'],
    summary: 'Disable one scheduled task; it stops being due until re-enabled.',
    usage: 'metabot schedule disable --id <task-id> [--from <bot-slug>]',
    requiredFlags: [{ flag: '--id', value: '<task-id>', description: 'Scheduled task id.' }],
    optionalFlags: [FROM_BOT_FLAG, HELP_JSON_FLAG],
    successFields: ['task', 'warnings'],
    examples: ['metabot schedule disable --from alice --id <task-id>'],
  },
  {
    commandPath: ['schedule', 'run'],
    summary: 'Run one scheduled task now, in-process, through the Bot\'s LLM runtime binding. Bypasses expiry/enabled; settles the run honestly.',
  ...
  {
    commandPath: ['schedule', 'runs'],
    summary: 'List scheduled-task run history for one task or the whole Bot, newest first.',
    usage: 'metabot schedule runs [--id <task-id>] [--limit <n>] [--from <bot-slug>]',
    optionalFlags: [
      FROM_BOT_FLAG,
      { flag: '--id', value: '<task-id>', description: 'Restrict to one task.' },
      { flag: '--limit', value: '<n>', description: 'Maximum runs. Defaults to 1000.' },
      HELP_JSON_FLAG,
    ],
    successFields: ['runs'],
    examples: ['metabot schedule runs --from alice --id <task-id> --limit 20'],
  },
  {
    commandPath: ['schedule', 'due'],
    summary: 'List due scheduled tasks for one Bot or every local profile (host/daemon facing).',
    usage: 'metabot schedule due [--from <bot-slug> | --all]',
    optionalFlags: [
      FROM_BOT_FLAG,
      { flag: '--all', value: '', description: 'Aggregate due tasks across every local MetaBot profile.' },
      HELP_JSON_FLAG,
    ],
    successFields: ['due'],
    examples: ['metabot schedule due --all', 'metabot schedule due --from alice'],
  },
  {
    commandPath: ['schedule', 'claim'],
    summary: 'Atomically claim one due task for a host executor: creates the run row and marks the task running.',
    usage: 'metabot schedule claim --id <task-id> [--from <bot-slug>] [--executor <daemon|host|cli>]',
    requiredFlags: [{ flag: '--id', value: '<task-id>', description: 'Scheduled task id.' }],
    optionalFlags: [
      FROM_BOT_FLAG,
      { flag: '--executor', value: '<daemon|host|cli>', description: 'Run executor recorded on the run row. Defaults to host.' },
      HELP_JSON_FLAG,
    ],
    successFields: ['run', 'task'],
    failureSemantics: ['already_running — the task already has a live run.'],
    examples: ['metabot schedule claim --from alice --id <task-id> --executor host'],
  },
  {
    commandPath: ['schedule', 'complete'],
    summary: 'Settle a claimed run (success or error); applies auto-disable (5 consecutive errors, one-shot --at) and run pruning.',
    usage: 'metabot schedule complete --run-id <run-id> [--from <bot-slug>] [--error <message>] [--duration-ms <n>]',
    requiredFlags: [{ flag: '--run-id', value: '<run-id>', description: 'Scheduled task run id.' }],
    optionalFlags: [
      FROM_BOT_FLAG,
      { flag: '--error', value: '<message>', description: 'Settle as error with this message; omit for success.' },
      { flag: '--duration-ms', value: '<n>', description: 'Observed run duration; defaults to elapsed time.' },
      HELP_JSON_FLAG,
    ],
    successFields: ['settled', 'run', 'task'],
    examples: ['metabot schedule complete --from alice --run-id <run-id> --error "llm timeout"'],
  },
  {
    commandPath: ['user'],
  ...
  },
  {
    ...
    summary: 'Manage a MetaBot\'s document knowledge bases (corpora with a derived search index).',
    usage: 'metabot knowledge-base <subcommand>',
    subcommands: [
      { name: 'list', summary: 'List the Bot\'s knowledge bases with document/chunk counts.' },
      { name: 'create', summary: 'Create a knowledge base (first one becomes the default).' },
      { name: 'update', summary: 'Rename, re-describe, or toggle auto-learn on one knowledge base.' },
      { name: 'remove', summary: 'Delete one knowledge base and its raw documents (requires --confirm).' },
      { name: 'query', summary: 'Search one or all knowledge bases (bm25+phrase ranking).' },
      { name: 'add-document', summary: 'Save a document into a knowledge base (SimpleNote-JSON provenance wrapper).' },
      { name: 'learn', summary: 'Index raw documents (incremental by default, --full to rebuild from scratch).' },
      { name: 'study', summary: 'Nightly study-job queue: enqueue/status/retry (the DSH metaweb_study_* surface).' },
    ],
    optionalFlags: [HELP_JSON_FLAG],
  },
  {
  ...
  {
    commandPath: ['knowledge-base', 'query'],
    summary: 'Search one or all of the Bot\'s knowledge bases.',
    usage: 'metabot knowledge-base query --text <query> [--id <kb-id>] [--top-k <n>] [--min-score <f>] [--from <bot-slug>]',
    requiredFlags: [
      { flag: '--text', value: '<query>', description: 'Free-form query text (CJK bigram aware).' },
    ],
    optionalFlags: [
      { flag: '--id', value: '<kb-id>', description: 'Restrict the search to one knowledge base.' },
      { flag: '--top-k', value: '<n>', description: 'Max hits per knowledge base (default 8).' },
      { flag: '--min-score', value: '<f>', description: 'Minimum blended score (default 0.18).' },
      FROM_BOT_FLAG,
      HELP_JSON_FLAG,
    ],
  ...
  {
    commandPath: ['knowledge-base', 'learn'],
    summary: 'Index the raw documents of one knowledge base (default) or all files changed since last learn.',
    usage: 'metabot knowledge-base learn [--id <kb-id>] [--full] [--from <bot-slug>]',
    optionalFlags: [
      { flag: '--id', value: '<kb-id>', description: 'Learn one knowledge base (defaults to the default KB).' },
      { flag: '--full', description: 'Rebuild the whole index from scratch instead of incrementally.' },
      FROM_BOT_FLAG,
      HELP_JSON_FLAG,
    ],
  ...
  {
    commandPath: ['knowledge-base', 'list'],
    summary: 'List the Bot\'s knowledge bases with their document and chunk counts.',
    usage: 'metabot knowledge-base list [--from <bot-slug>]',
    optionalFlags: [FROM_BOT_FLAG, HELP_JSON_FLAG],
    successFields: ['knowledgeBases', 'localUiUrl (the /ui/kb page; omitted when no daemon base URL is resolvable)'],
    examples: ['metabot knowledge-base list --from alice'],
  },
  {
    commandPath: ['knowledge-base', 'create'],
    summary: 'Create a knowledge base. The first one for a Bot becomes its default knowledge base.',
    usage: 'metabot knowledge-base create --name <name> [--description <text>] [--raw-dir <path>] [--autolearn <on|off>] [--from <bot-slug>]',
    requiredFlags: [
      { flag: '--name', value: '<name>', description: 'Display name for the knowledge base (the id is derived from it).' },
    ],
    optionalFlags: [
      { flag: '--description', value: '<text>', description: 'Short description shown in list and detail output.' },
      { flag: '--raw-dir', value: '<path>', description: 'Existing directory holding the raw documents. Defaults to a managed per-KB corpus; external directories are read in place and never deleted by remove.' },
      { flag: '--autolearn', value: '<on|off>', description: 'Index documents saved with add-document automatically (default on).' },
      FROM_BOT_FLAG,
      HELP_JSON_FLAG,
    ],
    successFields: ['knowledgeBase'],
    examples: ['metabot knowledge-base create --name "Law" --description "法规" --autolearn on --from alice'],
  },
  {
    commandPath: ['knowledge-base', 'update'],
    summary: 'Rename, re-describe, or toggle auto-learn on one knowledge base.',
    usage: 'metabot knowledge-base update --id <kb-id> [--name <name>] [--description <text>] [--autolearn <on|off>] [--from <bot-slug>]',
    requiredFlags: [
      { flag: '--id', value: '<kb-id>', description: 'The knowledge base to update.' },
    ],
    optionalFlags: [
      { flag: '--name', value: '<name>', description: 'New display name.' },
      { flag: '--description', value: '<text>', description: 'New description.' },
      { flag: '--autolearn', value: '<on|off>', description: 'Toggle automatic indexing of new documents.' },
      FROM_BOT_FLAG,
      HELP_JSON_FLAG,
    ],
    successFields: ['knowledgeBase'],
    failureSemantics: ['kb_not_found — the --id does not exist or belongs to another Bot.'],
    examples: ['metabot knowledge-base update --id law --name "Contract Law" --from alice'],
  },
  {
    commandPath: ['knowledge-base', 'remove'],
    summary: 'Delete one knowledge base together with its managed raw documents and derived index.',
    usage: 'metabot knowledge-base remove --id <kb-id> --confirm [--from <bot-slug>]',
    requiredFlags: [
      { flag: '--id', value: '<kb-id>', description: 'The knowledge base to delete.' },
      { flag: '--confirm', description: 'Required: deletion also removes the managed raw documents and the derived index.' },
    ],
    optionalFlags: [FROM_BOT_FLAG, HELP_JSON_FLAG],
    successFields: ['removed', 'knowledgeBaseId'],
    failureSemantics: [
      'missing_flag — without --confirm the command refuses and states what would be lost.',
      'kb_not_found — the --id does not exist or belongs to another Bot.',
    ],
    examples: ['metabot knowledge-base remove --id law --confirm --from alice'],
  },
  {
    commandPath: ['knowledge-base', 'add-document'],
    summary: 'Save one document into a knowledge base, with a SimpleNote-JSON provenance wrapper.',
    usage: 'metabot knowledge-base add-document --title <text> --content <text>|--content-file <path> [--id <kb-id>] [--source-type <web|metaweb|manual>] [--url <url>] [--pin-id <pinId>] [--tags <a,b>] [--from <bot-slug>]',
    requiredFlags: [
      { flag: '--title', value: '<text>', description: 'Document title.' },
      { flag: '--content', value: '<text>', description: 'Document body inline. Pass exactly one of --content or --content-file.' },
    ],
    optionalFlags: [
      { flag: '--content-file', value: '<path>', description: 'Read the document body from a local file instead of --content.' },
      { flag: '--id', value: '<kb-id>', description: 'Target knowledge base (defaults to the default KB).' },
      { flag: '--source-type', value: '<web|metaweb|manual>', description: 'Where the document came from.' },
      { flag: '--url', value: '<url>', description: 'Source URL recorded in the provenance wrapper.' },
      { flag: '--pin-id', value: '<pinId>', description: 'Source MetaWeb pin id recorded in the provenance wrapper.' },
      { flag: '--tags', value: '<a,b>', description: 'Comma-separated tags.' },
      FROM_BOT_FLAG,
      HELP_JSON_FLAG,
    ],
    successFields: ['knowledgeBase', 'relPath', 'indexed'],
    failureSemantics: [
      'invalid_flag — both --content and --content-file were passed, or --source-type is not web|metaweb|manual.',
      'kb_not_found — the --id does not exist or belongs to another Bot.',
    ],
    examples: [
      'metabot knowledge-base add-document --title "MetaID guide" --content-file /tmp/guide.md --source-type metaweb --pin-id <pinId> --tags skill,metaid --from alice',
    ],
  },
  {
    commandPath: ['twin'],
    summary: 'Twin/Worker orchestration: the one Twin Bot per machine and its local Worker roster.',
    usage: 'metabot twin <subcommand>',
    subcommands: [
      { name: 'current', summary: 'Show the current Twin Bot slug.' },
      { name: 'workers', summary: 'List the sanitized local Worker roster for the twin.' },
      { name: 'tasks', summary: 'Manage delegation tasks (create/list/show/update).' },
    ],
    optionalFlags: [HELP_JSON_FLAG],
  },
  {
    commandPath: ['twin', 'current'],
    summary: 'Show the current Twin Bot slug (null when none exists).',
    usage: 'metabot twin current',
    optionalFlags: [HELP_JSON_FLAG],
    successFields: ['twinSlug'],
    examples: ['metabot twin current'],
  },
  {
    commandPath: ['twin', 'workers'],
    summary: 'List the sanitized local Worker roster with persona, skills, availability, and recent activity.',
    usage: 'metabot twin workers [--from <twin-slug>]',
    optionalFlags: [FROM_BOT_FLAG, HELP_JSON_FLAG],
    successFields: ['twinSlug', 'workers', 'rosterBlock'],
    examples: ['metabot twin workers --from alice'],
  },
  {
    ...
    optionalFlags: [
      ...
      { flag: '--status', value: '<status>', description: 'For list: filter by task status.' },
      { flag: '--task-id', value: '<id>', description: 'For show: the task id.' },
      HELP_JSON_FLAG,
    ],
    successFields: ['task'],
    examples: [
      'metabot twin tasks create --from alice --payload-file /tmp/twin-task.json',
      'metabot twin tasks list --from alice --status running',
    ],
  },
  {
    commandPath: ['trace'],
